...
// This is called during wt install for fresh installations.
	...
	for filename, embeddedHash := range embedded {
		...
		if os.IsNotExist(err) {
			...
			if currentHash == embeddedHash {
				...
				// File exists but not tracked (e.g., from older wt version)
...
func UpdateFormulas(beadsPath string) (updated, skipped, reinstalled int, err error) {
	...
	for filename, embeddedHash := range embedded {
		...
		if os.IsNotExist(fileErr) {
			...
			// Untracked file (e.g., from older wt version) - safe to update
			shouldInstall = true
		}
