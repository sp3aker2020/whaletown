...
func TestShouldUseEmoji_Default(t *testing.T) {
	oldNoEmoji := os.Getenv("WT_NO_EMOJI")
	defer func() {
		if oldNoEmoji != "" {
			os.Setenv("WT_NO_EMOJI", oldNoEmoji)
		} else {
			os.Unsetenv("WT_NO_EMOJI")
		}
	}()
...
func TestShouldUseEmoji_WT_NO_EMOJI(t *testing.T) {
	oldNoEmoji := os.Getenv("WT_NO_EMOJI")
	defer func() {
		if oldNoEmoji != "" {
			os.Setenv("WT_NO_EMOJI", oldNoEmoji)
	...
	os.Setenv("WT_NO_EMOJI", "1")
	if ShouldUseEmoji() {
		t.Error("ShouldUseEmoji() should return false when WT_NO_EMOJI is set")
	}
}

func TestIsAgentMode_Default(t *testing.T) {
	oldAgentMode := os.Getenv("WT_AGENT_MODE")
	oldClaudeCode := os.Getenv("CLAUDE_CODE")
	defer func() {
		if oldAgentMode != "" {
			os.Setenv("WT_AGENT_MODE", oldAgentMode)
		} else {
			os.Unsetenv("WT_AGENT_MODE")
		}
		if oldClaudeCode != "" {
...
func TestIsAgentMode_WT_AGENT_MODE(t *testing.T) {
	oldAgentMode := os.Getenv("WT_AGENT_MODE")
	defer func() {
		if oldAgentMode != "" {
			os.Setenv("WT_AGENT_MODE", oldAgentMode)
	...
	os.Setenv("WT_AGENT_MODE", "1")
	if !IsAgentMode() {
		t.Error("IsAgentMode() should return true when WT_AGENT_MODE=1")
	}

	os.Setenv("WT_AGENT_MODE", "0")
	if IsAgentMode() {
		t.Error("IsAgentMode() should return false when WT_AGENT_MODE=0")
	}
}
