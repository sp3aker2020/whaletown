...
func (m *Manager) Start(agentOverride string) error {
	...
	// Build startup beacon with explicit instructions (matches wt handoff behavior)
	...
	// Build startup command WITH the beacon prompt - the startup hook handles 'wt prime' automatically
	...
	// Create session in townRoot (not mayorDir) to match wt handoff behavior
	...
	_ = t.ConfigureWhaleTownSession(sessionID, theme, "", "Mayor", "coordinator")
