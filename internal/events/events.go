// Package events provides event logging for the wt activity feed.
//
...
// Event represents an activity event in Whale Town.
type Event struct {
...
// Common event types for wt commands.
const (
	TypeSling   = "sling"
	TypeHook    = "hook"
	TypeUnhook  = "unhook"
...
func write(event Event) error {
	...
	if err != nil || townRoot == "" {
		// Silently ignore - we're not in a Whale Town workspace
		return nil
	}
...
// agent: Whale Town agent identity (e.g., "whaletown/polecats/Toast")
...
// role: Whale Town role (e.g., "whaletown/crew/joe", "deacon")
