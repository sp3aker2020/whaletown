...
// If so, runs wt convoy check to close the convoy.
func (w *ConvoyWatcher) checkConvoyCompletion(convoyID string) {
	...
	// Run wt convoy check to handle the completion
	...
	checkCmd := exec.Command("wt", "convoy", "check")
	...
	if err := checkCmd.Run(); err != nil {
		w.logger("convoy watcher: wt convoy check failed: %v: %s", err, checkStderr.String())
		return
	}
