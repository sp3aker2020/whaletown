...
// BeadsMessage represents a message from wt mail inbox --json.
...
func (d *Daemon) ProcessLifecycleRequests() {
	// Get mail for deacon identity (using wt mail, not bd mail)
	cmd := exec.Command("wt", "mail", "inbox", "--identity", "deacon/", "--json")
...
func (d *Daemon) identityToSession(identity string) string {
	...
	case "witness", "refinery":
		return fmt.Sprintf("wt-%s-%s", parsed.RigName, parsed.RoleType)
	case "crew":
		return fmt.Sprintf("wt-%s-crew-%s", parsed.RigName, parsed.AgentName)
	case "polecat":
		return fmt.Sprintf("wt-%s-%s", parsed.RigName, parsed.AgentName)
	default:
		return ""
	}
}
...
func (d *Daemon) getStartCommand(roleConfig *beads.RoleConfig, parsed *ParsedIdentity) string {
	...
	if runtimeConfig.Session != nil && runtimeConfig.Session.SessionIDEnv != "" {
		defaultCmd = config.PrependEnv(defaultCmd, map[string]string{"WT_SESSION_ID_ENV": runtimeConfig.Session.SessionIDEnv})
	}
...
func (d *Daemon) applySessionTheme(sessionName string, parsed *ParsedIdentity) {
	if parsed.RoleType == "mayor" {
		...
		_ = d.tmux.ConfigureWhaleTownSession(sessionName, theme, "", "Mayor", "coordinator")
	} else if parsed.RigName != "" {
		theme := tmux.AssignTheme(parsed.RigName)
		_ = d.tmux.ConfigureWhaleTownSession(sessionName, theme, parsed.RigName, parsed.RoleType, parsed.RoleType)
	}
}
...
// We use delete instead of read because wt mail read intentionally
// doesn't mark messages as read (to preserve handoff messages).
func (d *Daemon) closeMessage(id string) error {
	// Use wt mail delete to actually remove the message
	cmd := exec.Command("wt", "mail", "delete", id)
	cmd.Dir = d.config.TownRoot
...
// progressing before it's considered a GUPP (Whale Town Universal Propulsion
...
func (d *Daemon) checkRigGUPPViolations(rigName string) {
	...
	// Pattern: <prefix>-<rig>-polecat-<name> (e.g., gt-whaletown-polecat-Toast)
	...
	// Use the rig's configured prefix (e.g., "wt" for whaletown, "bd" for beads)
...
Action needed: Check if agent is alive and responsive. Consider restarting if stuck.`,
	...
	cmd := exec.Command("wt", "mail", "send", witnessAddr, "-s", subject, "-m", body)
	cmd.Dir = d.config.TownRoot
...
// Example: gt-whaletown-polecat-max → whaletown
