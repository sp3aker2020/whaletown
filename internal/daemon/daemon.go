...
import (
	...
	"github.com/speaker20/whaletown/internal/boot"
	...
	"github.com/speaker20/whaletown/internal/feed"
...
type Daemon struct {
	...
	// See: https://github.com/speaker20/whaletown/issues/567
	...
	for {
		...
		case sig := <-sigChan:
			if isLifecycleSignal(sig) {
				// Lifecycle signal: immediate lifecycle processing (from wt handoff)
...
func (d *Daemon) ensureBootRunning() {
	...
	degraded := os.Getenv("WT_DEGRADED") == "true"
...
// The Deacon needs time to initialize Claude, run SessionStart hooks, execute wt prime,
...
func (d *Daemon) restartPolecatSession(rigName, polecatName, sessionName string) error {
	...
	_ = d.tmux.ConfigureWhaleTownSession(sessionName, theme, rigName, polecatName, "polecat")
...
Manual intervention may be required.`,
	...
	cmd := exec.Command("wt", "mail", "send", witnessAddr, "-s", subject, "-m", body) //nolint:gosec // G204: args are constructed internally
