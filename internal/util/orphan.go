...
// getWhaleTownSessionPIDs returns a set of PIDs belonging to valid Whale Town tmux sessions.
...
func getWhaleTownSessionPIDs() map[int]bool {
	pids := make(map[int]bool)

	// Get list of Whale Town tmux sessions (gt-* and hq-*)
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		if strings.HasPrefix(line, "wt-") || strings.HasPrefix(line, "hq-") {
			gasTownSessions = append(gasTownSessions, line)
		}
	}

	// For each Whale Town session, get the PIDs of processes in its panes
...
func orphanStateFile() string {
	...
	return filepath.Join(dir, "whaletown-orphan-state")
}
...
func FindOrphanedClaudeProcesses() ([]OrphanedProcess, error) {
	// Get PIDs belonging to valid Whale Town tmux sessions.
	// These should not be killed even if they show TTY "?" during startup.
	gasTownPIDs := getWhaleTownSessionPIDs()
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// Skip processes that belong to valid Whale Town tmux sessions.
