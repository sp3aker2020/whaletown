...
func TestNamePool_Overflow(t *testing.T) {
	...
	pool := NewNamePoolWithConfig(tmpDir, "whaletown", "mad-max", nil, 5)
	...
	name, _ = pool.Allocate()
	if name != "whaletown-7" {
		t.Errorf("expected whaletown-7, got %s", name)
	}
}
...
func TestNamePool_OverflowNotReusable(t *testing.T) {
	...
	pool := NewNamePoolWithConfig(tmpDir, "whaletown", "mad-max", nil, 3)
	...
	if overflow1 != "whaletown-4" {
		t.Fatalf("expected whaletown-4, got %s", overflow1)
	}

	// Release it - should not be reused
	pool.Release(overflow1)

	// Next allocation should be whaletown-5, not whaletown-4
	name, _ := pool.Allocate()
	if name != "whaletown-5" {
		t.Errorf("expected whaletown-5 (overflow increments), got %s", name)
	}
}
...
func TestNamePool_IsPoolName(t *testing.T) {
	...
	tests := []struct {
		...
		{"nux", true},
		{"max", true},
		{"whaletown-51", false}, // overflow format
		{"random-name", false},
...
func TestThemeForRig(t *testing.T) {
	...
	for _, rigName := range []string{"whaletown", "beads", "myproject", "webapp"} {
