...
// Format: "rig/polecatName" (e.g., "whaletown/Toast")
...
// Format: "<prefix>-<rig>-polecat-<name>" (e.g., "wt-whaletown-polecat-Toast", "bd-beads-polecat-obsidian")
...
func (m *Manager) AddWithOptions(name string, opts AddOptions) (*Polecat, error) {
	...
	// Whale Town internals into the project repo if merged.
...
func (m *Manager) ReconcilePool() {
	...
	if m.tmux != nil {
		...
		for _, name := range poolNames {
			sessionName := fmt.Sprintf("wt-%s-%s", m.rig.Name, name)
...
func (m *Manager) DetectStalePolecats(threshold int) ([]*StalenessInfo, error) {
	...
	for _, p := range polecats {
		...
		sessionName := fmt.Sprintf("wt-%s-%s", m.rig.Name, p.Name)
