...
func TestSessionName(t *testing.T) {
	...
	if name != "wt-whaletown-Toast" {
		t.Errorf("sessionName = %q, want wt-whaletown-Toast", name)
	}
}
...
func TestSessionManagerPolecatDir(t *testing.T) {
	r := &rig.Rig{
		Name:     "whaletown",
		Path:     "/home/user/ai/whaletown",
		Polecats: []string{"Toast"},
	}
	m := NewSessionManager(tmux.NewTmux(), r)

	dir := m.polecatDir("Toast")
	expected := "/home/user/ai/whaletown/polecats/Toast"
	if dir != expected {
...
// WT_ROLE, WT_RIG, WT_POLECAT, and BD_ACTOR inline before starting Claude.
// This is a regression test for wt-y41ep - env vars must be exported inline
...
func TestPolecatCommandFormat(t *testing.T) {
	...
	rigName := "whaletown"
	polecatName := "Toast"
	expectedBdActor := "whaletown/polecats/Toast"

	// Build the expected command format (mirrors Start() logic)
	expectedPrefix := "export WT_ROLE=polecat WT_RIG=" + rigName + " WT_POLECAT=" + polecatName + " BD_ACTOR=" + expectedBdActor + " GIT_AUTHOR_NAME=" + expectedBdActor
	...
	// Verify WT_ROLE is specifically "polecat" (not "mayor" or "crew")
	if !strings.Contains(fullCommand, "WT_ROLE=polecat") {
		t.Error("WT_ROLE must be 'polecat', not 'mayor' or 'crew'")
	}
}
