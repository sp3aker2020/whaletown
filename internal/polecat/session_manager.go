...
// debugSession logs non-fatal errors during session startup when WT_DEBUG_SESSION=1.
func debugSession(context string, err error) {
	if os.Getenv("WT_DEBUG_SESSION") != "" && err != nil {
...
func (m *SessionManager) SessionName(polecat string) string {
	return fmt.Sprintf("wt-%s-%s", m.rig.Name, polecat)
}
...
func (m *SessionManager) Start(polecat string, opts SessionStartOptions) error {
	...
	debugSession("ConfigureWhaleTownSession", m.tmux.ConfigureWhaleTownSession(sessionID, theme, m.rig.Name, polecat, "polecat"))
...
func (m *SessionManager) List() ([]SessionInfo, error) {
	...
	prefix := fmt.Sprintf("wt-%s-", m.rig.Name)
