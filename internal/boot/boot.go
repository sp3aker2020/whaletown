...
// Note: We use "wt-boot" instead of "hq-deacon-boot" to avoid tmux prefix
...
func (b *Boot) spawnDegraded() error {
	// In degraded mode, we run wt boot triage directly
	// This performs the triage logic without a full Claude session
	cmd := exec.Command("wt", "boot", "triage", "--degraded")
	cmd.Dir = b.deaconDir
	...
	cmd.Env = append(cmd.Env, "WT_DEGRADED=true")
