...
// GetRepoRoot returns the git repository root for the wt source code.
// It looks for the whaletown repo by checking known paths.
func GetRepoRoot() (string, error) {
	// First, check if WT_ROOT environment variable is set
	...
	// Check if current directory is in a whaletown repo
	...
	return "", fmt.Errorf("cannot locate wt source repository")
}
...
// hasGastownMarker checks if a directory looks like the whaletown repo.
func hasGastownMarker(dir string) bool {
	// Check for cmd/gt directory which is unique to whaletown
