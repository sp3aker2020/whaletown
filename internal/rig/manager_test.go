...
func TestDiscoverRigs(t *testing.T) {
	...
	// Create test rig
	createTestRig(t, root, "whaletown")
	rigsConfig.Rigs["whaletown"] = config.RigEntry{
		GitURL: "git@github.com:test/whaletown.git",
	}
	...
	rig := rigs[0]
	if rig.Name != "whaletown" {
		t.Errorf("Name = %q, want whaletown", rig.Name)
	}
...
func TestInitBeads_TrackedBeads_CreatesRedirect(t *testing.T) {
	...
	manager := &Manager{}
	if err := manager.initBeads(rigPath, "wt"); err != nil {
...
`
	...
	if err := manager.initAgentBeads(rigPath, "demo", "wt"); err != nil {
	...
	expectedAgents := map[string]bool{
		"wt-demo-witness":  false,
		"wt-demo-refinery": false,
	}
