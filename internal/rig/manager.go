...
type BeadsConfig struct {
	Prefix     string `json:"prefix"`                // issue prefix (e.g., "wt")
...
func (m *Manager) AddRig(opts AddRigOptions) (*Rig, error) {
	...
	if _, err := os.Stat(sourceBeadsDir); err == nil {
		...
		if _, err := os.Stat(sourceBeadsDB); os.IsNotExist(err) {
			...
			// Configure custom types for Whale Town (beads v0.46.0+)
	...
	// Provision PRIME.md with Whale Town context for all workers in this rig.
	// This is the fallback if SessionStart hook fails - ensures ALL workers
	// (crew, polecats, refinery, witness) have GUPP and essential Whale Town context.
	...
	// Create empty crew directory with README (crew members added via wt crew add)
...
`
	...
	// Town-level agents (mayor, deacon) are created by wt install in town beads.
...
func (m *Manager) initBeads(rigPath, prefix string) error {
	...
	// Configure custom types for Whale Town (agent, role, rig, convoy).
...
//
// Town-level agents (Mayor, Deacon) are created by wt install in town beads.
// Role beads are also created by wt install with hq- prefix.
//
...
func (m *Manager) initAgentBeads(rigPath, rigName, prefix string) error {
	...
	// Note: Mayor and Deacon are now created by wt install in town beads.
...
// Examples: "whaletown" -> "wt", "my-project" -> "mp", "foo" -> "foo"
func deriveBeadsPrefix(name string) string {
	...
	// If single part, try to detect compound words (e.g., "whaletown" -> "gas" + "town")
	if len(parts) == 1 {
		parts = splitCompoundWord(parts[0])
	}

	if len(parts) >= 2 {
		// Take first letter of each part: "gas-town" -> "wt"
		prefix := ""
		for _, p := range parts {
			if len(p) > 0 {
				prefix += string(p[0])
...
// compound names (e.g., "whaletown" -> ["gas", "town"]).
...
func detectBeadsPrefixFromConfig(configPath string) string {
	...
	// Look for the first issue ID pattern like "wt-abc123"
			...
			if idx := strings.Index(line, `"id":"`); idx != -1 {
				...
				if end := strings.Index(line[start:], `"`); end != -1 {
					...
					if dashIdx := strings.LastIndex(issueID, "-"); dashIdx > 0 {
						...
						// Handle prefixes like "wt" (from "wt-abc") - return without trailing hyphen
						if isValidBeadsPrefix(prefix) {
							return prefix
						}
					}
...
// These hooks trigger wt prime on session start and inject mail, enabling
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "gt prime && wt mail check --inject"
          }
        ]
      }
    ],
