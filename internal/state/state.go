// ABOUTME: Global state management for Whale Town enable/disable toggle.
...
// State represents the global Whale Town state.
type State struct {
...
// Uses ~/.local/state/whaletown/ (per XDG Base Directory Specification).
func StateDir() string {
	// Check XDG_STATE_HOME first
	if xdg := os.Getenv("XDG_STATE_HOME"); xdg != "" {
		return filepath.Join(xdg, "whaletown")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".local", "state", "whaletown")
}

// ConfigDir returns the XDG-compliant config directory.
// Uses ~/.config/whaletown/
func ConfigDir() string {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "whaletown")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "whaletown")
}

// CacheDir returns the XDG-compliant cache directory.
// Uses ~/.cache/whaletown/
func CacheDir() string {
	if xdg := os.Getenv("XDG_CACHE_HOME"); xdg != "" {
		return filepath.Join(xdg, "whaletown")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".cache", "whaletown")
}
...
// IsEnabled checks if Whale Town is globally enabled.
...
// Enable enables Whale Town globally.
...
// Disable disables Whale Town globally.
func Disable() error {
	s, err := Load()
	if err != nil {
