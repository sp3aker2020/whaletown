...
func TestStateDir(t *testing.T) {
	...
	expected := filepath.Join(home, ".local", "state", "whaletown")
	...
	if got := StateDir(); got != "/custom/state/whaletown" {
		t.Errorf("StateDir() with XDG = %q, want /custom/state/whaletown", got)
	}
}

func TestConfigDir(t *testing.T) {
	home, _ := os.UserHomeDir()
	expected := filepath.Join(home, ".config", "whaletown")
	...
	if got := ConfigDir(); got != "/custom/config/whaletown" {
		t.Errorf("ConfigDir() with XDG = %q, want /custom/config/whaletown", got)
	}
}

func TestCacheDir(t *testing.T) {
	home, _ := os.UserHomeDir()
	expected := filepath.Join(home, ".cache", "whaletown")
	...
	if got := CacheDir(); got != "/custom/cache/whaletown" {
		t.Errorf("CacheDir() with XDG = %q, want /custom/cache/whaletown", got)
	}
}
