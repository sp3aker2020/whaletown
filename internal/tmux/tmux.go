...
// when all Whale Town sessions are killed but the user has no other sessions.
...
// (e.g., "wt-deacon-boot" won't match when checking for "wt-deacon").
...
func (t *Tmux) NudgeSession(session, message string) error {
	...
	// See: https://github.com/anthropics/whaletown/issues/307
...
//	- Deacon starting polecats → use 'wt deacon pending' + AI analysis
//	- Deacon restarting → Mayor watches via 'wt peek'
//	- Mayor restarting → Deacon watches via 'wt peek'
//
// See: wt deacon pending (ZFC-compliant AI observation)
// See: wt deacon trigger-pending (bootstrap mode, regex-based)
...
func (t *Tmux) SetStatusFormat(session, rig, worker, role string) error {
	...
	// Mayor: 🎩 Mayor
	// Crew:  👷 whaletown/crew/max (full path)
	// Polecat: 😺 whaletown/Toast
	var left string
	if rig == "" {
...
func (t *Tmux) SetDynamicStatus(session string) error {
	...
	// wt status-line reads env vars and mail to build the status
...
// ConfigureWhaleTownSession applies full Whale Town theming to a session.
// This is a convenience method that applies theme, status format, and dynamic status.
func (t *Tmux) ConfigureWhaleTownSession(session string, theme Theme, rig, worker, role string) error {
...
func (t *Tmux) SetMailClickBinding(session string) error {
	...
	// The popup runs wt mail peek and closes on any key
...
// The wt cycle command automatically detects the session type and cycles
// within the appropriate group:
// - Town sessions: Mayor ↔ Deacon
// - Crew sessions: All crew members in the same rig
//
// IMPORTANT: These bindings are conditional - they only run wt cycle for
// Whale Town sessions (those starting with "wt-" or "hq-"). For non-GT sessions,
// the default tmux behavior (next-window/previous-window) is preserved.
// See: https://github.com/speaker20/whaletown/issues/13
//
// IMPORTANT: We pass #{session_name} to the command because run-shell doesn't
// reliably preserve the session context. tmux expands #{session_name} at binding
// resolution time (when the key is pressed), giving us the correct session.
func (t *Tmux) SetCycleBindings(session string) error {
	// C-b n → wt cycle next for GT sessions, next-window otherwise
	// The if-shell checks if session name starts with "wt-" or "hq-"
	if _, err := t.run("bind-key", "-T", "prefix", "n",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(gt|hq)-'",
		"run-shell 'wt cycle next --session #{session_name}'",
		"next-window"); err != nil {
		return err
	}
	// C-b p → wt cycle prev for GT sessions, previous-window otherwise
	if _, err := t.run("bind-key", "-T", "prefix", "p",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(gt|hq)-'",
		"run-shell 'wt cycle prev --session #{session_name}'",
...
//
// IMPORTANT: This binding is conditional - it only runs for Whale Town sessions
// (those starting with "wt-" or "hq-"). For non-GT sessions, a help message is shown.
// See: https://github.com/speaker20/whaletown/issues/13
func (t *Tmux) SetFeedBinding(session string) error {
	// C-b a → wt feed --window for GT sessions, help message otherwise
	_, err := t.run("bind-key", "-T", "prefix", "a",
		"if-shell", "echo '#{session_name}' | grep -Eq '^(gt|hq)-'",
		"run-shell 'wt feed --window'",
		"display-message 'C-b a is for Whale Town sessions only'")
	return err
}
...
// The agentID is used to identify the agent in crash logs (e.g., "whaletown/Toast").
func (t *Tmux) SetPaneDiedHook(session, agentID string) error {
	...
	// We run wt log crash which records to the town log
