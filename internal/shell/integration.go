// ABOUTME: Shell integration installation and removal for Whale Town.
...
const (
	markerStart = "# --- Whale Town Integration (managed by gt) ---"
	markerEnd   = "# --- End Whale Town ---"
)
...
func addToRCFile(path string) error {
	...
	if len(data) > 0 {
		backupPath := path + ".whaletown-backup"
...
func updateRCFile(path, content string) error {
	...
	endIdx := strings.Index(content[startIdx:], markerEnd)
	if endIdx == -1 {
		return fmt.Errorf("malformed Whale Town block in %s", path)
	}
	endIdx += startIdx + len(markerEnd)
...
# Whale Town Shell Integration
# Installed by: wt install --shell
# Location: ~/.config/whaletown/shell-hook.sh

_whaletown_enabled() {
    [[ -n "$GASTOWN_DISABLED" ]] && return 1
    [[ -n "$GASTOWN_ENABLED" ]] && return 0
    local state_file="$HOME/.local/state/whaletown/state.json"
...
_whaletown_ignored() {
    local dir="$PWD"
    while [[ "$dir" != "/" ]]; do
        [[ -f "$dir/.whaletown-ignore" ]] && return 0
        dir="$(dirname "$dir")"
    done
    return 1
}

_whaletown_already_asked() {
    local repo_root="$1"
    local asked_file="$HOME/.cache/whaletown/asked-repos"
    [[ -f "$asked_file" ]] && grep -qF "$repo_root" "$asked_file" 2>/dev/null
}

_whaletown_mark_asked() {
    local repo_root="$1"
    local asked_file="$HOME/.cache/whaletown/asked-repos"
...
_whaletown_offer_add() {
    local repo_root="$1"
    
    _whaletown_already_asked "$repo_root" && return 0
    ...
    echo ""
    echo -n "Add '$repo_name' to Whale Town? [y/N/never] "
    read -r response </dev/tty
    
    _whaletown_mark_asked "$repo_root"
    
    case "$response" in
        y|Y|yes)
            echo "Adding to Whale Town..."
            local output
            ...
            if [[ $exit_code -eq 0 ]]; then
                local crew_path
                crew_path=$(echo "$output" | grep "^WT_CREW_PATH=" | cut -d= -f2)
                if [[ -n "$crew_path" && -d "$crew_path" ]]; then
                    echo ""
                    echo "Switching to crew workspace..."
                    cd "$crew_path" || true
                    # Re-run hook to set WT_TOWN_ROOT and WT_RIG
                    _whaletown_hook
                fi
            fi
            ;;
        never)
            touch "$repo_root/.whaletown-ignore"
            echo "Created .whaletown-ignore - won't ask again for this repo."
            ;;
        *)
            echo "Skipped. Run 'wt rig quick-add' later to add manually."
            ;;
    esac
}
...
_whaletown_hook() {
    ...
    local cache_file="$HOME/.cache/whaletown/rigs.cache"
...
case "${SHELL##*/}" in
    zsh)
        ...
        add-zsh-hook chpwd _whaletown_chpwd_hook
        add-zsh-hook precmd _whaletown_hook
        ;;
    bash)
        if [[ ";${PROMPT_COMMAND[*]:-};" != *";_whaletown_hook;"* ]]; then
            PROMPT_COMMAND="_whaletown_chpwd_hook${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
        fi
        ;;
esac
