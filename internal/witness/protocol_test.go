...
Branch: feature-branch`
	...
	if payload.IssueID != "wt-abc123" {
		t.Errorf("IssueID = %q, want %q", payload.IssueID, "wt-abc123")
	}
	if payload.MRID != "wt-mr-xyz" {
		t.Errorf("MRID = %q, want %q", payload.MRID, "wt-mr-xyz")
	}
...
func TestParseHelp(t *testing.T) {
	...
	body := `Agent: whaletown/polecats/nux
Issue: gt-abc123
...
Tried: Increased timeout, checked for deadlocks`
	...
	if payload.Agent != "whaletown/polecats/nux" {
		t.Errorf("Agent = %q, want %q", payload.Agent, "whaletown/polecats/nux")
	}
	if payload.IssueID != "wt-abc123" {
		t.Errorf("IssueID = %q, want %q", payload.IssueID, "wt-abc123")
	}
