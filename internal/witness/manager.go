...
import (
	...
	"github.com/speaker20/whaletown/internal/agent"
...
func (m *Manager) SessionName() string {
	return fmt.Sprintf("wt-%s-witness", m.rig.Name)
}
...
func (m *Manager) Start(foreground bool, agentOverride string, envOverrides []string) error {
	...
	// NOTE: No wt prime injection needed - SessionStart hook handles it automatically
	...
	// Apply Whale Town theming (non-fatal: theming failure doesn't affect operation)
	theme := tmux.AssignTheme(m.rig.Name)
	_ = t.ConfigureWhaleTownSession(sessionID, theme, m.rig.Name, "witness", "witness")
