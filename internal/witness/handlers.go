...
func HandlePolecatDone(workDir, rigName string, msg *mail.Message) *HandlerResult {
	...
	// when the gate closes via wt gate wake.
...
Please fix the issue and resubmit with 'wt done'.`,
			payload.Branch,
			payload.IssueID,
			payload.FailureType,
			payload.Error,
...
func createCleanupWisp(workDir, polecatName, issueID, branch string) (string, error) {
	...
	for _, line := range strings.Split(output, "\n") {
		...
		// Look for bead ID pattern (e.g., "wt-abc123")
...
func NukePolecat(workDir, rigName, polecatName string) error {
	...
	// We do this explicitly here because wt polecat nuke may fail to kill the
	...
	// Now run wt polecat nuke to clean up worktree, branch, and beads
	address := fmt.Sprintf("%s/%s", rigName, polecatName)

	if err := util.ExecRun(workDir, "wt", "polecat", "nuke", address); err != nil {
...
// An orphan is likely from a crash before wt done completed.
...
// (e.g., "whaletown" for whaletown.git). This function checks ALL remotes to find
