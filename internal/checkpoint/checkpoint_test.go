...
func TestWithHookedBead(t *testing.T) {
	cp := &Checkpoint{}
	result := cp.WithHookedBead("wt-123")

	if result != cp {
		t.Error("WithHookedBead should return same checkpoint")
	}
	if cp.HookedBead != "wt-123" {
		t.Errorf("HookedBead = %q, want %q", cp.HookedBead, "wt-123")
	}
}
...
func TestSummary(t *testing.T) {
	tests := []struct {
		...
		{
			name: "hooked bead",
			cp:   &Checkpoint{HookedBead: "wt-abc"},
			want.*wt-abc",
		},
		{
			name: "modified files",
		...
		},
		{
			...
			want.*wt-abc, 1 modified files, branch: main",
		},
	}
