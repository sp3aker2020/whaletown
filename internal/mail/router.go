...
//   - gt-mayor → mayor/
//   - gt-deacon → deacon/
//   - gt-whaletown-witness → whaletown/witness
//   - gt-whaletown-crew-max → whaletown/max
//   - gt-whaletown-polecat-Toast → whaletown/Toast
func agentBeadToAddress(bead *agentBead) string {
	...
	case 1:
		...
		return parts[0] + "/"
	case 2:
		// Rig singleton: gt-whaletown-witness
		return parts[0] + "/" + parts[1]
	default:
		// Rig named agent: gt-whaletown-crew-max, gt-whaletown-polecat-Toast
		// Skip the role part (parts[1]) and use rig/name format
		if len(parts) >= 3 {
			// Rejoin if name has hyphens: gt-whaletown-polecat-my-agent
...
func (r *Router) notifyRecipient(msg *Message) error {
	...
	notification := fmt.Sprintf("📬 You have new mail from %s. Subject: %s. Run 'wt mail inbox' to read.", msg.From, msg.Subject)
...
func addressToSessionID(address string) string {
	...
	return fmt.Sprintf("wt-%s-%s", rig, target)
}
