...
func TestAddressToIdentity(t *testing.T) {
	tests := []struct {
		...
		{"whaletown/polecats/Toast", "whaletown/Toast"},
		{"whaletown/crew/max", "whaletown/max"},
		{"whaletown/Toast", "whaletown/Toast"},         // Already canonical
		{"whaletown/max", "whaletown/max"},             // Already canonical
		{"whaletown/refinery", "whaletown/refinery"},
		{"whaletown/witness", "whaletown/witness"},
...
func TestNewMessage(t *testing.T) {
	msg := NewMessage("mayor/", "whaletown/Toast", "Test Subject", "Test Body")

	if msg.From != "mayor/" {
		t.Errorf("From = %q, want 'mayor/'", msg.From)
	}
	if msg.To != "whaletown/Toast" {
		t.Errorf("To = %q, want 'whaletown/Toast'", msg.To)
	}
...
func TestNewReplyMessage(t *testing.T) {
	original := &Message{
		ID:       "orig-001",
		ThreadID: "thread-001",
		From:     "whaletown/Toast",
		To:       "mayor/",
		Subject:  "Original Subject",
	}

	reply := NewReplyMessage("mayor/", "whaletown/Toast", "Re: Original Subject", "Reply body", original)
	...
	if reply.To != "whaletown/Toast" {
		t.Errorf("To = %q, want 'whaletown/Toast'", reply.To)
	}
...
func TestMessageIsQueueMessage(t *testing.T) {
	directMsg := NewMessage("mayor/", "whaletown/Toast", "Test", "Body")
...
func TestMessageValidate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg:     NewMessage("mayor/", "whaletown/Toast", "Test", "Body"),
			wantErr: false,
		},
		{
...
func TestBeadsMessageParseQueueChannelLabels(t *testing.T) {
	...
	if msg.ClaimedBy != "whaletown/nux" {
		t.Errorf("ClaimedBy = %q, want 'whaletown/nux'", msg.ClaimedBy)
	}
...
func TestMessageIsClaimed(t *testing.T) {
	...
	claimed.ClaimedBy = "whaletown/nux"
	now := time.Now()
