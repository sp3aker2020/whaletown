...
func TestDetectTownRoot(t *testing.T) {
	...
	rigDir := filepath.Join(townRoot, "whaletown", "polecats", "test")
...
func TestIsTownLevelAddress(t *testing.T) {
	tests := []struct {
		...
		{"deacon", true},
		{"deacon/", true},
		{"whaletown/refinery", false},
		{"whaletown/polecats/Toast", false},
		{"whaletown/", false},
		{"", false},
	}
...
func TestAddressToSessionID(t *testing.T) {
	tests := []struct {
		...
		{"whaletown/Toast", "wt-whaletown-Toast"},
		{"beads/witness", "wt-beads-witness"},
		{"whaletown/", ""},   // Empty target
		{"whaletown", ""},    // No slash
		{"", ""},           // Empty address
	}
...
func TestIsSelfMail(t *testing.T) {
	tests := []struct {
		...
		{"mayor", "mayor/", true},
		{"mayor/", "mayor", true},
		{"whaletown/Toast", "whaletown/Toast", true},
		{"whaletown/Toast/", "whaletown/Toast", true},
		{"mayor/", "deacon/", false},
		{"whaletown/Toast", "whaletown/Nux", false},
		{"", "", true},
	}
...
func TestResolveBeadsDir(t *testing.T) {
	...
	got := r.resolveBeadsDir("whaletown/Toast")
...
func TestParseListName(t *testing.T) {
	tests := []struct {
		...
		{"list:oncall", "oncall"},
		{"list:cleanup/whaletown", "cleanup/whaletown"},
		{"list:", ""},
		{"list:alerts", "alerts"},
	}
...
func TestParseQueueName(t *testing.T) {
	tests := []struct {
		...
		{"queue:work", "work"},
		{"queue:whaletown/polecats", "whaletown/polecats"},
		{"queue:", ""},
...
func TestExpandList(t *testing.T) {
  ...
  "lists": {
    ...
    "cleanup/whaletown": ["whaletown/witness", "deacon/"]
  }
}`
	...
	tests := []struct {
	...
	}{
		{
			name:     "oncall list",
			listName: "oncall",
			want:     []string{"mayor/", "whaletown/witness"},
		},
		{
			name:     "cleanup/whaletown list",
			listName: "cleanup/whaletown",
			want:     []string{"whaletown/witness", "deacon/"},
		},
		{
			name:      "unknown list",
...
func TestExpandQueue(t *testing.T) {
  ...
  "version": 1,
  "queues": {
    "work/whaletown": {"workers": ["whaletown/polecats/*"], "max_claims": 3},
    "priority-high": {"workers": ["mayor/", "whaletown/witness"]}
  }
}`
	if err := os.WriteFile(filepath.Join(configDir, "messaging.json"), []byte(configContent), 0644); err != nil {
		t.Fatal(err)
	...
	tests := []struct {
		...
		{
			name:        "work/whaletown queue",
			queueName:   "work/whaletown",
			wantWorkers: []string{"whaletown/polecats/*"},
			wantMax:     3,
		},
		{
			name:        "priority-high queue",
			queueName:   "priority-high",
			wantWorkers: []string{"mayor/", "whaletown/witness"},
...
func TestIsAnnounceAddress(t *testing.T) {
	tests := []struct {
		...
		{"announce:whaletown/updates", true},
...
func TestParseAnnounceName(t *testing.T) {
	tests := []struct {
		...
		{"announce:whaletown/updates", "whaletown/updates"},
		{"announce:", ""},
...
func TestIsGroupAddress(t *testing.T) {
	tests := []struct {
		...
		{"@dogs", true},
		{"@overseer", true},
		{"@polecats/whaletown", true},
		{"mayor/", false},
		{"whaletown/Toast", false},
		{"", false},
		{"rig/whaletown", false}, // Missing @
	}
...
func TestParseGroupAddress(t *testing.T) {
	tests := []struct {
		...
		{"@rig/whaletown", GroupTypeRig, "", "whaletown", false},
		{"@rig/beads", GroupTypeRig, "", "beads", false},

		// Rig+role patterns
		{"@crew/whaletown", GroupTypeRigRole, "crew", "whaletown", false},
		{"@polecats/whaletown", GroupTypeRigRole, "polecat", "whaletown", false},
...
func TestAgentBeadToAddress(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			bead: &agentBead{ID: "wt-mayor"},
			want: "mayor/",
		},
		{
			name: "town-level deacon",
			bead: &agentBead{ID: "wt-deacon"},
			want: "deacon/",
		},
		{
			name: "rig singleton witness",
			bead: &agentBead{ID: "wt-whaletown-witness"},
			want: "whaletown/witness",
		},
		{
			name: "rig singleton refinery",
			bead: &agentBead{ID: "wt-whaletown-refinery"},
			want: "whaletown/refinery",
		},
		{
			name: "rig crew worker",
			bead: &agentBead{ID: "wt-whaletown-crew-max"},
			want: "whaletown/max",
		},
		{
			name: "rig polecat worker",
			bead: &agentBead{ID: "wt-whaletown-polecat-Toast"},
			want: "whaletown/Toast",
		},
		{
			name: "rig polecat with hyphenated name",
			bead: &agentBead{ID: "wt-whaletown-polecat-my-agent"},
			want: "whaletown/my-agent",
		},
		{
			name: "non-gt prefix (invalid)",
			bead: &agentBead{ID: "bd-whaletown-witness"},
			want: "",
		},
		{
			name: "empty ID",
...
func TestExpandAnnounce(t *testing.T) {
  ...
  "announces": {
    ...
    "status/whaletown": {"readers": ["whaletown/witness", "mayor/"], "retain_count": 5}
  }
}`
	if err := os.WriteFile(filepath.Join(configDir, "messaging.json"), []byte(configContent), 0644); err != nil {
		t.Fatal(err)
	...
	tests := []struct {
		...
		{
			name:         "status/whaletown announce",
			announceName: "status/whaletown",
			wantReaders:  []string{"whaletown/witness", "mayor/"},
			wantRetain:   5,
		},
		{
