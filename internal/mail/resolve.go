...
type Recipient struct {
	Address      string        // The resolved address (e.g., "whaletown/crew/max")
...
// Patterns like "*/witness" or "whaletown/*" are expanded.
...
// E.g., "wt-whaletown-crew-max" → "whaletown/crew/max"
func agentBeadIDToAddress(id string) string {
	...
	case 1:
		...
		return parts[0] + "/"
	case 2:
		// Rig singleton: gt-whaletown-witness → whaletown/witness
		return parts[0] + "/" + parts[1]
	default:
		// Rig named agent: gt-whaletown-crew-max → whaletown/crew/max
		if len(parts) >= 3 {
