...
// Run checks for linked panes across Whale Town tmux sessions.
func (c *LinkedPaneCheck) Run(ctx *CheckContext) *CheckResult {
	...
	for _, session := range sessions {
		if strings.HasPrefix(session, "wt-") {
	...
	if len(conflicts) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: fmt.Sprintf("All %d Whale Town sessions have independent panes", len(gtSessions)),
		}
	}
	...
	return &CheckResult{
		...
		Details: conflicts,
		FixHint: "Run 'wt doctor --fix' to kill linked sessions (daemon will recreate)",
	}
}
