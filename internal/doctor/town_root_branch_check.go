...
// The town root should always stay on main to avoid confusion and broken wt commands.
...
func (c *TownRootBranchCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if branch == "" {
		return &CheckResult{
			...
			Details: []string{
				...
				"Detached HEAD can cause wt commands to fail",
			},
			FixHint: "Run 'wt doctor --fix' or manually: cd ~/gt && git checkout main",
		}
	}
	...
	return &CheckResult{
		...
		Details: []string{
			...
			"This can cause wt commands to fail (missing rigs.json, etc.)",
