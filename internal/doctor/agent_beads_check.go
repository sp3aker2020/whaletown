...
//
// Agent beads are created by wt rig add (see gt-h3hak, gt-pinkq) and wt crew add.
// Each rig uses its configured prefix (e.g., "wt-" for whaletown, "bd-" for beads).
...
func (c *AgentBeadsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Routes have format: prefix "wt-" -> path "whaletown/mayor/rig" or "my-saas"
	...
	if len(prefixToRig) == 0 {
		...
		return &CheckResult{
			...
			Details: missing,
			FixHint: "Run 'wt doctor --fix' to create missing agent beads",
		}
	}
