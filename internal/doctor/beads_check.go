	...
	if dbErr == nil && dbInfo.Size() == 0 {
		if jsonlErr == nil && jsonlInfo.Size() > 0 {
			return &CheckResult{
				...
				FixHint: "Run 'wt doctor --fix' or delete issues.db and run 'bd sync --from-main'",
			}
		}
	}
			...
			if rigDBErr == nil && rigDBInfo.Size() == 0 {
				if rigJSONLErr == nil && rigJSONLInfo.Size() > 0 {
					return &CheckResult{
						...
						FixHint: "Run 'wt doctor --fix' or delete the rig's issues.db",
					}
				}
			}
		}
...
func (c *PrefixMismatchCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt doctor --fix' to update rigs.json with correct prefixes",
	}
}
...
func (c *RoleLabelCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		FixHint: "Run 'wt doctor --fix' to add missing labels",
	}
}
