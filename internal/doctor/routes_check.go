...
func (c *RoutesCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if _, err := os.Stat(routesPath); os.IsNotExist(err) {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to create routes.jsonl",
		}
	}
	...
	if err != nil {
		...
		if missingTownRoute || missingConvoyRoute {
			return &CheckResult{
				...
				Details: details,
				FixHint: "Run 'wt doctor --fix' to add missing routes",
			}
		}
...
func (c *RoutesCheck) Fix(ctx *CheckContext) error {
	...
	// This is normally created by wt install but may be missing if routes.jsonl was corrupted
	modified := false
	if !routeMap["hq-"] {
