...
func (c *PatrolMoleculesExistCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if len(details) > 0 {
		return &CheckResult{
			...
			Details: details,
			FixHint: "Run 'wt doctor --fix' to create missing patrol molecules",
		}
	}
...
func (c *PatrolHooksWiredCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if _, err := os.Stat(daemonConfigPath); os.IsNotExist(err) {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to create default config, or 'wt daemon start' to start the daemon",
		}
	}
	...
	return &CheckResult{
		Name:    c.Name(),
		Status:  StatusWarning,
		Message: fmt.Sprintf("Configure patrols in %s or run 'wt daemon start'", relPath),
		FixHint: "Run 'wt doctor --fix' to create default config",
	}
}
...
func (c *PatrolRolesHavePromptsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if len(missingPrompts) > 0 {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to copy embedded templates to rig repos",
		}
	}
