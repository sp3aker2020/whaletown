...
const preCheckoutHookMarker = "Whale Town pre-checkout hook"
...
func (c *PreCheckoutHookCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if os.IsNotExist(err) {
		...
		return &CheckResult{
			...
			Details: []string{
				...
				"This can break wt commands (missing rigs.json, wrong configs)",
			},
			FixHint: "Run 'wt doctor --fix' or 'wt git-init' to install the hook",
		}
	}
	...
	if !strings.Contains(string(content), preCheckoutHookMarker) {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusWarning,
			Message: "Pre-checkout hook exists but is not Whale Town's",
			Details: []string{
				"A pre-checkout hook exists but doesn't contain the Whale Town marker",
