...
// Package doctor provides integration tests for Whale Town doctor functionality.
...
func TestIntegrationOrphanSessionDetection(t *testing.T) {
	...
	tests := []struct {
		...
		// Valid Whale Town sessions should NOT be detected as orphans
		{"mayor_session", "hq-mayor", false},
		{"deacon_session", "hq-deacon", false},
		{"witness_session", "wt-whaletown-witness", false},
		{"refinery_session", "wt-whaletown-refinery", false},
		{"crew_session", "wt-whaletown-crew-max", false},
		{"polecat_session", "wt-whaletown-polecat-abc123", false},

		// Different rig names
		{"niflheim_witness", "wt-niflheim-witness", false},
		{"niflheim_crew", "wt-niflheim-crew-codex1", false},

		// Invalid sessions SHOULD be detected as orphans
		{"unknown_rig", "wt-unknownrig-witness", true},
		{"malformed", "wt-only-two", true}, // Only 2 parts after gt
		{"non_gt_prefix", "foo-whaletown-witness", false}, // Not a gt- session, should be ignored
	}

	townRoot := setupIntegrationTown(t)

	// Create test rigs
	createTestRig(t, townRoot, "whaletown")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.expectOrphan && !isValid && strings.HasPrefix(tt.sessionName, "wt-") {
...
func TestIntegrationCrewSessionProtection(t *testing.T) {
	tests := []struct {
		...
		{"simple_crew", "wt-whaletown-crew-max", true},
		{"crew_with_numbers", "wt-whaletown-crew-worker1", true},
		{"crew_different_rig", "wt-niflheim-crew-codex1", true},
		{"witness_not_crew", "wt-whaletown-witness", false},
		{"refinery_not_crew", "wt-whaletown-refinery", false},
		{"polecat_not_crew", "wt-whaletown-polecat-abc", false},
...
func TestIntegrationEnvVarsConsistency(t *testing.T) {
	...
	tests := []struct {
		...
		{"mayor", "", "mayor"},
		{"deacon", "", "deacon"},
		{"witness", "whaletown", "whaletown/witness"},
		{"refinery", "whaletown", "whaletown/refinery"},
		{"crew", "whaletown", "whaletown/crew/"},
	}
...
func TestIntegrationBeadsDirRigLevel(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			role:           "witness",
			rig:            "whaletown",
			wantBeadsSuffix: "/whaletown/.beads",
		},
		{
...
func TestIntegrationAgentBeadsExist(t *testing.T) {
	...
	setupMockBeads(t, townRoot, "whaletown")
...
func TestIntegrationFixMultipleIssues(t *testing.T) {
	...
	breakRuntimeGitignore(t, townRoot)
	breakCrewGitignore(t, townRoot, "whaletown", "worker1")
...
func TestIntegrationSessionNaming(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "witness",
			sessionName: "wt-whaletown-witness",
			wantRig:     "whaletown",
			wantRole:    "witness",
			wantName:    "",
		},
		{
			name:        "crew",
			sessionName: "wt-whaletown-crew-max",
			wantRig:     "whaletown",
			wantRole:    "crew",
			wantName:    "max",
		},
		{
			...
			sessionName: "wt-niflheim-crew-codex1",
			wantRig:     "niflheim",
			wantRole:    "crew",
			wantName:    "codex1",
		},
...
func breakRuntimeGitignore(t *testing.T, townRoot string) {
	...
	crewDir := filepath.Join(townRoot, "whaletown", "crew", "test-worker")
