// ABOUTME: Doctor check for Whale Town global state configuration.
...
func NewGlobalStateCheck() *GlobalStateCheck {
	return &GlobalStateCheck{
		BaseCheck: BaseCheck{
			...
			CheckDescription: "Validates Whale Town global state and shell integration",
...
func (c *GlobalStateCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if s.Enabled {
		details = append(details, "Whale Town: enabled")
	} else {
		details = append(details, "Whale Town: disabled")
		warnings = append(warnings, "Whale Town is disabled globally")
	}
	...
	if len(errors) > 0 {
		...
		result.FixHint = "Run: wt install --shell"
...
func hasShellIntegration(rcPath string) bool {
	...
	return strings.Contains(string(data), "Whale Town Integration")
}
