...
func TestPrefixMismatchCheck_Matching(t *testing.T) {
	...
	// Create rigs.json with matching wt prefix
	rigsPath := filepath.Join(mayorDir, "rigs.json")
	rigsContent := `{
		"version": 1,
		"rigs": {
			"whaletown": {
				"git_url": "https://github.com/example/whaletown",
				"beads": {
					"prefix": "wt"
				}
			}
...
func TestPrefixMismatchCheck_Fix(t *testing.T) {
	...
	if cfg.Rigs["whaletown"].BeadsConfig.Prefix != "wt" {
		t.Errorf("expected prefix 'gt' after fix, got %q", cfg.Rigs["whaletown"].BeadsConfig.Prefix)
	}
}
