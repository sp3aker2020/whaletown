...
func (c *ThemeCheck) Run(ctx *CheckContext) *CheckResult {
	...
	for _, session := range gtSessions {
		...
		// Old format had brackets like [Mayor] or [whaletown/crew]
	...
	if len(needsUpdate) > 0 {
		...
		return &CheckResult{
			...
			Details: details,
			FixHint: "Run 'wt theme apply --all' or 'wt doctor --fix'",
		}
	}
...
func (c *ThemeCheck) Fix(ctx *CheckContext) error {
	cmd := exec.Command("wt", "theme", "apply", "--all")
