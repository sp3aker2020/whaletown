...
func (c *TownConfigExistsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		return &CheckResult{
			...
			FixHint: "Run 'wt install' to initialize workspace",
		}
	}
...
func (c *RigsRegistryExistsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if _, err := os.Stat(rigsPath); os.IsNotExist(err) {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to create empty rigs.json",
		}
	}
...
func (c *RigsRegistryValidCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if len(missing) > 0 {
		...
		return &CheckResult{
			...
			Details: details,
			FixHint: "Run 'wt doctor --fix' to remove missing rigs from registry",
		}
	}
...
func (c *MayorExistsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if !info.IsDir() {
		return &CheckResult{
			...
			FixHint: "Remove mayor file and run 'wt install'",
		}
	}
