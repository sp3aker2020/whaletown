...
func TestOrphanedAttachmentsCheck_FormatOrphan(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: `hq-123: agent "whaletown/nux" no longer exists`,
		},
		{
			orph: orphanedHandoff{
				beadID: "wt-456",
				agent:  "whaletown/crew/joe",
			},
			expected: `gt-456: agent "whaletown/crew/joe" no longer exists`,
		},
	}
...
func TestOrphanedAttachmentsCheck_AgentExists(t *testing.T) {
	...
	polecatDir := filepath.Join(tmpDir, "whaletown", "polecats", "nux")
	if err := os.MkdirAll(polecatDir, 0755); err != nil {
		t.Fatal(err)
	}

	crewDir := filepath.Join(tmpDir, "whaletown", "crew", "joe")
	...
	witnessDir := filepath.Join(tmpDir, "whaletown", "witness")
	...
	tests := []struct {
		...
		// Existing agents
		{"whaletown/nux", true},
		{"whaletown/crew/joe", true},
		{"mayor", true},
		{"whaletown-witness", true},

		// Non-existent agents
		{"whaletown/deleted", false},
		{"whaletown/crew/gone", false},
