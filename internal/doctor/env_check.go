...
// Run checks environment variables for all Whale Town sessions.
func (c *EnvVarsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if err != nil {
		// No tmux server - treat as success (valid when Whale Town is down)
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
	...
	// Filter to Whale Town sessions only (gt-* and hq-*)
	...
	if len(gtSessions) == 0 {
		// No Whale Town sessions - treat as success (valid when Whale Town is down)
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: "No Whale Town sessions running",
		}
	}
	...
	if len(beadsDirWarnings) > 0 {
		...
		return &CheckResult{
			...
			Details: details,
			FixHint: "Remove BEADS_DIR from session environment: wt shutdown && wt up",
		}
	}
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt shutdown && wt up' to restart sessions with correct env vars",
	}
}
