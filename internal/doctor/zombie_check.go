...
// ZombieSessionCheck detects tmux sessions that are valid Whale Town sessions
...
// Run checks for zombie Whale Town sessions (tmux alive but Claude dead).
func (c *ZombieSessionCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Check each Whale Town session for zombie status
	var zombies []string
	...
	for _, sess := range sessions {
		...
		// Only check Whale Town sessions (gt-* and hq-*)
	...
	if len(zombies) == 0 {
		...
		if healthyCount > 0 {
			msg = fmt.Sprintf("All %d Whale Town sessions have running Claude processes", healthyCount)
		}
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt doctor --fix' to kill zombie sessions",
	}
}
...
func (c *ZombieSessionCheck) Fix(ctx *CheckContext) error {
	...
	for _, sess := range c.zombieSessions {
		...
		_ = events.LogFeed(events.TypeSessionDeath, sess,
			events.SessionDeathPayload(sess, "unknown", "zombie cleanup", "wt doctor"))
