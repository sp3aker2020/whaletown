...
func (c *SettingsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt doctor --fix' to create missing directories",
	}
}
...
// LegacyGastownCheck warns if old .whaletown/ directories still exist.
type LegacyGastownCheck struct {
	FixableCheck
	legacyDirs []string // Cached during Run for use in Fix
}

// NewLegacyGastownCheck creates a new legacy whaletown check.
func NewLegacyGastownCheck() *LegacyGastownCheck {
	return &LegacyGastownCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				CheckName:        "legacy-whaletown",
				CheckDescription: "Check for old .whaletown/ directories that should be migrated",
				CheckCategory:    CategoryConfig,
			},
		},
	}
...
// Run checks for legacy .whaletown/ directories.
func (c *LegacyGastownCheck) Run(ctx *CheckContext) *CheckResult {
	var found []string

	// Check town-level .whaletown/
	townGastown := filepath.Join(ctx.TownRoot, ".whaletown")
	if info, err := os.Stat(townGastown); err == nil && info.IsDir() {
		found = append(found, ".whaletown/ (town root)")
	}

	// Check each rig for .whaletown/
	rigs := c.findRigs(ctx.TownRoot)
	for _, rig := range rigs {
		rigGastown := filepath.Join(rig, ".whaletown")
		if info, err := os.Stat(rigGastown); err == nil && info.IsDir() {
			relPath, _ := filepath.Rel(ctx.TownRoot, rig)
			found = append(found, fmt.Sprintf("%s/.whaletown/", relPath))
		}
	}
	...
	if len(found) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: "No legacy .whaletown/ directories found",
		}
	}
	...
	return &CheckResult{
		Name:    c.Name(),
		Status:  StatusWarning,
		Message: fmt.Sprintf("%d legacy .whaletown/ directory(ies) found", len(found)),
		Details: found,
		FixHint: "Run 'wt doctor --fix' to remove after verifying migration is complete",
	}
}

// Fix removes legacy .whaletown/ directories.
...
// Valid options: session-start.sh wrapper OR 'wt prime --hook'.
// Without proper config, wt seance cannot discover sessions.
...
func (c *SessionHookCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: issues,
		FixHint: "Update hooks to use 'wt prime --hook' or 'bash ~/.claude/hooks/session-start.sh' for session_id passthrough",
	}
}
...
func (c *SessionHookCheck) checkSettingsFile(path string) []string {
	...
	if strings.Contains(content, "SessionStart") {
		if !c.usesSessionStartScript(content, "SessionStart") {
			problems = append(problems, "SessionStart uses bare 'wt prime' - add --hook flag or use session-start.sh")
		}
	}
	...
	if strings.Contains(content, "PreCompact") {
		if !c.usesSessionStartScript(content, "PreCompact") {
			problems = append(problems, "PreCompact uses bare 'wt prime' - add --hook flag or use session-start.sh")
		}
	}
...
// Valid: session-start.sh wrapper OR 'wt prime --hook'. Returns true if properly configured.
func (c *SessionHookCheck) usesSessionStartScript(content, hookType string) bool {
	...
	// Check if it uses 'wt prime --hook' which handles session_id via stdin
	if strings.Contains(section, "gt prime") {
		// wt prime --hook is valid - it reads session_id from stdin JSON
		// Must match --hook as complete flag, not substring (e.g., --hookup)
		if containsFlag(section, "--hook") {
			return true
		}
		// Bare 'wt prime' without --hook doesn't get session_id
		return false
	}

	// No wt prime or session-start.sh found - might be a different hook configuration
	return true
}
...
// CustomTypesCheck verifies Whale Town custom types are registered with beads.
	...
	return &CustomTypesCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				...
				CheckDescription: "Check that Whale Town custom types are registered with beads",
				CheckCategory:    CategoryConfig,
			},
		},
	}
...
func (c *CustomTypesCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if err != nil {
		...
		return &CheckResult{
			...
			Details: []string{
				"Whale Town custom types (agent, role, rig, convoy, slot) are not registered",
				"This may cause bead creation/validation errors",
			},
			FixHint: "Run 'wt doctor --fix' or 'bd config set types.custom \"" + constants.BeadsCustomTypes + "\"'",
		}
	}
	...
	return &CheckResult{
		...
		FixHint: "Run 'wt doctor --fix' to register missing types",
	}
}
