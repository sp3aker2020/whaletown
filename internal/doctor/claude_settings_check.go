...
func (c *ClaudeSettingsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	fixHint := "Run 'wt doctor --fix' to update settings and restart affected agents"
	if hasModifiedFiles {
		fixHint = "Run 'wt doctor --fix' to fix safe issues. Files with local modifications require manual review."
	}
			...
			for _, crewEntry := range crewEntries {
				...
				if fileExists(crewWrongSettings) {
					files = append(files, staleSettingsInfo{
						...
						agentType:     "crew",
						rigName:       rigName,
						sessionName:   fmt.Sprintf("wt-%s-crew-%s", rigName, crewEntry.Name()),
						wrongLocation: true,
					})
				}
			}
				...
				for _, pcWrongSettings := range wrongPaths {
					if fileExists(pcWrongSettings) {
						files = append(files, staleSettingsInfo{
							...
							agentType:     "polecat",
							rigName:       rigName,
							sessionName:   fmt.Sprintf("wt-%s-%s", rigName, pcEntry.Name()),
							wrongLocation: true,
						})
					}
				}
...
func (c *ClaudeSettingsCheck) checkSettings(path, _ string) []string {
	...
	// 3. Stop hook with wt costs record (for autonomous)
	// 4. wt nudge deacon session-started in SessionStart
	...
	// Check Stop hook exists with wt costs record (for all roles)
...
func (c *ClaudeSettingsCheck) Fix(ctx *CheckContext) error {
	...
	for _, sf := range c.staleSettings {
		...
		if sf.wrongLocation {
			...
			// especially since deacon runs wt doctor automatically which would create a loop.
		...
		if ctx.RestartSessions {
			if sf.agentType == "witness" || sf.agentType == "refinery" ||
				...
				if running {
					// Cycle the agent by killing and letting wt up restart it
