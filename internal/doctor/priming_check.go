...
// This ensures agents receive proper context on startup via the wt prime chain.
...
type primingIssue struct {
	location    string // e.g., "mayor", "whaletown/crew/max", "whaletown/witness"
...
func (c *PrimingCheck) Run(ctx *CheckContext) *CheckResult {
	...
	// Check 1: wt binary in PATH
	if err := exec.Command("which", "wt").Run(); err != nil {
		c.issues = append(c.issues, primingIssue{
			location:    "system",
			issueType:   "gt_not_in_path",
			description: "wt binary not found in PATH",
			fixable:     false,
		})
		details = append(details, "wt binary not found in PATH")
	}
	...
	fixHint := ""
	if fixableCount > 0 {
		fixHint = fmt.Sprintf("Run 'wt doctor --fix' to fix %d issue(s)", fixableCount)
	}
...
func (c *PrimingCheck) checkAgentPriming(townRoot, agentDir, _ string) []primingIssue {
	...
	// Check for SessionStart hook with wt prime
			...
			if err := json.Unmarshal(data, &settings); err == nil {
				if !c.hasGtPrimeHook(settings) {
					issues = append(issues, primingIssue{
						...
						description: "SessionStart hook missing 'wt prime'",
	...
	if fileExists(agentsMdPath) {
		...
		if lines > 20 {
			issues = append(issues, primingIssue{
				...
				fixable:     false, // Full context should come from wt prime templates
			})
		}
	}
	...
	for _, entry := range entries {
		...
		if !fileExists(primeMdPath) {
			issues = append(issues, primingIssue{
				...
				description: "Missing .beads/PRIME.md (Whale Town context fallback)",
				fixable:     true,
			})
		}
			...
			for _, crewEntry := range crewEntries {
				...
				if !fileExists(primeMdPath) {
					issues = append(issues, primingIssue{
						...
						issueType:   "missing_prime_md",
						description: "Missing PRIME.md (Whale Town context fallback)",
						fixable:     true,
					})
				}
			}
...
// hasGtPrimeHook checks if settings have a SessionStart hook that calls wt prime.
