...
func TestSessionHookCheck_UsesSessionStartScript(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "PreCompact bare wt prime fails",
		...
		{
			name:     "both session-start.sh and wt prime passes (session-start.sh wins)",
			content:  `{"hooks": {"SessionStart": [{"hooks": [{"type": "command", "command": "bash session-start.sh && wt prime"}]}]}}`,
			hookType: "SessionStart",
			want:     true,
		},
		{
...
func TestSessionHookCheck_Run(t *testing.T) {
	t.Run("bare wt prime warns", func(t *testing.T) {
		tmpDir := t.TempDir()
	...
	t.Run("rig-level bare wt prime warns", func(t *testing.T) {
		tmpDir := t.TempDir()
		...
		if result.Status != StatusWarning {
			t.Errorf("expected StatusWarning for rig-level bare wt prime, got %v", result.Status)
		}
	})
