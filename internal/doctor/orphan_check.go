...
// the expected Whale Town session naming patterns.
...
// Run checks for orphaned Whale Town tmux sessions.
func (c *OrphanSessionCheck) Run(ctx *CheckContext) *CheckResult {
	...
	for _, sess := range sessions {
		...
		// Only check gt-* sessions (Whale Town sessions)
		if !strings.HasPrefix(sess, "wt-") {
			continue
		}
	...
	if len(orphans) == 0 {
		return &CheckResult{
			Name:    c.Name(),
			Status:  StatusOK,
			Message: fmt.Sprintf("All %d Whale Town sessions are valid", validCount),
		}
	}
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt doctor --fix' to kill orphaned sessions",
	}
}
...
func (c *OrphanSessionCheck) Fix(ctx *CheckContext) error {
	...
	for _, sess := range c.orphanSessions {
		...
		_ = events.LogFeed(events.TypeSessionDeath, sess,
			events.SessionDeathPayload(sess, "unknown", "orphan cleanup", "wt doctor"))
...
func isCrewSession(session string) bool {
	...
	// Example: gt-whaletown-crew-joe
	parts := strings.Split(session, "-")
	if len(parts) >= 4 && parts[0] == "wt" && parts[2] == "crew" {
		return true
	}
	return false
}
...
// isValidSession checks if a session name matches expected Whale Town patterns.
// Valid patterns:
...
// or legitimately orphaned processes from crashed Whale Town sessions.
// This check is informational only - it does not auto-fix since we cannot
// distinguish user sessions from orphaned Whale Town processes.
...
func (c *OrphanProcessCheck) Run(ctx *CheckContext) *CheckResult {
	...
	details = append(details, "These may be your personal sessions or orphaned Whale Town processes.")
