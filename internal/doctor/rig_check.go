...
// GitExcludeConfiguredCheck verifies .git/info/exclude has Whale Town directories.
	...
	return &GitExcludeConfiguredCheck{
		FixableCheck: FixableCheck{
			BaseCheck: BaseCheck{
				...
				CheckDescription: "Check .git/info/exclude has Whale Town directories",
				CheckCategory:    CategoryRig,
			},
		},
	}
...
func (c *GitExcludeConfiguredCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		Name:    c.Name(),
		Status:  StatusWarning,
		Message: fmt.Sprintf("%d Whale Town directories not excluded", len(c.missingEntries)),
		Details: []string{fmt.Sprintf("Missing: %s", strings.Join(c.missingEntries, ", "))},
		FixHint: "Run 'wt doctor --fix' to add missing entries",
	}
}
...
func (c *GitExcludeConfiguredCheck) Fix(ctx *CheckContext) error {
	...
	info, _ := f.Stat()
	if info.Size() == 0 {
		if _, err := f.WriteString("# Whale Town directories\n"); err != nil {
			return err
		}
	} else {
		// Add newline before new entries
		if _, err := f.WriteString("\n# Whale Town directories\n"); err != nil {
			return err
		}
	}
...
func (c *HooksPathConfiguredCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: details,
		FixHint: "Run 'wt doctor --fix' to configure hooks",
	}
}
...
func (c *WitnessExistsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: issues,
		FixHint: "Run 'wt doctor --fix' to create missing structure",
	}
}
...
func (c *MayorCloneExistsCheck) Run(ctx *CheckContext) *CheckResult {
	...
	return &CheckResult{
		...
		Details: issues,
		FixHint: "Run 'wt doctor --fix' to create structure (clone requires repo URL)",
	}
}
	...
	if err != nil {
		...
		if strings.Contains(outputStr, "out of sync") || strings.Contains(outputStr, "behind") {
			...
			return &CheckResult{
				...
				FixHint: "Run 'wt doctor --fix' or 'bd sync' to synchronize",
			}
		}
	}
...
func (c *BeadsRedirectCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if _, err := os.Stat(mayorRigBeads); os.IsNotExist(err) {
		...
		if _, err := os.Stat(rigBeadsDir); os.IsNotExist(err) {
			return &CheckResult{
				...
				FixHint: "Run 'wt doctor --fix --rig " + ctx.RigName + "' to initialize beads",
			}
		}
		return &CheckResult{
			Name:    c.Name(),
	...
	if hasLocalData && !redirectExists {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix --rig " + ctx.RigName + "' to fix",
		}
	}
	...
	if !redirectExists {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to create the redirect",
		}
	}
	...
	if target != "mayor/rig/.beads" {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix --rig " + ctx.RigName + "' to correct the redirect",
		}
	}
...
func (c *BeadsRedirectCheck) Fix(ctx *CheckContext) error {
	...
	if !hasTrackedBeads && !hasLocalBeads {
		// Get the rig's beads prefix from rigs.json (falls back to "wt" if not found)
...
// See: https://github.com/anthropics/whaletown/issues/286
...
func (c *BareRepoRefspecCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if err != nil {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to configure the refspec",
		}
	}
	...
	if refspec != expectedRefspec {
		return &CheckResult{
			...
			FixHint: "Run 'wt doctor --fix' to update the refspec",
		}
	}
