...
func (m *Manager) Add(name string, createBranch bool) (*CrewWorker, error) {
	...
	// Provision PRIME.md with Whale Town context for this worker.
	// This is the fallback if SessionStart hook fails - ensures crew workers
	// always have GUPP and essential Whale Town context.
	...
	// NOTE: Slash commands (.claude/commands/) are provisioned at town level by wt install.
	// All agents inherit them via Claude's directory traversal - no per-workspace copies needed.

	// NOTE: We intentionally do NOT write to CLAUDE.md here.
	// Whale Town context is injected ephemerally via SessionStart hook (gt prime).
	// Writing to CLAUDE.md would overwrite project instructions and leak
	// Whale Town internals into the project repo when workers commit/push.
...
func (m *Manager) SessionName(name string) string {
	return fmt.Sprintf("wt-%s-crew-%s", m.rig.Name, name)
}
...
func (m *Manager) Start(name string, opts StartOptions) error {
	...
	_ = t.ConfigureWhaleTownSession(sessionID, theme, m.rig.Name, name, "crew")
