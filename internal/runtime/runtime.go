...
import (
	...
	"github.com/speaker20/whaletown/internal/opencode"
...
// It checks WT_SESSION_ID_ENV first, then falls back to CLAUDE_SESSION_ID.
func SessionIDFromEnv() string {
	if envName := os.Getenv("WT_SESSION_ID_ENV"); envName != "" {
...
func StartupFallbackCommands(role string, rc *config.RuntimeConfig) []string {
	...
	if isAutonomousRole(role) {
		command += " && wt mail check --inject"
	}
	command += " && wt nudge deacon session-started"
