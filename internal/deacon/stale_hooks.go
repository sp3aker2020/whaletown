...
// Supports formats like "whaletown/polecats/max", "whaletown/crew/joe", etc.
func assigneeToSessionName(assignee string) string {
	...
	case 3:
		// rig/type/name: "whaletown/polecats/max", "whaletown/crew/joe"
		rig, agentType, name := parts[0], parts[1], parts[2]
		switch agentType {
		case "polecats":
			return fmt.Sprintf("wt-%s-%s", rig, name)
		case "crew":
			return fmt.Sprintf("wt-%s-crew-%s", rig, name)
		default:
			return ""
		}
	default:
