...
func (m *Manager) SessionName() string {
	return fmt.Sprintf("wt-%s-refinery", m.rig.Name)
}
...
func (m *Manager) Start(foreground bool, agentOverride string) error {
	...
	if foreground {
		...
		// Use IsClaudeRunning for robust detection (see whaletown#566)
	...
	if running {
		...
		// and caused healthy sessions to be killed. See: whaletown#566
	...
	_ = t.ConfigureWhaleTownSession(sessionID, theme, m.rig.Name, "refinery", "refinery")
...
func (m *Manager) run(_ *Refinery) error { // ref unused: deprecated function
	...
	_, _ = fmt.Fprintln(m.output, "║  Use 'wt refinery start' to run in background mode.         ║")
	_, _ = fmt.Fprintln(m.output, "╚══════════════════════════════════════════════════════════════╝")
	_, _ = fmt.Fprintln(m.output, "")
	return nil
}
