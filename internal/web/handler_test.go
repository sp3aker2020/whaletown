...
func TestConvoyHandler_FullDashboard(t *testing.T) {
	...
	if !strings.Contains(body, "Whale Town Convoys") {
...
func TestE2E_Server_FullDashboard(t *testing.T) {
	...
	checks := []struct {
		...
		{"Convoy section header", "Whale Town Convoys"},
		{"Convoy ID", "hq-cv-e2e"},
				...
				Polecats: []PolecatRow{
					{
						Name:         "test-worker",
						Rig:          "test-rig",
						SessionID:    "wt-test-rig-test-worker",
						LastActivity: activity.Calculate(time.Now().Add(-tt.age)),
						StatusHint:   "Testing",
					},
				},
...
func TestE2E_Server_HTMLStructure(t *testing.T) {
	...
	elements := []string{
		...
		"<html",
		"<head>",
		"<title>Whale Town Dashboard</title>",
		"htmx.org",
		"<body>",
		"</body>",
		"</html>",
