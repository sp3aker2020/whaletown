...
// Assignees are in format "rigname/polecats/polecatname" which maps to tmux session "wt-rigname-polecatname".
...
// Assignee format: "rigname/polecats/polecatname" -> session "wt-rigname-polecatname"
func (f *LiveConvoyFetcher) getSessionActivityForAssignee(assignee string) *time.Time {
	...
	sessionName := fmt.Sprintf("wt-%s-%s", rig, polecat)
	...
	// Parse output: "wt-roxas-dag|1704312345"
...
func (f *LiveConvoyFetcher) getAllPolecatActivity() *time.Time {
	...
	for _, line := range strings.Split(stdout.String(), "\n") {
		...
		if len(nameParts) < 3 || nameParts[0] != "wt" {
			continue
		}
...
func (f *LiveConvoyFetcher) FetchMergeQueue() ([]MergeQueueRow, error) {
	...
	repos := []struct {
		...
		{"michaellady/whaletown", "whaletown"},
	}
