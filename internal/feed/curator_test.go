...
func TestCurator_FiltersByVisibility(t *testing.T) {
	...
	feedEvent := events.Event{
		...
		Actor:      "mayor",
		Payload:    map[string]interface{}{"bead": "wt-123", "target": "whaletown/slit"},
...
func TestCurator_GeneratesSummary(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: "mayor assigned gt-123 to whaletown/slit",
		},
		{
			event: &events.Event{
				Type:    events.TypeDone,
			...
			expected: "whaletown/slit completed work on slit-12345",
		},
		{
			event: &events.Event{
				Type:  events.TypeHandoff,
				Actor: "whaletown/witness",
			},
			expected: "whaletown/witness handed off to fresh session",
		},
	}
