...
type AttachmentFields struct {
	...
	AttachedArgs     string // Natural language args passed via wt sling --args (no-tmux mode)
...
type MRFields struct {
	...
	SourceIssue string // The work item being merged (e.g., "wt-xyz")
...
type RoleConfig struct {
	...
	// Examples: "hq-mayor", "hq-deacon", "wt-{rig}-{role}", "wt-{rig}-{name}"
