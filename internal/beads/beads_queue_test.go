...
func TestMatchClaimPattern(t *testing.T) {
	tests := []struct {
		...
		// Suffix wildcard
		{
			...
			identity: "whaletown/polecats/capable",
			want:     true,
		},
		{
			name:     "suffix wildcard matches different name",
			pattern:  "whaletown/polecats/*",
			identity: "whaletown/polecats/nux",
			want:     true,
		},
		{
			name:     "suffix wildcard doesn't match nested path",
			pattern:  "whaletown/polecats/*",
			identity: "whaletown/polecats/sub/capable",
			want:     false,
		},
		{
...
func TestFormatQueueDescription(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fields: &QueueFields{
				Name:         "work-requests",
				ClaimPattern: "whaletown/crew/*",
				Status:       QueueStatusActive,
			},
			want: []string{
				"Queue: work-requests",
				"name: work-requests",
				"claim_pattern: whaletown/crew/*",
				"status: active",
			},
		},
		{
