...
type Route struct {
	Prefix string `json:"prefix"` // Issue ID prefix (e.g., "wt-")
...
// The townRoot should be the Whale Town root directory (e.g., ~/gt).
...
// If the rig is not found in routes, returns "wt" as the default.
// The townRoot should be the Whale Town root directory (e.g., ~/gt).
func GetPrefixForRig(townRoot, rigName string) string {
	...
	// Routes paths are like "whaletown/mayor/rig" or "beads/mayor/rig"
	for _, r := range routes {
