...
type ListOptions struct {
	...
	Assignee   string // filter by assignee (e.g., "whaletown/Toast")
...
// BD_ACTOR, BEADS_DB, WT_ROOT, HOME etc. from routing commands to production databases.
...
// The assignee is typically in the format "rig/polecatName" (e.g., "whaletown/Toast").
...
// primeContent is the Whale Town PRIME.md content that provides essential context
// for crew workers. This is the fallback if the SessionStart hook fails.
const primeContent = `# Whale Town Worker Context
...
This is physics, not politeness. Whale Town is a steam engine - you are a piston.
...
// ProvisionPrimeMD writes the Whale Town PRIME.md file to the specified beads directory.
// This provides essential Whale Town context (GUPP, startup protocol) as a fallback
