...
func TestGetPrefixForRig(t *testing.T) {
	...
	routesContent := `{"prefix": "wt-", "path": "whaletown/mayor/rig"}
...
`
	...
	tests := []struct {
		...
		{"wt-", filepath.Join(tmpDir, "whaletown/mayor/rig")},
...
func TestAgentBeadIDsWithPrefix(t *testing.T) {
	tests := []struct {
		...
		{"PolecatBeadIDWithPrefix wt whaletown Toast",
			func() string { return PolecatBeadIDWithPrefix("wt", "whaletown", "Toast") },
			"wt-whaletown-polecat-Toast"},
