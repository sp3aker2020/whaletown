...
func TestCreateOptions(t *testing.T) {
	...
	if opts.Parent != "wt-abc" {
		t.Errorf("Parent = %q, want.*wt-abc", opts.Parent)
	}
}
...
func TestWrapError(t *testing.T) {
	...
	tests := []struct {
		...
		{"wt-xyz not found", ErrNotFound, false},
	}
...
func TestParseAgentBeadID(t *testing.T) {
	tests := []struct {
		...
		// Town-level agents
		{"wt-mayor", "", "mayor", "", true},
		{"wt-deacon", "", "deacon", "", true},
		// Rig-level singletons
		{"wt-whaletown-witness", "whaletown", "witness", "", true},
		{"wt-whaletown-refinery", "whaletown", "refinery", "", true},
		// Rig-level named agents
		{"wt-whaletown-crew-joe", "whaletown", "crew", "joe", true},
		{"wt-whaletown-crew-max", "whaletown", "crew", "max", true},
		{"wt-whaletown-polecat-capable", "whaletown", "polecat", "capable", true},
		// Names with hyphens
		{"wt-whaletown-polecat-my-agent", "whaletown", "polecat", "my-agent", true},
		// Parseable but not valid agent roles (IsAgentSessionBead will reject)
		{"wt-abc123", "", "abc123", "", true}, // Parses as town-level but not valid role
...
func TestIsAgentSessionBead(t *testing.T) {
	tests := []struct {
		...
		{"wt-whaletown-polecat-capable", true},
...
env_var: WT_ROLE=polecat
env_var: WT_RIG={rig}`,
			wantConfig: &RoleConfig{
				SessionPattern: "wt-{rig}-{name}",
				WorkDirPattern: "{town}/{rig}/polecats/{name}",
				NeedsPreSync:   true,
				StartCommand:   "exec claude --dangerously-skip-permissions",
				EnvVars:        map[string]string{"WT_ROLE": "polecat", "WT_RIG": "{rig}"},
			},
		},
		{
			name: "partial fields",
...
func TestExpandRolePattern(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rig:      "whaletown",
			name:     "toast",
			want:     "/Users/stevey/gt/whaletown/polecats/toast",
		},
		{
			pattern:  "{town}/{rig}/refinery/rig",
			townRoot: "/Users/stevey/gt",
			rig:      "whaletown",
			want:     "/Users/stevey/gt/whaletown/refinery/rig",
		},
		{
			pattern:  "export WT_ROLE={role} WT_RIG={rig} BD_ACTOR={rig}/polecats/{name}",
			townRoot: "/Users/stevey/gt",
			rig:      "whaletown",
			name:     "toast",
			role:     "polecat",
			want:     "export WT_ROLE=polecat WT_RIG=whaletown BD_ACTOR=whaletown/polecats/toast",
		},
	}
...
func TestRoleBeadID(t *testing.T) {
	tests := []struct {
		...
		{"mayor", "wt-mayor-role"},
		{"deacon", "wt-deacon-role"},
		{"witness", "wt-witness-role"},
		{"refinery", "wt-refinery-role"},
		{"crew", "wt-crew-role"},
		{"polecat", "wt-polecat-role"},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			delegation: Delegation{
				...
				Child:       "hop://alice@example.com/main-town/whaletown/gt-xyz",
			...
			wantJSON: `{"parent":"hop://accenture.com/eng/proj-123/task-a","child":"hop://alice@example.com/main-town/whaletown/gt-xyz","delegated_by":"hop://accenture.com","delegated_to":"hop://alice@example.com","terms":{"portion":"backend-api","deadline":"2025-06-01","credit_share":80},"created_at":"2025-01-15T10:00:00Z"}`,
		},
		{
			...
			wantJSON: `{"parent":"wt-abc","child":"wt-xyz","delegated_by":"steve","delegated_to":"alice"}`,
		},
	}
