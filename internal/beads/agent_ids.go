...
// This distinguishes them from rig-level beads (which use project prefixes like "wt-").
...
//   - gt-whaletown-witness (rig-level singleton)
//   - gt-whaletown-refinery (rig-level singleton)
//   - gt-whaletown-crew-max (rig-level named agent)
//   - gt-whaletown-polecat-Toast (rig-level named agent)

// AgentBeadIDWithPrefix generates an agent bead ID using the specified prefix.
// The prefix should NOT include the hyphen (e.g., "wt", "bd", not "wt-", "bd-").
...
// AgentBeadID generates the canonical agent bead ID using "wt" prefix.
// For non-whaletown rigs, use AgentBeadIDWithPrefix with the rig's configured prefix.
func AgentBeadID(rig, role, name string) string {
	return AgentBeadIDWithPrefix("wt", rig, role, name)
}

// MayorBeadID returns the Mayor agent bead ID.
//
// Deprecated: Use MayorBeadIDTown() for town-level beads (hq- prefix).
// This function returns "wt-mayor" which is for rig-level storage.
...
// This function returns "wt-deacon" which is for rig-level storage.
...
// WitnessBeadID returns the Witness agent bead ID for a rig using "wt" prefix.
func WitnessBeadID(rig string) string {
	return WitnessBeadIDWithPrefix("wt", rig)
}
...
// RefineryBeadID returns the Refinery agent bead ID for a rig using "wt" prefix.
func RefineryBeadID(rig string) string {
	return RefineryBeadIDWithPrefix("wt", rig)
}
...
// CrewBeadID returns a Crew worker agent bead ID using "wt" prefix.
func CrewBeadID(rig, name string) string {
	return CrewBeadIDWithPrefix("wt", rig, name)
}
...
// PolecatBeadID returns a Polecat agent bead ID using "wt" prefix.
func PolecatBeadID(rig, name string) string {
	return PolecatBeadIDWithPrefix("wt", rig, name)
}
...
func ParseAgentBeadID(id string) (rig, role, name string, ok bool) {
	...
	case 1:
		...
		return "", parts[0], "", true
	case 2:
		// Could be rig-level singleton (gt-whaletown-witness) or
		// town-level named (gt-dog-alpha for dogs)
		if parts[0] == "dog" {
			// Dogs are town-level named agents: gt-dog-<name>
			return "", "dog", parts[1], true
		}
		// Rig-level singleton: gt-whaletown-witness
		return parts[0], parts[1], "", true
	case 3:
		// Rig-level named: gt-whaletown-crew-max, bd-beads-polecat-pearl
		return parts[0], parts[1], parts[2], true
	default:
		// Handle names with hyphens: gt-whaletown-polecat-my-agent-name
...
// Agent session beads follow patterns like gt-mayor, bd-beads-witness, gt-whaletown-crew-joe.
// Supports any valid prefix (e.g., "wt-", "bd-"), not just "wt-".
