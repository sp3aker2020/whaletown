...
type RigFields struct {
	...
	Prefix string // Beads prefix for this rig (e.g., "wt", "bd")
...
// The ID format is: <prefix>-rig-<name> (e.g., gt-rig-whaletown)
...
// Format: <prefix>-rig-<name> (e.g., gt-rig-whaletown)
func RigBeadIDWithPrefix(prefix, name string) string {
	return fmt.Sprintf("%s-rig-%s", prefix, name)
}

// RigBeadID generates a rig identity bead ID using "wt" prefix.
// For non-whaletown rigs, use RigBeadIDWithPrefix with the rig's configured prefix.
func RigBeadID(name string) string {
	return RigBeadIDWithPrefix("wt", name)
}
