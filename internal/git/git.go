...
func (g *Git) Clone(url, dest string) error {
	...
	// Configure hooks path for Whale Town clones
...
// if it exists. This ensures Whale Town agents use the pre-push hook that blocks
...
// This ensures source repo settings don't override Whale Town agent settings.
...
func ConfigureSparseCheckout(repoPath string) error {
	...
	// from interfering with Whale Town agent context:
