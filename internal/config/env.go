...
type AgentEnvConfig struct {
	...
	// TownRoot is the root of the Whale Town workspace.
	// Sets WT_ROOT environment variable.
	TownRoot string
	...
	// Sets WT_SESSION_ID_ENV so the runtime knows where to find the session ID.
	SessionIDEnv string
...
func AgentEnv(cfg AgentEnvConfig) map[string]string {
	...
	if cfg.SessionIDEnv != "" {
		env["WT_SESSION_ID_ENV"] = cfg.SessionIDEnv
	}
...
// Returns a string like "export WT_ROLE=mayor BD_ACTOR=mayor && "
