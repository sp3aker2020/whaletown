...
func TestRigLevelCustomAgentIntegration(t *testing.T) {
	...
	t.Run("BuildPolecatStartupCommand uses custom agent", func(t *testing.T) {
		...
		if !strings.Contains(cmd, "WT_ROLE=polecat") {
			t.Errorf("Expected WT_ROLE=polecat in command, got: %s", cmd)
		}

		if !strings.Contains(cmd, "WT_POLECAT=test-polecat") {
			t.Errorf("Expected WT_POLECAT=test-polecat in command, got: %s", cmd)
		}
	})
...
echo "WT_ROLE: ${WT_ROLE:-not_set}"
echo "WT_POLECAT: ${WT_POLECAT:-not_set}"
echo "WT_RIG: ${WT_RIG:-not_set}"
echo "=========================================="
...
func testTmuxSessionWithStubAgent(t *testing.T, tmpDir, stubAgentPath, rigName string) {
	...
	sessionName := fmt.Sprintf("wt-test-pid%d-%d", os.Getpid(), time.Now().UnixNano())
	workDir := tmpDir
	...
	if !strings.Contains(output, "WT_ROLE: polecat") {
		t.Logf("Warning: WT_ROLE not visible in agent output (tmux env may not propagate to subshell)")
	}
