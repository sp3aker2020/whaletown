...
// Priority order:
//  1. WT_ACCOUNT environment variable
...
func ResolveAccountConfigDir(accountsPath, accountFlag string) (configDir, handle string, err error) {
	...
	// Priority 1: WT_ACCOUNT env var
	if envAccount := os.Getenv("WT_ACCOUNT"); envAccount != "" {
		acct := cfg.GetAccount(envAccount)
		if acct == nil {
			return "", "", fmt.Errorf("WT_ACCOUNT '%s' not found in accounts config", envAccount)
		}
...
// rigPath should be the path to the rig directory (e.g., ~/gt/whaletown).
//
...
// rigPath is the path to the rig directory (e.g., ~/gt/whaletown).
...
// townRoot is the path to the town directory (e.g., ~/gt).
// rigPath is the path to the rig directory (e.g., ~/gt/whaletown), or empty for town-level roles.
...
//
// If envVars contains WT_ROLE, the function uses role-based agent resolution
...
func BuildStartupCommand(envVars map[string]string, rigPath, prompt string) string {
	...
	// Add WT_ROOT so agents can find town-level resources (formulas, etc.)
	if townRoot != "" {
		resolvedEnv["WT_ROOT"] = townRoot
	}
	if rc.Session != nil && rc.Session.SessionIDEnv != "" {
		resolvedEnv["WT_SESSION_ID_ENV"] = rc.Session.SessionIDEnv
	}
...
//  2. role_agents[WT_ROLE] (if WT_ROLE is in envVars)
...
// Sets WT_ROLE, WT_RIG, WT_POLECAT, BD_ACTOR, GIT_AUTHOR_NAME, and WT_ROOT.
...
// Sets WT_ROLE, WT_RIG, WT_CREW, BD_ACTOR, GIT_AUTHOR_NAME, and WT_ROOT.
...
// Falls back to "wt" if the rig isn't found or has no prefix configured.
// townRoot is the path to the town directory (e.g., ~/gt).
func GetRigPrefix(townRoot, rigName string) string {
	...
	// Strip trailing hyphen if present (prefix stored as "wt-" but used as "wt")
