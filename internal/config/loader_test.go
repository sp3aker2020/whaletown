...
func TestRigsConfigRoundTrip(t *testing.T) {
	...
	original := &RigsConfig{
		...
		Rigs: map[string]RigEntry{
			"whaletown": {
				GitURL:    "git@github.com:steveyegge/whaletown.git",
	...
	rig, ok := loaded.Rigs["whaletown"]
	if !ok {
		t.Fatal("missing 'whaletown' rig")
	}
	if rig.BeadsConfig == nil || rig.BeadsConfig.Prefix != "wt-" {
...
func TestRigConfigRoundTrip(t *testing.T) {
	...
	original := NewRigConfig("whaletown", "git@github.com:test/whaletown.git")
	original.CreatedAt = time.Now().Truncate(time.Second)
	original.Beads = &BeadsConfig{Prefix: "wt-"}
	...
	if loaded.Name != "whaletown" {
		t.Errorf("Name = %q, want 'whaletown'", loaded.Name)
	}
	if loaded.GitURL != "git@github.com:test/whaletown.git" {
	...
	if loaded.Beads == nil || loaded.Beads.Prefix != "wt-" {
...
func TestMessagingConfigRoundTrip(t *testing.T) {
	...
	original.Lists["oncall"] = []string{"mayor/", "whaletown/witness"}
	original.Lists["cleanup"] = []string{"whaletown/witness", "deacon/"}
	original.Queues["work/whaletown"] = QueueConfig{
		Workers:   []string{"whaletown/polecats/*"},
		MaxClaims: 5,
	}
	...
	original.NudgeChannels["workers"] = []string{"whaletown/polecats/*", "whaletown/crew/*"}
	...
	if q, ok := loaded.Queues["work/whaletown"]; !ok || q.MaxClaims != 5 {
		...
		},
		{
			...
			config: &MessagingConfig{
				...
				Lists: map[string][]string{
					"oncall": {"mayor/", "whaletown/witness"},
				},
			},
			wantErr: false,
		},
		...
		},
		{
			...
			config: &MessagingConfig{
				...
				NudgeChannels: map[string][]string{
					"workers": {"whaletown/polecats/*", "whaletown/crew/*"},
				},
			},
			wantErr: false,
		},
...
func TestBuildAgentStartupCommand(t *testing.T) {
	...
	cmd := BuildAgentStartupCommand("witness", "whaletown", "", "", "")

	// Should contain environment exports and claude command
	if !strings.Contains(cmd, "export") {
		t.Error("expected export in command")
	}
	if !strings.Contains(cmd, "WT_ROLE=witness") {
		t.Error("expected WT_ROLE=witness in command")
	}
	if !strings.Contains(cmd, "BD_ACTOR=whaletown/witness") {
...
func TestBuildPolecatStartupCommand(t *testing.T) {
	t.Parallel()
	cmd := BuildPolecatStartupCommand("whaletown", "toast", "", "")

	if !strings.Contains(cmd, "WT_ROLE=polecat") {
		t.Error("expected WT_ROLE=polecat in command")
	}
	if !strings.Contains(cmd, "WT_RIG=whaletown") {
		t.Error("expected WT_RIG=whaletown in command")
	}
	if !strings.Contains(cmd, "WT_POLECAT=toast") {
		t.Error("expected WT_POLECAT=toast in command")
	}
	if !strings.Contains(cmd, "BD_ACTOR=whaletown/polecats/toast") {
		t.Error("expected BD_ACTOR in command")
	}
}
...
func TestBuildCrewStartupCommand(t *testing.T) {
	t.Parallel()
	cmd := BuildCrewStartupCommand("whaletown", "max", "", "")

	if !strings.Contains(cmd, "WT_ROLE=crew") {
		t.Error("expected WT_ROLE=crew in command")
	}
	if !strings.Contains(cmd, "WT_RIG=whaletown") {
		t.Error("expected WT_RIG=whaletown in command")
	}
	if !strings.Contains(cmd, "WT_CREW=max") {
		t.Error("expected WT_CREW=max in command")
	}
	if !strings.Contains(cmd, "BD_ACTOR=whaletown/crew/max") {
		t.Error("expected BD_ACTOR in command")
	}
}
...
func TestBuildPolecatStartupCommandWithAgentOverride(t *testing.T) {
	...
	if !strings.Contains(cmd, "WT_ROLE=polecat") {
		t.Fatalf("expected WT_ROLE export in command: %q", cmd)
	}
	if !strings.Contains(cmd, "WT_RIG=testrig") {
		t.Fatalf("expected WT_RIG export in command: %q", cmd)
	}
	if !strings.Contains(cmd, "WT_POLECAT=toast") {
		t.Fatalf("expected WT_POLECAT export in command: %q", cmd)
	}
...
func TestBuildAgentStartupCommandWithAgentOverride(t *testing.T) {
	...
	t.Run("empty override uses default agent", func(t *testing.T) {
		...
		if !strings.Contains(cmd, "WT_ROLE=mayor") {
			t.Fatalf("expected WT_ROLE export in command: %q", cmd)
		}
...
func TestBuildCrewStartupCommandWithAgentOverride(t *testing.T) {
	...
	if !strings.Contains(cmd, "WT_CREW=max") {
		t.Fatalf("expected WT_CREW export in command: %q", cmd)
	}
...
func TestBuildStartupCommand_UsesRigAgentWhenRigPathProvided(t *testing.T) {
	...
	cmd := BuildStartupCommand(map[string]string{"WT_ROLE": "witness"}, rigPath, "")
	if !strings.Contains(cmd, "codex") {
...
func TestBuildStartupCommand_UsesRoleAgentsFromTownSettings(t *testing.T) {
	...
	t.Run("refinery role gets gemini from role_agents", func(t *testing.T) {
		cmd := BuildStartupCommand(map[string]string{"WT_ROLE": constants.RoleRefinery}, rigPath, "")
	...
	t.Run("witness role gets codex from role_agents", func(t *testing.T) {
		cmd := BuildStartupCommand(map[string]string{"WT_ROLE": constants.RoleWitness}, rigPath, "")
		if !strings.Contains(cmd, "codex") {
	...
	t.Run("crew role falls back to default_agent (not in role_agents)", func(t *testing.T) {
		cmd := BuildStartupCommand(map[string]string{"WT_ROLE": constants.RoleCrew}, rigPath, "")
...
func TestBuildAgentStartupCommand_UsesRoleAgents(t *testing.T) {
	...
	// BuildAgentStartupCommand passes role via WT_ROLE env var
	...
	if !strings.Contains(cmd, "WT_ROLE="+constants.RoleRefinery) {
		t.Fatalf("expected WT_ROLE=%s in command: %q", constants.RoleRefinery, cmd)
	}
}
...
func TestDaemonPatrolConfigPath(t *testing.T) {
	...
	tests := []struct {
		...
		{"/var/lib/whaletown", "/var/lib/whaletown/mayor/daemon.json"},
...
func TestBuildStartupCommandWithAgentOverride_PriorityOverRoleAgents(t *testing.T) {
	...
	cmd, err := BuildStartupCommandWithAgentOverride(
		map[string]string{"WT_ROLE": constants.RoleRefinery},
		rigPath,
		"",
...
func TestBuildStartupCommandWithAgentOverride_IncludesGTRoot(t *testing.T) {
	...
	cmd, err := BuildStartupCommandWithAgentOverride(
		map[string]string{"WT_ROLE": constants.RoleWitness},
		rigPath,
		"",
		"gemini",
	)
	...
	// Should include WT_ROOT in export
	if !strings.Contains(cmd, "WT_ROOT="+townRoot) {
		t.Errorf("expected WT_ROOT=%s in command, got: %q", townRoot, cmd)
	}
}
