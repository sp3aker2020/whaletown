...
Examples:
  wt migrate-agents              # Dry-run: show what would be migrated
  wt migrate-agents --execute    # Apply the migration
  wt migrate-agents --force      # Re-migrate even if new beads exist`,
...
func runMigrateAgents(cmd *cobra.Command, args []string) error {
	...
	for _, r := range routes {
		if strings.TrimSuffix(r.Prefix, "-") == "wt" && r.Path != "." {
