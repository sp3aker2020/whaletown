...
import (
	...
	"github.com/speaker20/whaletown/internal/townlog"
...
Examples:
  wt done                              # Submit branch, notify COMPLETED, exit session
  wt done --issue gt-abc               # Explicit issue ID
  wt done --status ESCALATED           # Signal blocker, skip MR
  wt done --status DEFERRED            # Pause work, skip MR
  wt done --phase-complete --gate g-x  # Phase done, waiting on gate g-x`,
	RunE: runDone,
}
...
func runDone(cmd *cobra.Command, args []string) error {
	...
	// If the polecat's worktree was deleted by Witness before wt done finishes,
	// getcwd will fail. We fall back to WT_TOWN_ROOT env var in that case.
	...
	if !cwdAvailable {
		...
		// Try to get cwd from WT_POLECAT_PATH env var (set by session manager)
		if polecatPath := os.Getenv("WT_POLECAT_PATH"); polecatPath != "" {
	...
	var branch string
	if !cwdAvailable {
		// Try to get branch from WT_BRANCH env var (set by session manager)
		branch = os.Getenv("WT_BRANCH")
	}
	if branch == "" {
		var err error
	...
	if exitType == ExitCompleted {
		...
		// Polecats calling wt done without commits results in lost work.
		// We MUST check for:
		...
		// nuked at the end of wt done, so the commits are lost forever.
...
// If the polecat's worktree is deleted before wt done finishes, we use env vars as fallback.
// All errors are warnings, not failures - wt done must complete even if bead ops fail.
func updateAgentStateOnDone(cwd, townRoot, exitType, _ string) { // issueID unused but kept for future audit logging
	...
	if err != nil {
		...
		envRole := os.Getenv("WT_ROLE")
		envRig := os.Getenv("WT_RIG")
		envPolecat := os.Getenv("WT_POLECAT")
	...
	// wt done must be resilient - missing agent bead is not an error.
	//
	...
	if err := bd.ClearHookBead(agentBeadID); err != nil {
		// Non-fatal: warn but don't fail wt done
...
func selfKillSession(townRoot string, roleInfo RoleInfo) error {
	...
	rigName := os.Getenv("WT_RIG")
	polecatName := os.Getenv("WT_POLECAT")
	...
	sessionName := fmt.Sprintf("wt-%s-%s", rigName, polecatName)
