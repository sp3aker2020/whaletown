...
Examples:
  wt plugin list                    # List all discovered plugins
  wt plugin show <name>             # Show plugin details
  wt plugin list --json             # JSON output`,
...
Examples:
  wt plugin list              # Human-readable output
  wt plugin list --json       # JSON output for scripting`,
	RunE: runPluginList,
}
...
Examples:
  wt plugin show rebuild-gt
  wt plugin show rebuild-gt --json`,
...
Examples:
  wt plugin run rebuild-gt              # Run if gate allows
  wt plugin run rebuild-gt --force      # Bypass gate check
  wt plugin run rebuild-gt --dry-run    # Show what would happen`,
	Args: cobra.ExactArgs(1),
	RunE: runPluginRun,
}
...
Examples:
  wt plugin history rebuild-gt
  wt plugin history rebuild-gt --json
  wt plugin history rebuild-gt --limit 20`,
	Args: cobra.ExactArgs(1),
...
func runPluginRun(cmd *cobra.Command, args []string) error {
	...
	beadID, err := recorder.RecordRun(plugin.PluginRunRecord{
		...
		Body:       "Manual run via wt plugin run",
	})
	if err != nil {
