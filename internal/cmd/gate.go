...
// Gate command provides wt wrappers for gate operations.
// Most gate commands are in beads (bd gate ...), but wt provides
// integration with the Whale Town mail system for wake notifications.
...
The wt gate command provides Whale Town integration:
  wt gate wake     - Send wake mail to gate waiters after close`,
}
...
func runGateWake(cmd *cobra.Command, args []string) error {
	...
	body := fmt.Sprintf("Gate %s has closed.\n\nReason: %s\n\nRun 'wt resume' to continue your parked work.",
