...
var staleCmd = &cobra.Command{
	Use:     "stale",
	GroupID: GroupDiag,
	Short:   "Check if the wt binary is stale",
	Long: `Check if the wt binary was built from an older commit than the current repo HEAD.

This command compares the commit hash embedded in the binary at build time
with the current HEAD of the whaletown repository.

Examples:
  wt stale              # Human-readable output
  wt stale --json       # Machine-readable JSON output
  wt stale --quiet      # Exit code only (0=stale, 1=fresh)
...
func runStale(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("cannot find whaletown repo: %w", err)
	}
