...
// This enables no-tmux mode where agents discover args via wt prime / bd show.
...
// This is required for wt hook to recognize that a molecule is attached to the bead.
...
func injectStartPrompt(pane, beadID, subject, args string) error {
	...
	// Use the reliable nudge pattern (same as wt nudge / tmux.NudgeSession)
	t := tmux.NewTmux()
...
// - "wt-rig-name:0.0" (session:window.pane) - extract session name
...
// Rig-level agents use the rig's configured prefix (default "wt-").
...
func updateAgentHookBead(agentID, beadID, workDir, townBeadsDir string) {
	...
	if err != nil {
		// Not in a Whale Town workspace - can't update agent bead
...
func wakeRigAgents(rigName string) {
	...
	bootCmd := exec.Command("wt", "rig", "boot", rigName)
...
//
// Per issue #288: wt sling should auto-attach mol-polecat-work when slinging to polecats.
func attachPolecatWorkMolecule(targetAgent, hookWorkDir, townRoot string) error {
	...
	// Format: "<prefix>-<rig>-polecat-<name>" (e.g., "wt-whaletown-polecat-Toast")
