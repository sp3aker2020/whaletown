...
func formatReescalationMailBody(result *beads.ReescalationResult, reescalatedBy string) string {
	...
	lines = append(lines, "")
	lines = append(lines, "---")
	lines = append(lines, "To acknowledge: wt escalate ack "+result.ID)
	lines = append(lines, "To close: wt escalate close "+result.ID+" --reason \"resolution\"")
...
func formatEscalationMailBody(beadID, severity, reason, from, related string) string {
	...
	lines = append(lines, "")
	lines = append(lines, "---")
	lines = append(lines, "To acknowledge: wt escalate ack "+beadID)
	lines = append(lines, "To close: wt escalate close "+beadID+" --reason \"resolution\"")
	return strings.Join(lines, "\n")
}
...
func detectSenderFallback() string {
	...
	// Try WT_ROLE
	if role := os.Getenv("WT_ROLE"); role != "" {
		return role
	}
	return ""
}
