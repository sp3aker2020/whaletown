...
Alias: 'wt mr' is equivalent to 'wt mq' (merge request vs merge queue).
...
Examples:
  wt mq submit                           # Auto-detect everything + auto-cleanup
  wt mq submit --issue gp-abc            # Explicit issue
  wt mq submit --epic gt-xyz             # Target integration branch explicitly
  wt mq submit --priority 0              # Override priority (P0)
  wt mq submit --no-cleanup              # Submit without auto-cleanup`,
	RunE: runMqSubmit,
}
...
Examples:
  wt mq retry greenplace gp-mr-abc123
  wt mq retry greenplace gp-mr-abc123 --now`,
...
Examples:
  wt mq list greenplace
  wt mq list greenplace --ready
  wt mq list greenplace --status=open
  wt mq list greenplace --worker=Nux`,
...
Examples:
  wt mq reject greenplace polecat/Nux/gp-xyz --reason "Does not meet requirements"
  wt mq reject greenplace mr-Nux-12345 --reason "Superseded by other work" --notify`,
	Args: cobra.ExactArgs(2),
	RunE: runMQReject,
}
...
Examples:
  wt mq integration create gt-auth-epic
  # Creates integration/gt-auth-epic (default)

  wt mq integration create RA-123 --branch "klauern/PROJ-1234/{epic}"
...
Examples:
  wt mq integration land gt-auth-epic
  wt mq integration land gt-auth-epic --dry-run
  wt mq integration land gt-auth-epic --force --skip-tests`,
	Args: cobra.ExactArgs(1),
...
Example:
  wt mq integration status gt-auth-epic`,
	Args: cobra.ExactArgs(1),
