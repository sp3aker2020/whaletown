...
var nudgeCmd = &cobra.Command{
	...
	GroupID: GroupComm,
	Short:   "Send a synchronous message to any Whale Town worker",
	Long: `Universal synchronous messaging API for Whale Town worker-to-worker communication.
...
Channel syntax:
  channel:<name>  Nudges all members of a named channel defined in
                  ...
                  Patterns like "whaletown/polecats/*" are expanded.
...
Examples:
  wt nudge greenplace/furiosa "Check your mail and start working"
  wt nudge greenplace/alpha -m "What's your status?"
  wt nudge mayor "Status update requested"
  wt nudge witness "Check polecat health"
  wt nudge deacon session-started
  wt nudge channel:workers "New priority work available"`,
...
func runNudge(cmd *cobra.Command, args []string) error {
	...
	// These shortcuts let users type "mayor" instead of "wt-mayor"
	switch target {
	case "mayor":
...
// Patterns can be:
//   - Literal: "whaletown/witness" → gt-whaletown-witness
//   - Wildcard: "whaletown/polecats/*" → all polecat sessions in whaletown
...
func addressToAgentBeadID(address string) string {
	...
	switch role {
	case "witness":
		return fmt.Sprintf("wt-%s-witness", rig)
	case "refinery":
		return fmt.Sprintf("wt-%s-refinery", rig)
	default:
		// Assume polecat
		if strings.HasPrefix(role, "crew/") {
			crewName := strings.TrimPrefix(role, "crew/")
			return fmt.Sprintf("wt-%s-crew-%s", rig, crewName)
		}
		return fmt.Sprintf("wt-%s-polecat-%s", rig, role)
	}
}
