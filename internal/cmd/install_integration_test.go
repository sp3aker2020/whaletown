...
// TestInstallCreatesCorrectStructure validates that a fresh wt install
...
func TestInstallBeadsHasCorrectPrefix(t *testing.T) {
	...
	// Run wt install (includes beads init by default)
...
// TestInstallIdempotent validates that running wt install twice
...
func TestInstallIdempotent(t *testing.T) {
	...
	if !strings.Contains(string(output), "already a Whale Town HQ") {
		t.Errorf("expected 'already a Whale Town HQ' error, got: %s", output)
	}
...
func TestInstallFormulasProvisioned(t *testing.T) {
	...
	// Run wt install (includes beads and formula provisioning)
	cmd := exec.Command(gtBinary, "install", hqPath)
	cmd.Env = append(os.Environ(), "HOME="+tmpDir)
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestInstallWrappersInExistingTown(t *testing.T) {
	...
	if !strings.Contains(string(output), "wt-codex") && !strings.Contains(string(output), "wt-opencode") {
...
func TestInstallNoBeadsFlag(t *testing.T) {
	...
	// Run wt install with --no-beads
	cmd := exec.Command(gtBinary, "install", hqPath, "--no-beads")
	cmd.Env = append(os.Environ(), "HOME="+tmpDir)
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("wt install --no-beads failed: %v\nOutput: %s", err, output)
	}
...
// buildGT builds the wt binary and returns its path.
...
func buildGT(t *testing.T) string {
	...
	// Build wt binary to a persistent temp location (not per-test)
	tmpDir := os.TempDir()
	tmpBinary := filepath.Join(tmpDir, "wt-integration-test")
