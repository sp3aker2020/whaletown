...
import (
	...
	"github.com/speaker20/whaletown/internal/daemon"
	"github.com/speaker20/whaletown/internal/deacon"
	"github.com/speaker20/whaletown/internal/git"
	"github.com/speaker20/whaletown/internal/mayor"
...
var startCmd = &cobra.Command{
	...
	Short:   "Start Whale Town or a crew workspace",
	Long: `Start Whale Town by launching the Deacon and Mayor.
...
Crew shortcut:
  ...
  This is equivalent to 'wt start crew rig/name'.

To stop Whale Town, use 'wt shutdown'.`,
...
var shutdownCmd = &cobra.Command{
	Use:     "shutdown",
	GroupID: GroupServices,
	Short:   "Shutdown Whale Town with cleanup",
	Long: `Shutdown Whale Town by stopping agents and cleaning up polecats.

This is the "done for the day" command - it stops everything AND removes
polecat worktrees/branches. For a reversible pause, use 'wt down' instead.

Comparison:
  wt down      - Pause (stop processes, keep worktrees) - reversible
  wt shutdown  - Done (stop + cleanup worktrees) - permanent cleanup
...
This is a convenience command that combines 'wt crew add' and 'wt crew at --detached'.
...
Examples:
  wt start crew joe                    # Start joe in current rig
  wt start crew greenplace/joe            # Start joe in whaletown rig
  wt start crew joe --rig beads        # Start joe in beads rig`,
...
func runStart(cmd *cobra.Command, args []string) error {
	...
	// Verify we're in a Whale Town workspace
	...
	fmt.Printf("Starting Whale Town from %s\n\n", style.Dim.Render(townRoot))
	...
	fmt.Println()
	fmt.Printf("%s Whale Town is running\n", style.Bold.Render("✓"))
	fmt.Println()
...
// Called when --all flag is passed to wt start.
...
func runShutdown(cmd *cobra.Command, args []string) error {
	...
	if len(toStop) == 0 {
		fmt.Printf("%s Whale Town was not running\n", style.Dim.Render("○"))
		return nil
	}
...
func categorizeSessions(sessions []string, mayorSession, deaconSession string) (toStop, preserved []string) {
	for _, sess := range sessions {
		// Whale Town sessions use gt- (rig-level) or hq- (town-level) prefix
		if !strings.HasPrefix(sess, "wt-") && !strings.HasPrefix(sess, "hq-") {
			continue // Not a Whale Town session
		}
...
func runGracefulShutdown(t *tmux.Tmux, gtSessions []string, townRoot string) error {
	fmt.Printf("Graceful shutdown of Whale Town (waiting up to %ds)...\n\n", shutdownWait)
	...
	shutdownMsg := "[SHUTDOWN] Whale Town is shutting down. Please save your state and update your handoff bead, then type /exit or wait to be terminated."
	for _, sess := range gtSessions {
...
func runImmediateShutdown(t *tmux.Tmux, gtSessions []string, townRoot string) error {
	fmt.Println("Shutting down Whale Town...")
	...
	fmt.Println()
	fmt.Printf("%s Whale Town shutdown complete (%d sessions stopped)\n", style.Bold.Render("✓"), stopped)
...
// This combines the functionality of 'wt crew add' and 'wt crew at --detached'.
func runStartCrew(cmd *cobra.Command, args []string) error {
	name := args[0]

	// Parse rig/name format (e.g., "greenplace/joe" -> rig=whaletown, name=joe)
