...
The Refinery serializes all merges to main for a rig:
  - Receives MRs submitted by polecats (via wt done)
...
Work flows: Polecat completes → wt done → MR in queue → Refinery merges.
...
Examples:
  wt refinery start greenplace
  wt refinery start greenplace --foreground
  wt refinery start              # infer rig from cwd`,
...
Examples:
  wt refinery attach greenplace
  wt refinery attach          # infer rig from cwd`,
	Args: cobra.MaximumNArgs(1),
...
Examples:
  wt refinery restart greenplace
  wt refinery restart          # infer rig from cwd`,
	Args: cobra.MaximumNArgs(1),
...
The worker ID is automatically determined from the WT_REFINERY_WORKER
environment variable, or defaults to "refinery-1".

Examples:
  wt refinery claim gt-abc123
  WT_REFINERY_WORKER=refinery-2 wt refinery claim gt-abc123`,
...
Examples:
  wt refinery unclaimed
  wt refinery unclaimed --json`,
	Args: cobra.MaximumNArgs(1),
...
func getRefineryManager(rigName string) (*refinery.Manager, *rig.Rig, string, error) {
	...
	if rigName == "" {
		...
		if err != nil {
			return nil, nil, "", fmt.Errorf("not in a Whale Town workspace: %w", err)
		}
		rigName, err = inferRigFromCwd(townRoot)
		if err != nil {
			return nil, nil, "", fmt.Errorf("could not determine rig: %w\nUsage: wt refinery <command> <rig>", err)
		}
	}
...
func runRefineryStart(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  %s\n", style.Dim.Render("Use 'wt refinery status' to check progress"))
	return nil
}
...
func runRefineryAttach(cmd *cobra.Command, args []string) error {
	...
	sessionID := fmt.Sprintf("wt-%s-refinery", rigName)
...
func runRefineryRestart(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  %s\n", style.Dim.Render("Use 'wt refinery attach' to connect"))
	return nil
}

// getWorkerID returns the refinery worker ID from environment or default.
func getWorkerID() string {
	if id := os.Getenv("WT_REFINERY_WORKER"); id != "" {
		return id
	}
	return "refinery-1"
}
