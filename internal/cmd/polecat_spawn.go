// Package cmd provides polecat spawning utilities for wt sling.
package cmd
...
type SpawnedPolecatInfo struct {
	RigName     string // Rig name (e.g., "whaletown")
	PolecatName string // Polecat name (e.g., "Toast")
	ClonePath   string // Path to polecat's git worktree
	SessionName string // Tmux session name (e.g., "wt-whaletown-p-Toast")
	Pane        string // Tmux pane ID
}

// AgentID returns the agent identifier (e.g., "whaletown/polecats/Toast")
...
// This is used by wt sling when the target is a rig name.
...
func SpawnPolecatForSling(rigName string, opts SlingSpawnOptions) (*SpawnedPolecatInfo, error) {
	...
	_ = events.LogFeed(events.TypeSpawn, "wt", events.SpawnPayload(rigName, polecatName))
