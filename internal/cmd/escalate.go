...
WORKFLOW:
  ...
  2. Runs: wt escalate "Description" --severity high --reason "details"
  3. Escalation is routed based on settings/escalation.json
  4. Recipient acknowledges with: wt escalate ack <id>
  5. After resolution: wt escalate close <id> --reason "fixed"
...
Examples:
  wt escalate "Build failing" --severity critical --reason "CI blocked"
  wt escalate "Need API credentials" --severity high --source "plugin:rebuild-gt"
  wt escalate "Code review requested" --reason "PR #123 ready"
  wt escalate list                          # Show open escalations
  wt escalate ack hq-abc123                 # Acknowledge
  wt escalate close hq-abc123 --reason "Fixed in commit abc"
  wt escalate stale                         # Re-escalate stale escalations`,
}
...
Examples:
  wt escalate list              # Open escalations only
  wt escalate list --all        # Include closed
  wt escalate list --json       # JSON output`,
...
Examples:
  wt escalate close hq-abc123 --reason "Fixed in commit abc"
  wt escalate close hq-abc123 --reason "Not reproducible"`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt escalate stale              # Re-escalate stale escalations
  wt escalate stale --dry-run    # Show what would be done
  wt escalate stale --json       # JSON output of results`,
...
Examples:
  wt escalate show hq-abc123
  wt escalate show hq-abc123 --json`,
	Args: cobra.ExactArgs(1),
