...
With no arguments, shows your current hook status (alias for 'wt mol status').
With a bead ID, attaches that work to your hook.

The hook is the "durability primitive" - work on your hook survives session
restarts, context compaction, and handoffs. When you restart (via wt handoff),
your SessionStart hook finds the attached work and you continue from where
you left off.

Examples:
  wt hook                           # Show what's on my hook
  wt hook status                    # Same as above
  wt hook gt-abc                    # Attach issue gt-abc to your hook
  wt hook gt-abc -s "Fix the bug"   # With subject for handoff mail

Related commands:
  wt sling <bead>    # Hook + start now (keep context)
  wt handoff <bead>  # Hook + restart (fresh context)
  wt unsling         # Remove work from hook`,
...
This is an alias for 'wt mol status'. Shows what work is currently
attached to your hook, along with progress information.

Examples:
  wt hook status                    # Show my hook
  wt hook status greenplace/nux     # Show nux's hook`,
	Args: cobra.MaximumNArgs(1),
...
Examples:
  wt hook show                         # What's on MY hook? (auto-detect)
  wt hook show whaletown/polecats/nux    # What's nux working on?
  wt hook show whaletown/witness         # What's the witness hooked to?
  wt hook show mayor                   # What's the mayor working on?

Output format (one line):
  whaletown/polecats/nux: gt-abc123 'Fix the widget bug' [in_progress]`,
	Args: cobra.MaximumNArgs(1),
	RunE: runHookShow,
}
...
func init() {
	...
	hookCmd.Flags().BoolVar(&hookClear, "clear", false, "Clear your hook (alias for 'wt unhook')")

	// --json flag for status output (used when no args, i.e., wt hook --json)
...
func runHookOrStatus(cmd *cobra.Command, args []string) error {
	// --clear flag is alias for 'wt unhook'
	if hookClear {
...
func runHook(_ *cobra.Command, args []string) error {
	...
	// Polecats cannot hook - they use wt done for lifecycle
	if polecatName := os.Getenv("WT_POLECAT"); polecatName != "" {
		return fmt.Errorf("polecats cannot hook work (use wt done for handoff)")
	}
			...
			if !hookDryRun {
				if hasAttachment {
					...
					closeArgs := []string{"close", existing.ID, "--force",
						"--reason=Auto-replaced by wt hook (molecule complete)"}
	...
	fmt.Printf("  Use 'wt handoff' to restart with this work\n")
	fmt.Printf("  Use 'wt hook' to see hook status\n")
...
// findTownRoot finds the Whale Town root directory.
func findTownRoot() (string, error) {
	cmd := exec.Command("wt", "root")
	out, err := cmd.Output()
	if err != nil {
		return "", err
	}
