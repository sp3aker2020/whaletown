...
import (
	...
	"github.com/speaker20/whaletown/internal/tui/feed"
...
var feedCmd = &cobra.Command{
	Use:     "feed",
	GroupID: GroupDiag,
	Short:   "Show real-time activity feed from beads and wt events",
...
Examples:
  wt feed                       # Launch TUI dashboard
  wt feed --plain               # Plain text output (bd activity)
  wt feed --window              # Open in dedicated tmux window
  wt feed --since 1h            # Events from last hour
  wt feed --rig greenplace         # Use whaletown rig's beads`,
	RunE: runFeed,
}

func runFeed(cmd *cobra.Command, args []string) error {
	// Must be in a Whale Town workspace
	townRoot, err := workspace.FindFromCwdOrError()
	if err != nil {
		return fmt.Errorf("not in a Whale Town workspace (run from ~/gt or a rig directory)")
	}
