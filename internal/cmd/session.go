...
import (
	...
	"github.com/speaker20/whaletown/internal/suggest"
...
TIP: To send messages to a running session, use 'wt nudge' (not 'session inject').
...
Examples:
  wt session start wyvern/Toast
  wt session start wyvern/Toast --issue gt-123`,
...
Examples:
  wt session capture wyvern/Toast        # Last 100 lines (default)
  wt session capture wyvern/Toast 50     # Last 50 lines
  wt session capture wyvern/Toast -n 50  # Same as above`,
...
var sessionInjectCmd = &cobra.Command{
	...
	Short: "Send message to session (prefer 'wt nudge')",
	Long: `Send a message to a polecat session.

NOTE: For sending messages to Claude sessions, use 'wt nudge' instead.
...
Examples:
  wt nudge greenplace/furiosa "Check your mail"     # Preferred
  wt session inject wyvern/Toast -f prompt.txt   # For file injection`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt session check              # Check all rigs
  wt session check greenplace      # Check specific rig`,
...
func runSessionStart(cmd *cobra.Command, args []string) error {
	...
	if !found {
		...
		hint := fmt.Sprintf("Create with: wt polecat add %s/%s", rigName, polecatName)
...
func runSessionCheck(cmd *cobra.Command, args []string) error {
	...
	for _, r := range rigs {
		...
		for _, entry := range entries {
			...
			sessionName := fmt.Sprintf("wt-%s-%s", r.Name, polecatName)
			totalChecked++
	...
	if totalCrashed > 0 {
		fmt.Printf("\n%s To restart crashed polecats: wt session restart <rig>/<polecat>\n",
