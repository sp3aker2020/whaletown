...
func crewSessionName(rigName, crewName string) string {
	return fmt.Sprintf("wt-%s-crew-%s", rigName, crewName)
}
...
func detectCrewFromCwd() (*crewDetection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("not in Whale Town workspace: %w", err)
	}
	if townRoot == "" {
		return nil, fmt.Errorf("not in Whale Town workspace")
	}
	...
	if len(parts) < 3 {
		return nil, fmt.Errorf("not inside a crew workspace - specify the crew name or cd into a crew directory (e.g., whaletown/crew/max)")
	}
...
func parseCrewSessionName(sessionName string) (rigName, crewName string, ok bool) {
	// Must start with "wt-" and contain "-crew-"
	if !strings.HasPrefix(sessionName, "wt-") {
		return "", "", false
	}
...
func findRigCrewSessions(rigName string) ([]string, error) { //nolint:unparam // error return kept for future use
	...
	prefix := fmt.Sprintf("wt-%s-crew-", rigName)
	var sessions []string
