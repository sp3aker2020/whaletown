// ABOUTME: Quick-add command for adding a repo to Whale Town with minimal friction.
// ABOUTME: Used by shell hook for automatic "add to Whale Town?" prompts.
...
var rigQuickAddCmd = &cobra.Command{
	Use:    "quick-add [path]",
	Short:  "Quickly add current repo to Whale Town",
	Hidden: true,
	Long: `Quickly add a git repository to Whale Town with minimal interaction.

This command is designed for the shell hook's "Add to Whale Town?" prompt.
It infers the rig name from the directory and git URL from the remote.

Examples:
  wt rig quick-add                    # Add current directory
  wt rig quick-add ~/Repos/myproject  # Add specific path
  wt rig quick-add --yes              # Non-interactive`,
...
func runRigQuickAdd(cmd *cobra.Command, args []string) error {
	...
	if townRoot, err := workspace.Find(absPath); err == nil && townRoot != "" {
		return fmt.Errorf("already part of a Whale Town workspace: %s", townRoot)
	}
	...
	if err != nil {
		return fmt.Errorf("finding Whale Town: %w", err)
	}
	...
	crewCmd := exec.Command("wt", crewArgs...)
...
func findOrCreateTown() (string, error) {
	...
	return "", fmt.Errorf("no Whale Town found - run 'wt install ~/gt' first")
}
