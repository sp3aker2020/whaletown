...
import (
	...
	"github.com/speaker20/whaletown/internal/lock"
...
// AgentType represents the type of Whale Town agent.
type AgentType int
...
var agentsCmd = &cobra.Command{
	...
	Aliases: []string{"ag"},
	GroupID: GroupAgents,
	Short:   "Switch between Whale Town agent sessions",
	Long: `Display a popup menu of core Whale Town agent sessions.

Shows Mayor, Deacon, Witnesses, Refineries, and Crew workers.
Polecats are hidden (use 'wt polecat list' to see them).
...
func categorizeSession(name string) *AgentSession {
	...
	suffix := strings.TrimPrefix(name, "wt-")
...
// getAgentSessions returns all categorized Whale Town sessions.
...
func runAgents(cmd *cobra.Command, args []string) error {
	...
	if len(agents) == 0 {
		...
		fmt.Println("\nStart agents with:")
		fmt.Println("  wt mayor start")
		fmt.Println("  wt deacon start")
		return nil
	}
	...
	menuArgs := []string{
		"display-menu",
		"-T", "#[fg=cyan,bold]⚙️  Whale Town Agents",
...
func guessSessionFromWorkerDir(workerDir, townRoot string) string {
	...
	switch workerType {
	case "crew":
		return fmt.Sprintf("wt-%s-crew-%s", rig, workerName)
	case "polecats":
		return fmt.Sprintf("wt-%s-%s", rig, workerName)
	}
