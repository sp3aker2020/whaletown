...
func resolvePolecatTargets(args []string, useAll bool) ([]polecatTarget, error) {
	...
	if useAll {
		...
		if _, _, err := parseAddress(rigName); err == nil {
			return nil, fmt.Errorf("with --all, provide just the rig name (e.g., 'wt polecat <cmd> %s --all')", strings.Split(rigName, "/")[0])
		}
		...
		for _, arg := range args {
			...
			if !strings.Contains(arg, "/") {
				return nil, fmt.Errorf("invalid address '%s': must be in 'rig/polecat' format (e.g., 'whaletown/Toast')", arg)
			}
...
func displaySafetyCheckBlocked(blocked []*SafetyCheckResult) {
	...
	fmt.Println("Options:")
	fmt.Printf("  1. Complete work: wt done (from polecat session)\n")
	fmt.Printf("  2. Push changes: git push (from polecat worktree)\n")
	fmt.Printf("  3. Escalate: wt mail send mayor/ -s \"RECOVERY_NEEDED\" -m \"...\"\n")
	fmt.Printf("  4. Force nuke (LOSES WORK): wt polecat nuke --force %s\n", strings.Join(polecatList, " "))
	fmt.Println()
}
