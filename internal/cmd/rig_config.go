...
// This file implements the wt rig config commands for viewing and manipulating
...
Example output:
  wt rig config show whaletown --layers
  Key                 Value        Source
  status              parked       wisp
...
Examples:
  wt rig config set whaletown status parked           # Wisp layer
  wt rig config set whaletown status docked --global  # Bead layer
  wt rig config set whaletown auto_restart --block    # Block inheritance`,
...
Example:
  wt rig config unset whaletown status`,
...
func setBeadLabel(townRoot string, r *rig.Rig, key, value string) error {
	...
	if err != nil {
		return fmt.Errorf("rig identity bead %s not found (run 'wt rig add' to create it)", rigBeadID)
	}
