...
Examples:
  wt audit --actor=greenplace/crew/joe       # Show all work by joe
  wt audit --actor=greenplace/polecats/toast # Show polecat toast's work
  wt audit --actor=mayor                  # Show mayor's activity
  wt audit --since=24h                    # Show all activity in last 24h
  wt audit --actor=joe --since=1h         # Combined filters
  wt audit --json                         # Output as JSON`,
	RunE: runAudit,
}
...
func collectBeadsActivity(townRoot, actor string, since time.Time) ([]AuditEntry, error) {
	...
	// Find the whaletown beads path (where gt- prefix issues live)
	whaletownBeadsPath := filepath.Join(townRoot, "whaletown", "mayor", "rig")
	b := beads.New(whaletownBeadsPath)
