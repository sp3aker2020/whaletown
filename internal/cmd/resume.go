...
By default, this command checks for parked work (from 'wt park') and whether
...
Examples:
  wt resume              # Check for and resume parked work
  wt resume --status     # Just show parked work status without resuming
  wt resume --handoff    # Check inbox for handoff messages`,
	RunE: runResume,
}
...
func runResume(cmd *cobra.Command, args []string) error {
	...
	if parked == nil {
		...
		fmt.Printf("  Use 'wt park <gate-id>' to park work on a gate\n")
		return nil
	}
...
func displayResumeStatus(status ResumeStatus, parked *ParkedWork) error {
	...
	if status.GateClosed {
		fmt.Printf("\n%s Gate cleared! Run 'wt resume' (without --status) to restore work.\n",
			style.Bold.Render("→"))
	}
...
func checkHandoffMessages() error {
	...
	inboxCmd := exec.Command("wt", "mail", "inbox", "--json")
	output, err := inboxCmd.Output()
	if err != nil {
		// Fallback to non-JSON if --json not supported
		inboxCmd = exec.Command("wt", "mail", "inbox")
		output, err = inboxCmd.Output()
		if err != nil {
	...
	if len(handoffs) == 0 {
		fmt.Printf("%s No handoff messages in inbox\n", style.Dim.Render("○"))
		fmt.Printf("  Handoff messages have 'HANDOFF' in the subject.\n")
		fmt.Printf("  Use 'wt handoff -s \"...\"' to create one when handing off.\n")
		return nil
	}
	...
	if len(handoffs) == 1 {
		fmt.Printf("%s Read full message: wt mail read %s\n", style.Bold.Render("→"), handoffs[0].ID)
	} else {
		fmt.Printf("%s Read messages: wt mail read <id>\n", style.Bold.Render("→"))
	}
	fmt.Printf("%s Clear after reading: wt mail close <id>\n", style.Dim.Render("💡"))
