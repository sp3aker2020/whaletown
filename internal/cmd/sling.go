...
This is THE command for assigning work in Whale Town. It handles:
...
Auto-Convoy:
  ...
  all work appears in 'wt convoy list', even "swarm of one" assignments.

  wt sling gt-abc whaletown              # Creates "Work: <issue-title>" convoy
  wt sling gt-abc whaletown --no-convoy  # Skip auto-convoy creation

Target Resolution:
  wt sling gt-abc                       # Self (current agent)
  wt sling gt-abc crew                  # Crew worker in current rig
  wt sling gp-abc greenplace               # Auto-spawn polecat in rig
  wt sling gt-abc greenplace/Toast         # Specific polecat
  wt sling gt-abc mayor                 # Mayor
  wt sling gt-abc deacon/dogs           # Auto-dispatch to idle dog
  wt sling gt-abc deacon/dogs/alpha     # Specific dog

Spawning Options (when target is a rig):
  wt sling gp-abc greenplace --create               # Create polecat if missing
  wt sling gp-abc greenplace --force                # Ignore unread mail
  wt sling gp-abc greenplace --account work         # Use specific Claude account

Natural Language Args:
  wt sling gt-abc --args "patch release"
  wt sling code-review --args "focus on security"

The --args string is stored in the bead and shown via wt prime. Since the
executor is an LLM, it interprets these instructions naturally.

Formula Slinging:
  wt sling mol-release mayor/           # Cook + wisp + attach + nudge
  wt sling towers-of-hanoi --var disks=3

Formula-on-Bead (--on flag):
  wt sling mol-review --on gt-abc       # Apply formula to existing work
  wt sling shiny --on gt-abc crew       # Apply formula, sling to crew

Compare:
  wt hook <bead>      # Just attach (no action)
  wt sling <bead>     # Attach + start now (keep context)
  wt handoff <bead>   # Attach + restart (fresh context)

The propulsion principle: if it's on your hook, YOU RUN IT.

Batch Slinging:
  wt sling gt-abc gt-def gt-ghi whaletown   # Sling multiple beads to a rig

  When multiple beads are provided with a rig target, each bead gets its own
  polecat. This parallelizes work dispatch without running wt sling N times.`,
...
func runSling(cmd *cobra.Command, args []string) error {
	...
	if polecatName := os.Getenv("WT_POLECAT"); polecatName != "" {
		return fmt.Errorf("polecats cannot sling (use wt done for handoff)")
	}
	...
	// Pattern: wt sling gt-abc gt-def gt-ghi whaletown
	...
	if slingOnTarget != "" {
		// Formula-on-bead mode: wt sling <formula> --on <bead>
		formulaName = args[0]
		...
		if err := verifyBeadExists(firstArg); err == nil {
			...
			if err := verifyFormulaExists(firstArg); err == nil {
				// Standalone formula mode: wt sling <formula> [target]
			...
			// This fixes: wt sling bd-ka761 beads/crew/dave failing with 'not a valid bead or formula'
	...
	if formulaName != "" {
		...
		wispCmd.Env = append(os.Environ(), "WT_ROOT="+townRoot)
		...
		// This is required for wt hook to recognize the molecule attachment.
	...
	// See: https://github.com/speaker20/whaletown/issues/148
	...
	if targetPane == "" {
		fmt.Printf("%s No pane to nudge (agent will discover work via wt prime)\n", style.Dim.Render("○"))
	} else {
		...
		if sessionName != "" {
			if err := ensureAgentReady(sessionName); err != nil {
				// Non-fatal: warn and continue, agent will discover work via wt prime
		...
		if err := injectStartPrompt(targetPane, beadID, slingSubject, slingArgs); err != nil {
			...
			fmt.Printf("  Agent will discover work via wt prime / bd show\n")
		} else {
