...
const DefaultAgentEmailDomain = "whaletown.local"
...
When run by an agent (WT_ROLE set), this command:
...
Examples:
  wt commit -m "Fix bug"              # Commit as current agent
  wt commit -am "Quick fix"           # Stage all and commit
  wt commit -- --amend                # Amend last commit

Identity mapping:
  Agent: whaletown/crew/jack  →  Name: whaletown/crew/jack
                                Email: whaletown.crew.jack@whaletown.local

When run without WT_ROLE (human), passes through to git commit with no changes.`,
	RunE:               runCommit,
...
func runCommit(cmd *cobra.Command, args []string) error {
	...
	// "whaletown/crew/jack" → "whaletown.crew.jack@domain"
...
// identityToEmail converts a Whale Town identity to a git email address.
// "whaletown/crew/jack" → "whaletown.crew.jack@domain"
// "mayor/" → "mayor@domain"
