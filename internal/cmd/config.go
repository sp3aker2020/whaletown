...
var configCmd = &cobra.Command{
	Use:     "config",
	GroupID: GroupConfig,
	Short:   "Manage Whale Town configuration",
	RunE:    requireSubcommand,
	Long: `Manage Whale Town configuration settings.

This command allows you to view and modify configuration settings
for your Whale Town workspace, including agent aliases and defaults.

Commands:
  wt config agent list              List all agents (built-in and custom)
  wt config agent get <name>         Show agent configuration
  wt config agent set <name> <cmd>   Set custom agent command
  wt config agent remove <name>      Remove custom agent
  wt config default-agent [name]     Get or set default agent`,
}
...
Examples:
  wt config agent list           # Text output
  wt config agent list --json    # JSON output`,
...
Examples:
  wt config agent get claude
  wt config agent get my-custom-agent`,
...
Examples:
  ...
  wt config agent set gemini-custom gemini --approval-mode yolo
  wt config agent set claude \"claude-glm\"  # Override built-in claude`,
...
Examples:
  wt config agent remove claude-glm`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt config default-agent           # Show current default
  wt config default-agent claude    # Set to claude
  wt config default-agent gemini    # Set to gemini
  wt config default-agent my-custom # Set to custom agent`,
...
When agents commit code via 'wt commit', their identity is converted
to a git email address. For example, "whaletown/crew/jack" becomes
"whaletown.crew.jack@{domain}".

With no arguments, shows the current domain.
With an argument, sets the domain.

Default: whaletown.local

Examples:
  wt config agent-email-domain                 # Show current domain
  wt config agent-email-domain whaletown.local   # Set to whaletown.local
  wt config agent-email-domain example.com     # Set custom domain`,
...
func runConfigAgentRemove(cmd *cobra.Command, args []string) error {
	...
	for _, builtin := range builtInAgents {
		if name == builtin {
			return fmt.Errorf("cannot remove built-in agent '%s' (use 'wt config agent set' to override it)", name)
		}
	}
...
func runConfigDefaultAgent(cmd *cobra.Command, args []string) error {
	...
	if !isValid {
		return fmt.Errorf("agent '%s' not found (use 'wt config agent list' to see available agents)", name)
	}
...
func runConfigAgentEmailDomain(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		...
		fmt.Printf("\nExample: whaletown/crew/jack → whaletown.crew.jack@%s\n", domain)
		return nil
	}
