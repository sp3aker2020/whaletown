...
There is NO idle state. A polecat is either:
  ...
  - Zombie: Finished but wt done failed (needs cleanup)

Self-cleaning model: When work completes, the polecat runs 'wt done',
...
Examples:
  wt polecat list greenplace
  wt polecat list --all
  wt polecat list greenplace --json`,
	RunE: runPolecatList,
}
...
var polecatAddCmd = &cobra.Command{
	...
	Deprecated: "use 'wt polecat identity add' instead. This command will be removed in v1.0.",
...
Example:
  wt polecat identity add greenplace Toast  # Preferred
  wt polecat add greenplace Toast           # Deprecated`,
...
Examples:
  wt polecat remove greenplace/Toast
  wt polecat remove greenplace/Toast greenplace/Furiosa
  wt polecat remove greenplace --all
  wt polecat remove greenplace --all --force`,
...
Examples:
  wt polecat sync greenplace/Toast
  wt polecat sync greenplace --all
  wt polecat sync greenplace/Toast --from-main`,
...
Examples:
  wt polecat status greenplace/Toast
  wt polecat status greenplace/Toast --json`,
...
Examples:
  wt polecat gc greenplace
  wt polecat gc greenplace --dry-run`,
	Args: cobra.ExactArgs(1),
	RunE: runPolecatGC,
}
...
Examples:
  wt polecat nuke greenplace/Toast
  wt polecat nuke greenplace/Toast greenplace/Furiosa
  wt polecat nuke greenplace --all
  wt polecat nuke greenplace --all --dry-run
  wt polecat nuke greenplace/Toast --force  # bypass safety checks`,
	Args: cobra.MinimumNArgs(1),
	RunE: runPolecatNuke,
}
...
Examples:
  wt polecat git-state greenplace/Toast
  wt polecat git-state greenplace/Toast --json`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt polecat check-recovery greenplace/Toast
  wt polecat check-recovery greenplace/Toast --json`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt polecat stale greenplace
  wt polecat stale greenplace --threshold 50
  wt polecat stale greenplace --json
  wt polecat stale greenplace --cleanup
  wt polecat stale greenplace --cleanup --dry-run`,
	Args: cobra.ExactArgs(1),
...
func runPolecatAdd(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(os.Stderr, "%s 'wt polecat add' is deprecated. Use 'wt polecat identity add' instead.\n",
...
func runPolecatNuke(cmd *cobra.Command, args []string) error {
	...
	for _, p := range targets {
		if polecatNukeDryRun {
			...
			fmt.Printf("  - Kill session: wt-%s-%s\n", p.rigName, p.polecatName)
