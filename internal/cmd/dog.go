...
import (
	...
	"github.com/speaker20/whaletown/internal/dog"
	"github.com/speaker20/whaletown/internal/mail"
	"github.com/speaker20/whaletown/internal/plugin"
...
Each dog gets a worktree per configured rig (e.g., whaletown, beads).
...
Examples:
  wt dog remove alpha
  wt dog remove alpha bravo
  wt dog remove --all
  wt dog remove alpha --force`,
...
Examples:
  wt dog dispatch --plugin rebuild-gt
  wt dog dispatch --plugin rebuild-gt --rig whaletown
  wt dog dispatch --plugin rebuild-gt --dog alpha
  wt dog dispatch --plugin rebuild-gt --create
  wt dog dispatch --plugin rebuild-gt --dry-run
  wt dog dispatch --plugin rebuild-gt --json`,
	RunE: runDogDispatch,
}
...
func showDogStatus(mgr *dog.Manager, name string) error {
	...
	if townRoot != "" {
		...
		if err == nil {
			sessionName := fmt.Sprintf("wt-%s-deacon-%s", townName, name)
			tm := tmux.NewTmux()
...
func showPackStatus(mgr *dog.Manager) error {
	...
	if len(dogs) == 0 {
		fmt.Println("  No dogs in kennel")
		fmt.Println()
		fmt.Println("  Use 'wt dog add <name>' to add a dog")
		return nil
	}
	...
	if idleCount > 0 {
		fmt.Println()
		fmt.Println(style.Dim.Render("  Ready for work. Use 'wt dog call' to wake."))
	}
...
// dogDispatchResult is the JSON output for wt dog dispatch.
