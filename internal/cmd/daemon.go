...
var daemonCmd = &cobra.Command{
	...
	Short:   "Manage the Whale Town daemon",
	RunE:    requireSubcommand,
	Long: `Manage the Whale Town background daemon.
...
var daemonStartCmd = &cobra.Command{
	...
	Long: `Start the Whale Town daemon in the background.

The daemon will run until stopped with 'wt daemon stop'.`,
	RunE: runDaemonStart,
}
...
var daemonStopCmd = &cobra.Command{
	...
	Long:  `Stop the running Whale Town daemon.`,
	RunE:  runDaemonStop,
}
...
var daemonStatusCmd = &cobra.Command{
	...
	Long:  `Show the current status of the Whale Town daemon.`,
...
func runDaemonStart(cmd *cobra.Command, args []string) error {
	...
	// We use 'wt daemon run' as the actual daemon process
	...
	if !running {
		return fmt.Errorf("daemon failed to start (check logs with 'wt daemon logs')")
	}
			...
			if binaryModTime, err := getBinaryModTime(); err == nil {
				...
				if binaryModTime.After(state.StartedAt) {
					fmt.Printf("  %s Binary is newer than process - consider '%s'\n",
						style.Bold.Render("⚠"),
						style.Dim.Render("gt daemon stop && wt daemon start"))
				}
			}
		}
	} else {
