...
The Mayor is the global coordinator for Whale Town:
...
func runMayorStart(cmd *cobra.Command, args []string) error {
	...
	if err := mgr.Start(mayorAgentOverride); err != nil {
		if err == mayor.ErrAlreadyRunning {
			return fmt.Errorf("Mayor session already running. Attach with: wt mayor attach")
		}
		return err
	}
...
func runMayorAttach(cmd *cobra.Command, args []string) error {
	...
	if !running {
		...
		if !t.IsAgentRunning(sessionID, config.ExpectedPaneCommands(agentCfg)...) {
			...
			// Build startup beacon for context (like wt handoff does)
