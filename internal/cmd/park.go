...
After parking, you can exit the session safely. Use 'wt resume' to check
...
type ParkedWork struct {
	// AgentID is the agent that parked (e.g., "whaletown/crew/max")
...
func runPark(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("\n%s You can now safely exit. Run 'wt resume' to check for cleared gates.\n",
		style.Dim.Render("→"))
