...
Examples:
  wt orphans              # Last 7 days (default)
  wt orphans --days=14    # Last 2 weeks
  wt orphans --all        # Show all orphans (no date filter)`,
	RunE: runOrphans,
}
...
The command will:
1. Find orphaned commits (same as 'wt orphans')
2. Find orphaned Claude processes (same as 'wt orphans procs')
3. Show what will be removed/killed
4. Ask for confirmation (unless --force)
5. Run git gc and kill processes

Examples:
  wt orphans kill              # Kill orphans from last 7 days (default)
  wt orphans kill --days=14    # Kill orphans from last 2 weeks
  wt orphans kill --all        # Kill all orphans
  wt orphans kill --dry-run    # Preview without deleting
  wt orphans kill --force      # Skip confirmation prompt`,
	RunE: runOrphansKill,
}
...
Examples:
  wt orphans procs        # List orphaned Claude processes
  wt orphans procs list   # Same as above
  wt orphans procs kill   # Kill orphaned processes`,
...
Examples:
  wt orphans procs list      # Show all orphan Claude processes`,
...
Examples:
  wt orphans procs kill      # Kill with confirmation
  wt orphans procs kill -f   # Force kill without confirmation`,
...
func runOrphansListProcesses(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("\n%s\n", style.Dim.Render("Use 'wt orphans procs kill' to terminate these processes"))
