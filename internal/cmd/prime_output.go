...
import (
	...
	"github.com/speaker20/whaletown/internal/checkpoint"
...
func outputMayorContext(ctx RoleContext) {
	...
	fmt.Println("You are the **Mayor** - the global coordinator of Whale Town.")
	fmt.Println()
...
func outputUnknownContext(ctx RoleContext) {
	fmt.Printf("%s\n\n", style.Bold.Render("# Whale Town Context"))
...
func outputHandoffContent(ctx RoleContext) {
	...
	fmt.Println()
	fmt.Println(style.Dim.Render("(Clear with: wt rig reset --handoff)"))
}
