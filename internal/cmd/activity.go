...
var activityCmd = &cobra.Command{
	...
	Long: `Emit and view activity events for the Whale Town activity feed.

Events are written to ~/gt/.events.jsonl and can be viewed with 'wt feed'.
...
var activityEmitCmd = &cobra.Command{
	...
	Long: `Emit an activity event to the Whale Town activity feed.
...
Examples:
  wt activity emit patrol_started --rig greenplace --count 3
  wt activity emit polecat_checked --rig greenplace --polecat Toast --status working --issue gp-xyz
  wt activity emit polecat_nudged --rig greenplace --polecat Toast --reason "idle for 10 minutes"
  wt activity emit escalation_sent --rig greenplace --target Toast --to mayor --reason "unresponsive"
  wt activity emit patrol_complete --rig greenplace --count 3 --message "All polecats healthy"`,
...
func runActivityEmit(cmd *cobra.Command, args []string) error {
	...
	// Validate we're in a Whale Town workspace
