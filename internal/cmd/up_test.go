...
func TestAgentStartResult_Fields(t *testing.T) {
	result := agentStartResult{
		name:   "Witness (whaletown)",
		ok:     true,
		detail: "wt-whaletown-witness",
	}

	if result.name != "Witness (whaletown)" {
		t.Errorf("name = %q, want %q", result.name, "Witness (whaletown)")
	}
	if !result.ok {
		t.Error("ok should be true")
	}
	if result.detail != "wt-whaletown-witness" {
		t.Errorf("detail = %q, want %q", result.detail, "wt-whaletown-witness")
	}
}
