...
import (
	...
	"github.com/speaker20/whaletown/internal/formula"
	...
	"github.com/speaker20/whaletown/internal/templates"
...
var installCmd = &cobra.Command{
	...
	Short:   "Create a new Whale Town HQ (workspace)",
	Long: `Create a new Whale Town HQ at the specified path.

The HQ (headquarters) is the top-level directory where Whale Town is installed -
...
Examples:
  wt install ~/gt                              # Create HQ at ~/gt
  wt install . --name my-workspace             # Initialize current dir
  wt install ~/gt --no-beads                   # Skip .beads/ initialization
  wt install ~/gt --git                        # Also init git with .gitignore
  wt install ~/gt --github=user/repo           # Create private GitHub repo (default)
  wt install ~/gt --github=user/repo --public  # Create public GitHub repo
  wt install ~/gt --shell                      # Install shell integration (sets WT_TOWN_ROOT/WT_RIG)`,
...
func init() {
	...
	installCmd.Flags().BoolVar(&installShell, "shell", false, "Install shell integration (sets WT_TOWN_ROOT/WT_RIG env vars)")
...
func runInstall(cmd *cobra.Command, args []string) error {
	...
	if isWS, _ := workspace.IsWorkspace(absPath); isWS && !installForce {
		...
		return fmt.Errorf("directory is already a Whale Town HQ (use --force to reinitialize)")
	}
	...
	fmt.Printf("%s Creating Whale Town HQ at %s\n\n",
	...
	if installShell {
		...
		if err := state.Enable(Version); err != nil {
			...
			fmt.Printf("   ✓ Enabled Whale Town globally\n")
		}
	}
...
func initTownBeads(townPath string) error {
	...
	// Configure custom types for Whale Town (agent, role, rig, convoy, slot).
	...
	// Register hq-cv- prefix for convoy beads (auto-created by wt sling).
...
// ensureCustomTypes registers Whale Town custom issue types with beads.
// Beads core only supports built-in types (bug, feature, task, etc.).
// Whale Town needs custom types: agent, role, rig, convoy, slot.
...
// Rig-level agent beads (witness, refinery) are created by wt rig add in rig beads.
//
...
func initTownAgentBeads(townPath string) error {
	...
	// bd init doesn't enable "custom" issue types by default, but Whale Town uses
