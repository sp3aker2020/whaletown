...
type agentStartResult struct {
	name   string // Display name like "Witness (whaletown)"
...
var upCmd = &cobra.Command{
	...
	Short:   "Bring up all Whale Town services",
	Long: `Start all Whale Town long-lived services.

This is the idempotent "boot" command for Whale Town. It ensures all
...
Running 'wt up' multiple times is safe - it only starts services that
...
func runUp(cmd *cobra.Command, args []string) error {
	...
	if upRestore {
		for _, rigName := range rigs {
			...
			for _, name := range crewStarted {
				printStatus(fmt.Sprintf("Crew (%s/%s)", rigName, name), true, fmt.Sprintf("wt-%s-crew-%s", rigName, name))
			}
		...
		for _, rigName := range rigs {
			...
			for _, name := range polecatsStarted {
				printStatus(fmt.Sprintf("Polecat (%s/%s)", rigName, name), true, fmt.Sprintf("wt-%s-polecat-%s", rigName, name))
			}
	...
	if allOK {
		...
		_ = events.LogFeed(events.TypeBoot, "wt", events.BootPayload("town", startedServices))
	} else {
