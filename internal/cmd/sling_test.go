...
func TestParseWispIDFromJSON(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "new_epic_id",
			json:   `{"new_epic_id":"wt-wisp-abc","created":7,"phase":"vapor"}`,
			wantID: "wt-wisp-abc",
		},
		{
			name:   "root_id legacy",
			json:   `{"root_id":"wt-wisp-legacy"}`,
			wantID: "wt-wisp-legacy",
		},
		{
			name:   "result_id forward compat",
			json:   `{"result_id":"wt-wisp-result"}`,
			wantID: "wt-wisp-result",
		},
		{
			name:   "precedence prefers new_epic_id",
			json:   `{"root_id":"wt-wisp-legacy","new_epic_id":"wt-wisp-new"}`,
			wantID: "wt-wisp-new",
		},
		{
...
func TestFormatTrackBeadID(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "whaletown rig bead",
			beadID:   "wt-mol-abc123",
...
func TestSlingFormulaOnBeadRoutesBDCommandsToTargetRig(t *testing.T) {
	...
	rigDir := filepath.Join(townRoot, "whaletown", "mayor", "rig")
...
case "$cmd" in
  ...
  mol)
    ...
    case "$sub" in
      wisp)
        echo '{"new_epic_id":"wt-wisp-xyz"}'
        ;;
      bond)
        echo '{"root_id":"wt-wisp-xyz"}'
        ;;
    esac
    ;;
esac
...
// wt sling <formula> --on <bead>, both --var feature=<title> and --var issue=<beadID>
...
// TestSlingWithAllowStale tests the full wt sling flow with --allow-stale fix.
...
`
	...
	// EXPECTED: wt sling should use daemon mode and succeed
...
// This ensures wt sling accepts bead IDs even when routing-based verification fails.
// Fixes: wt sling bd-ka761 failing with 'not a valid bead or formula'
//
...
// wt sling <formula> --on <bead>, the attached_molecule field is set in the
// hooked bead's description after bonding. This is required for wt hook to
...
// wt hook to report "No molecule attached".
...
`
	...
	slingNoConvoy = true
	slingVars = nil
	slingOnTarget = "wt-abc123" // The bug bead we're applying formula to
	...
	// --description with attached_molecule field. This is what wt hook looks for.
	logLines := strings.Split(string(logBytes), "\n")
