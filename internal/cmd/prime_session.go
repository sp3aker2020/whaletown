...
// Priority: stdin JSON, WT_SESSION_ID env, CLAUDE_SESSION_ID env, auto-generate.
func readHookSessionID() (sessionID, source string) {
	...
	if id := os.Getenv("WT_SESSION_ID"); id != "" {
		return id, ""
	}
...
// This allows subsequent wt prime calls to find the session ID.
...
// Priority: WT_SESSION_ID env, CLAUDE_SESSION_ID env, persisted file, fallback.
func resolveSessionIDForPrime(actor string) string {
	// 1. Try runtime's session ID lookup (checks WT_SESSION_ID_ENV, then CLAUDE_SESSION_ID)
	if id := runtime.SessionIDFromEnv(); id != "" {
		return id
	}

	// 2. Persisted session file (from wt prime --hook)
...
// The event is written to ~/gt/.events.jsonl and can be queried via wt seance.
// Session ID resolution order: WT_SESSION_ID, CLAUDE_SESSION_ID, persisted file, fallback.
...
// This enables wt seance to discover sessions from wt prime output.
