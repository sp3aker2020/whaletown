...
Examples:
  wt unsling                        # Clear my hook (whatever's there)
  wt unsling gt-abc                 # Only unsling if gt-abc is hooked
  wt unsling greenplace/joe            # Clear joe's hook
  wt unsling gt-abc greenplace/joe     # Unsling gt-abc from joe

The bead's status changes from 'hooked' back to 'open'.

Related commands:
  wt sling <bead>    # Hook + start (inverse of unsling)
  wt hook <bead>     # Hook without starting
  wt hook      # See what's on your hook`,
...
func runUnsling(cmd *cobra.Command, args []string) error {
	...
	// Extract rig name from agent ID (e.g., "whaletown/crew/joe" -> "whaletown")
