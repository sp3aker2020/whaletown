...
import (
	...
	"github.com/speaker20/whaletown/internal/tui/convoy"
...
WHAT IS A CONVOY:
  ...
  - Tracks issues across rigs (frontend+backend, beads+whaletown, etc.)
...
Examples:
  wt convoy create "Deploy v2.0" gt-abc bd-xyz
  wt convoy create "Release prep" gt-abc --notify           # defaults to mayor/
  wt convoy create "Release prep" gt-abc --notify ops/      # notify ops/
  wt convoy create "Feature rollout" gt-a gt-b --owner mayor/ --notify ops/
  wt convoy create "Feature rollout" gt-a gt-b gt-c --molecule mol-release`,
...
Examples:
  wt convoy list              # Open convoys only (default)
  wt convoy list --all        # All convoys (open + closed)
  wt convoy list --status=closed  # Recently landed
  wt convoy list --tree       # Show convoy + child status tree
  wt convoy list --json`,
	RunE: runConvoyList,
}
...
Examples:
  wt convoy add hq-cv-abc gt-new-issue
  wt convoy add hq-cv-abc gt-issue1 gt-issue2 gt-issue3`,
...
Examples:
  wt convoy stranded              # Show stranded convoys
  wt convoy stranded --json       # Machine-readable output for automation`,
...
Examples:
  wt convoy close hq-cv-abc
  wt convoy close hq-cv-abc --reason="work done differently"
  wt convoy close hq-cv-xyz --notify mayor/`,
...
func sendCloseNotification(addr, convoyID, title, reason string) {
	...
	mailCmd := exec.Command("wt", mailArgs...)
...
func runConvoyStranded(cmd *cobra.Command, args []string) error {
	...
	for _, s := range stranded {
		fmt.Printf("  wt sling mol-convoy-feed deacon/dogs --var convoy=%s\n", s.ID)
	}
...
func runConvoyStatus(cmd *cobra.Command, args []string) error {
	...
	if len(tracked) > 0 {
		...
		for _, t := range tracked {
			...
			// Show assignee in brackets (extract short name from path like whaletown/polecats/goose -> goose)
...
func showAllConvoyStatus(townBeads string) error {
	...
	if len(convoys) == 0 {
		...
		fmt.Println("Create a convoy with: wt convoy create <name> [issues...]")
		return nil
	}
	...
	fmt.Printf("\nUse 'wt convoy status <id>' for detailed status.\n")
...
func runConvoyList(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("\nUse 'wt convoy status <id>' or 'wt convoy status <n>' for detailed view.\n")
...
type trackedIssueInfo struct {
	...
	Type      string `json:"dependency_type"`
	IssueType string `json:"issue_type"`
	Assignee  string `json:"assignee,omitempty"`   // Assigned agent (e.g., whaletown/polecats/goose)
	Worker    string `json:"worker,omitempty"`     // Worker currently assigned (e.g., whaletown/nux)
...
type workerInfo struct {
	Worker string // Agent identity (e.g., whaletown/nux)
...
// Input: "wt-whaletown-polecat-nux" -> Output: "whaletown/nux"
// Input: "wt-beads-crew-amber" -> Output: "beads/crew/amber"
...
// Numbers correspond to the order shown in 'wt convoy list'.
