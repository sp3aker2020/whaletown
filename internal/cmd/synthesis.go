...
Examples:
  wt synthesis status hq-cv-abc     # Check if ready for synthesis
  wt synthesis start hq-cv-abc      # Start synthesis step
  wt synthesis close hq-cv-abc      # Close convoy after synthesis`,
}
...
// runSynthesisStart implements wt synthesis start.
func runSynthesisStart(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  Monitor: wt convoy status %s\n", convoyID)

	return nil
}

// runSynthesisStatus implements wt synthesis status.
func runSynthesisStatus(cmd *cobra.Command, args []string) error {
	...
	if allComplete {
		...
		fmt.Printf("    Run: wt synthesis start %s\n", convoyID)
	} else {
		completedCount := 0
		for _, leg := range legOutputs {
			if leg.Status == "closed" {
...
// runSynthesisClose implements wt synthesis close.
...
func createSynthesisBead(convoyID string, meta *ConvoyMeta, f *formula.Formula,
	...
	if err := json.Unmarshal(stdout.Bytes(), &result); err != nil {
		...
		if strings.HasPrefix(out, "hq-") || strings.HasPrefix(out, "wt-") {
			return out, nil
		}
...
func findFormula(name string) (string, error) {
	...
	// Add WT_ROOT formulas if set
	if gtRoot := os.Getenv("WT_ROOT"); gtRoot != "" {
