...
func runCrewAt(cmd *cobra.Command, args []string) error {
	...
	// Debug mode: --debug flag or WT_DEBUG env var
	debug := crewDebug || os.Getenv("WT_DEBUG") != ""
	if debug {
		cwd, _ := os.Getwd()
	...
	if !hasSession {
		...
		_ = t.ConfigureWhaleTownSession(sessionID, theme, r.Name, name, "crew")
		...
		// Export WT_ROLE and BD_ACTOR since tmux SetEnvironment only affects new panes
	...
	// Outside tmux: attach unless --detached flag is set
	if crewDetached {
		fmt.Printf("Started %s/%s. Run 'wt crew at %s' to attach.\n", r.Name, name, name)
		return nil
	}
