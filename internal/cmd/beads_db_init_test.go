...
func TestBeadsDbInitAfterClone(t *testing.T) {
	...
	t.Run("TrackedRepoWithExistingPrefix", func(t *testing.T) {
		// GitHub Issue #72: wt rig add should detect existing prefix from tracked beads
		// https://github.com/speaker20/whaletown/issues/72
		//
		// This tests that when a tracked beads repo has existing issues in issues.jsonl,
		// wt rig add can detect the prefix from those issues WITHOUT --prefix flag.
	...
	t.Run("TrackedRepoWithNoIssuesRequiresPrefix", func(t *testing.T) {
		...
		// wt rig add must use the --prefix flag since there's nothing to detect from.
	...
	t.Run("TrackedRepoWithPrefixMismatchErrors", func(t *testing.T) {
		...
		// the prefix detected from existing issues, wt rig add fails with an error.
	...
	t.Run("TrackedRepoWithNoIssuesFallsBackToDerivedPrefix", func(t *testing.T) {
		...
		// and NO --prefix is provided, wt rig add should derive prefix from rig name.
