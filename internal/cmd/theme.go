...
var themeCmd = &cobra.Command{
	...
	Long: `Manage tmux status bar themes for Whale Town sessions.

Without arguments, shows the current theme assignment.
With a name argument, sets the theme for this rig.

Examples:
  wt theme              # Show current theme
  wt theme --list       # List available themes
  wt theme forest       # Set theme to 'forest'
  wt theme apply        # Apply theme to all running sessions in this rig`,
	RunE: runTheme,
}
...
var themeApplyCmd = &cobra.Command{
	...
	Long: `Apply theme to running Whale Town sessions.
...
func runTheme(cmd *cobra.Command, args []string) error {
	...
	fmt.Println("Run 'wt theme apply' to apply to running sessions")
...
func runThemeApply(cmd *cobra.Command, args []string) error {
	...
	for _, sess := range sessions {
		...
		if sess == mayorSession {
			...
			worker = "Deacon"
			role = "health-check"
		} else if strings.HasSuffix(sess, "-witness") && strings.HasPrefix(sess, "wt-") {
			// Witness sessions: gt-<rig>-witness
			rig = strings.TrimPrefix(strings.TrimSuffix(sess, "-witness"), "wt-")
...
func detectCurrentRig() string {
	// Try environment first (WT_RIG is set in tmux sessions)
	...
	if session := detectCurrentSession(); session != "" {
		...
		if len(parts) >= 2 && parts[0] == "wt" && parts[1] != "mayor" && parts[1] != "deacon" {
			return parts[1]
		}
	}
