...
// setupHookTestTown creates a minimal Whale Town with a polecat for testing hooks.
...
func setupHookTestTown(t *testing.T) (townRoot, polecatDir string) {
	...
	polecatDir = filepath.Join(townRoot, "whaletown", "polecats", "toast")
...
func TestHookSlot_BasicHook(t *testing.T) {
	...
	agentID := "whaletown/polecats/toast"
...
func TestHookSlot_Singleton(t *testing.T) {
	...
	// The singleton constraint is enforced by wt hook, not bd itself
	hookedBeads, err := b.List(beads.ListOptions{
		Status:   beads.StatusHooked,
		Assignee: agentID,
		Priority: -1,
	...
	// The wt hook command enforces singleton behavior
...
func TestHookSlot_DifferentAgents(t *testing.T) {
	...
	polecat2Dir := filepath.Join(townRoot, "whaletown", "polecats", "nux")
	...
	b := beads.New(rigDir)
	agent1 := "whaletown/polecats/toast"
	agent2 := "whaletown/polecats/nux"
	status := beads.StatusHooked
