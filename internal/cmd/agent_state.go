...
OPERATIONS:
  Get all labels (default):
    wt agent state <agent-bead>

  Set a label:
    wt agent state <agent-bead> --set idle=0
    wt agent state <agent-bead> --set idle=0 --set backoff=30s

  Increment a numeric label:
    wt agent state <agent-bead> --incr idle
    (Creates label with value 1 if not present)

  Delete a label:
    wt agent state <agent-bead> --del idle
...
EXAMPLES:
  ...
  wt agent state gt-whaletown-witness

  # Reset idle counter after finding work
  wt agent state gt-whaletown-witness --set idle=0

  # Increment idle counter on timeout
  wt agent state gt-whaletown-witness --incr idle

  # Get state as JSON
  wt agent state gt-whaletown-witness --json`,
