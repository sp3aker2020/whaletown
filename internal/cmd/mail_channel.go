...
Examples:
  wt mail channel              # List all channels
  wt mail channel alerts       # View messages from 'alerts' channel
  wt mail channel list         # Alias for listing channels
  wt mail channel show alerts  # Same as: wt mail channel alerts
  wt mail channel create alerts --retain-count=100
  wt mail channel delete alerts`,
...
func runChannelList(cmd *cobra.Command, args []string) error {
	...
	if len(channels) == 0 {
		...
		fmt.Println("\nCreate one with: wt mail channel create <name>")
		return nil
	}
