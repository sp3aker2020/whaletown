...
func TestDoneUsesResolveBeadsDir(t *testing.T) {
	...
	mayorRigBeadsDir := filepath.Join(tmpDir, "whaletown", "mayor", "rig", ".beads")
	polecatDir := filepath.Join(tmpDir, "whaletown", "polecats", "fixer")
	...
	t.Run("redirect not present uses local beads", func(t *testing.T) {
		...
		localDir := filepath.Join(tmpDir, "whaletown", "mayor", "rig")
