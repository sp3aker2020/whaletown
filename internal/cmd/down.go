...
var downCmd = &cobra.Command{
	...
	Short:   "Stop all Whale Town services",
	Long: `Stop Whale Town services (reversible pause).

Shutdown levels (progressively more aggressive):
  wt down                    Stop infrastructure (default)
  wt down --polecats         Also stop all polecat sessions
  wt down --all              Also stop bd daemons/activity
  wt down --nuke             Also kill the tmux server (DESTRUCTIVE)
...
This is a "pause" operation - use 'wt start' to bring everything back up.
For permanent cleanup (removing worktrees), use 'wt shutdown' instead.
...
func runDown(cmd *cobra.Command, args []string) error {
	...
	if allOK {
		...
		_ = events.LogFeed(events.TypeHalt, "wt", events.HaltPayload(stoppedServices))
	} else {
