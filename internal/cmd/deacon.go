...
import (
	...
	"github.com/speaker20/whaletown/internal/claude"
	"github.com/speaker20/whaletown/internal/config"
	"github.com/speaker20/whaletown/internal/constants"
	"github.com/speaker20/whaletown/internal/deacon"
	"github.com/speaker20/whaletown/internal/polecat"
	"github.com/speaker20/whaletown/internal/runtime"
	"github.com/speaker20/whaletown/internal/session"
	"github.com/speaker20/whaletown/internal/style"
	"github.com/speaker20/whaletown/internal/tmux"
	"github.com/speaker20/whaletown/internal/util"
...
var deaconCmd = &cobra.Command{
	...
	Long: `Manage the Deacon - the town-level watchdog for Whale Town.
...
Examples:
  wt deacon heartbeat                    # Touch heartbeat with timestamp
  wt deacon heartbeat "checking mayor"   # Touch with action description`,
...
In steady-state, the Deacon should use AI-based observation instead:
  wt deacon pending     # View pending spawns with captured output
  wt peek <session>     # Observe session output (AI analyzes)
  wt nudge <session>    # Trigger when AI determines ready
...
Examples:
  wt deacon health-check whaletown/polecats/max
  wt deacon health-check whaletown/witness --timeout=60s
  wt deacon health-check deacon --failures=5`,
...
- wt rig boot restarts it
- Or stays asleep until next activity trigger

This respects the cooldown period - won't kill if recently killed.

Examples:
  wt deacon force-kill whaletown/polecats/max
  wt deacon force-kill whaletown/witness --reason="unresponsive for 90s"`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt deacon stale-hooks                 # Find and unhook stale beads
  wt deacon stale-hooks --dry-run       # Preview what would be unhooked
  wt deacon stale-hooks --max-age=30m   # Use 30 minute threshold`,
...
The pause state persists across session restarts. Use 'wt deacon resume'
to allow the Deacon to work again.

Examples:
  wt deacon pause                           # Pause with no reason
  wt deacon pause --reason="testing"        # Pause with a reason`,
	RunE: runDeaconPause,
}
...
func runDeaconStart(cmd *cobra.Command, args []string) error {
	...
	if running {
		return fmt.Errorf("Deacon session already running. Attach with: wt deacon attach")
	}
...
func startDeaconSession(t *tmux.Tmux, sessionName, agentOverride string) error {
	...
	// Deacon runs from its own directory (for correct role detection by wt prime)
	...
	// Export WT_ROLE and BD_ACTOR in the command since tmux SetEnvironment only affects new panes
	...
	// See: https://github.com/anthropics/whaletown/issues/280
	...
	// Note: ConfigureWhaleTownSession includes cycle bindings
	theme := tmux.DeaconTheme()
	_ = t.ConfigureWhaleTownSession(sessionName, theme, "", "Deacon", "health-check")
	...
	// GUPP: Whale Town Universal Propulsion Principle
...
func runDeaconRestart(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  %s\n", style.Dim.Render("Use 'wt deacon attach' to connect"))
	return nil
}
...
func runDeaconHeartbeat(cmd *cobra.Command, args []string) error {
	...
	if paused {
		fmt.Printf("%s Deacon is paused. Use 'wt deacon resume' to unpause.\n", style.Bold.Render("⏸️"))
		if state.Reason != "" {
			fmt.Printf("  Reason: %s\n", state.Reason)
		}
...
func runDeaconForceKill(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  %s\n", style.Dim.Render("Agent is now 'asleep'. Use 'wt rig boot' to restart."))
...
// Supports formats: "whaletown/polecats/max", "whaletown/witness", "deacon", "mayor"
...
func agentAddressToIDs(address string) (beadID, sessionName string, err error) {
	...
	switch len(parts) {
	case 2:
		// rig/role: "whaletown/witness", "whaletown/refinery"
		rig, role := parts[0], parts[1]
		switch role {
		case "witness":
			return fmt.Sprintf("wt-%s-witness", rig), fmt.Sprintf("wt-%s-witness", rig), nil
		case "refinery":
			return fmt.Sprintf("wt-%s-refinery", rig), fmt.Sprintf("wt-%s-refinery", rig), nil
		default:
			return "", "", fmt.Errorf("unknown role: %s", role)
		}
	case 3:
		// rig/type/name: "whaletown/polecats/max", "whaletown/crew/alpha"
		rig, agentType, name := parts[0], parts[1], parts[2]
		switch agentType {
		case "polecats":
			return fmt.Sprintf("wt-%s-polecat-%s", rig, name), fmt.Sprintf("wt-%s-%s", rig, name), nil
		case "crew":
			return fmt.Sprintf("wt-%s-crew-%s", rig, name), fmt.Sprintf("wt-%s-crew-%s", rig, name), nil
		default:
...
// sendMail sends a mail message using wt mail send.
func sendMail(townRoot, to, subject, body string) {
	cmd := exec.Command("wt", "mail", "send", to, "-s", subject, "-m", body)
	cmd.Dir = townRoot
