...
import (
	...
	"github.com/speaker20/whaletown/internal/swarm"
...
var swarmCmd = &cobra.Command{
	Use:        "swarm",
	GroupID:    GroupWork,
	Short:      "[DEPRECATED] Use 'wt convoy' instead",
	Deprecated: "Use 'wt convoy' for work tracking. A 'swarm' is now just the ephemeral workers on a convoy.",
	RunE:       requireSubcommand,
	Long: `DEPRECATED: Use 'wt convoy' instead.

The term "swarm" now refers to the ephemeral set of workers on a convoy's issues,
not a persistent tracking unit. Use 'wt convoy' for creating and tracking batched work.
...
MIGRATION:
  wt swarm create  →  wt convoy create
  wt swarm status  →  wt convoy status
  wt swarm list    →  wt convoy list

See 'wt convoy --help' for the new workflow.`,
}
...
Examples:
  wt swarm create greenplace --epic gp-abc --worker Toast --worker Nux
  wt swarm create greenplace --epic gp-abc --worker Toast --start`,
...
Examples:
  wt swarm list
  wt swarm list greenplace
  wt swarm list --status=active
  wt swarm list greenplace --status=landed`,
...
Examples:
  wt swarm dispatch gt-abc         # Dispatch next task from epic gt-abc
  wt swarm dispatch gt-abc --rig greenplace  # Dispatch in specific rig`,
	Args: cobra.ExactArgs(1),
...
func runSwarmCreate(cmd *cobra.Command, args []string) error {
	...
	if swarmStart {
		...
		fmt.Printf("\n  %s\n", style.Dim.Render("Use --start or 'wt swarm start' to activate"))
	}
...
func runSwarmStart(cmd *cobra.Command, args []string) error {
	...
	if len(swarmWorkers) > 0 {
		...
		fmt.Printf("\nUse 'wt sling <task-id> <rig>/<worker>' to assign tasks\n")
	}
...
func runSwarmDispatch(cmd *cobra.Command, args []string) error {
	...
	// Just sling to the rig and let wt sling spawn a fresh polecat.
	task := unassigned[0]

	fmt.Printf("Dispatching %s to fresh polecat in %s...\n", task.ID, foundRig.Name)

	// Use wt sling to spawn a fresh polecat and assign the task
	slingCmd := exec.Command("wt", "sling", task.ID, foundRig.Name)
...
func spawnSwarmWorkersFromBeads(r *rig.Rig, townRoot string, swarmID string, workers []string, tasks []struct {
	...
	for _, task := range tasks {
		...
		// Use wt sling to assign task to worker (this updates beads)
		slingCmd := exec.Command("wt", "sling", task.ID, fmt.Sprintf("%s/%s", r.Name, worker))
		slingCmd.Dir = townRoot
		if err := slingCmd.Run(); err != nil {
...
func runSwarmList(cmd *cobra.Command, args []string) error {
	...
	if len(allSwarms) == 0 {
		...
		fmt.Println("Create a swarm with: wt swarm create <rig> --epic <epic-id>")
		return nil
	}
	...
	fmt.Printf("\nUse 'wt swarm status <id>' for detailed status.\n")
