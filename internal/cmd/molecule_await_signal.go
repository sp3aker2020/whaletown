...
EXAMPLES:
  ...
  wt mol await-signal --timeout 60s

  # Backoff mode with agent bead tracking:
  wt mol await-signal --agent-bead gt-whaletown-witness \
    --backoff-base 30s --backoff-mult 2 --backoff-max 5m

  # On timeout, the agent bead's idle:N label is auto-incremented
  # On signal, caller should reset: wt agent state gt-whaletown-witness --set idle=0

  # Quiet mode (no output, for scripting)
  wt mol await-signal --timeout 30s --quiet`,
