// Package cmd provides CLI commands for the wt tool.
package cmd
...
var costsCmd = &cobra.Command{
	...
	Long: `Display costs for Claude Code sessions in Whale Town.
...
Examples:
  wt costs              # Live costs from running sessions
  wt costs --today      # Today's costs from wisps (not yet digested)
  wt costs --week       # This week's costs from digest beads + today's wisps
  wt costs --by-role    # Breakdown by role (polecat, witness, etc.)
  wt costs --by-rig     # Breakdown by rig
  wt costs --json       # Output as JSON

Subcommands:
  wt costs record       # Record session cost as ephemeral wisp (Stop hook)
  wt costs digest       # Aggregate wisps into daily digest bead (Deacon patrol)`,
	RunE: runCosts,
}
...
Session cost wisps are aggregated daily by 'wt costs digest' into a single
permanent "Cost Report YYYY-MM-DD" bead for audit purposes.

Examples:
  wt costs record --session gt-whaletown-toast
  wt costs record --session gt-whaletown-toast --work-item gt-abc123`,
	RunE: runCostsRecord,
}
...
Examples:
  wt costs digest --yesterday   # Digest yesterday's costs (default for patrol)
  wt costs digest --date 2026-01-07  # Digest a specific date
  wt costs digest --yesterday --dry-run  # Preview without changes`,
	RunE: runCostsDigest,
}
...
Examples:
  wt costs migrate            # Migrate legacy beads
  wt costs migrate --dry-run  # Preview what would be migrated`,
...
func runLiveCosts() error {
	...
	for _, session := range sessions {
		// Only process Whale Town sessions (start with "wt-")
...
func querySessionEvents() []CostEntry {
	...
	if err != nil {
		// Not in a Whale Town workspace - return empty list
		return nil
	}
...
//   - gt-whaletown-toast -> role=polecat, rig=whaletown, worker=toast
//   - gt-whaletown-witness -> role=witness, rig=whaletown, worker=""
//   - gt-whaletown-refinery -> role=refinery, rig=whaletown, worker=""
//   - gt-whaletown-crew-joe -> role=crew, rig=whaletown, worker=joe
...
func outputCostsHuman(costs []SessionCost, total float64) error {
	if len(costs) == 0 {
		fmt.Println(style.Dim.Render("No Whale Town sessions found"))
		return nil
	}
...
func runCostsRecord(cmd *cobra.Command, args []string) error {
	...
	if session == "" {
		// Derive session name from WT_* environment variables
	...
	if session == "" {
		return fmt.Errorf("--session flag required (or set WT_SESSION env var, or WT_RIG/WT_ROLE)")
	}
	...
	// - Will be aggregated into daily digests by 'wt costs digest'
	bdArgs := []string{
		"create",
		"--ephemeral",
		"--type=event",
	...
	if townRoot == "" {
		return fmt.Errorf("not in a Whale Town workspace")
	}
...
// deriveSessionName derives the tmux session name from WT_* environment variables.
// Session naming patterns:
//   - Polecats: gt-{rig}-{polecat} (e.g., gt-whaletown-toast)
//   - Crew: gt-{rig}-crew-{crew} (e.g., gt-whaletown-crew-max)
//   - Witness/Refinery: gt-{rig}-{role} (e.g., gt-whaletown-witness)
//   - Mayor/Deacon: gt-{town}-{role} (e.g., gt-ai-mayor)
func deriveSessionName() string {
	role := os.Getenv("WT_ROLE")
	rig := os.Getenv("WT_RIG")
	polecat := os.Getenv("WT_POLECAT")
	crew := os.Getenv("WT_CREW")
	town := os.Getenv("WT_TOWN")

	// Polecat: gt-{rig}-{polecat}
	if polecat != "" && rig != "" {
		return fmt.Sprintf("wt-%s-%s", rig, polecat)
	}

	// Crew: gt-{rig}-crew-{crew}
	if crew != "" && rig != "" {
		return fmt.Sprintf("wt-%s-crew-%s", rig, crew)
	}

	// Town-level roles (mayor, deacon): gt-{town}-{role} or gt-{role}
	if role == "mayor" || role == "deacon" {
		if town != "" {
			return fmt.Sprintf("wt-%s-%s", town, role)
		}
		// No town set - use simple gt-{role} pattern
		return fmt.Sprintf("wt-%s", role)
	}

	// Rig-based roles (witness, refinery): gt-{rig}-{role}
	if role != "" && rig != "" {
		return fmt.Sprintf("wt-%s-%s", rig, role)
	}
...
func detectCurrentTmuxSession() string {
	...
	// Only return if it looks like a Whale Town session
...
// Examples: "mayor", "whaletown/witness", "whaletown/polecats/toast"
