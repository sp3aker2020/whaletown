...
func TestRoutesJSONLCorruption(t *testing.T) {
	...
	t.Run("TownLevelRoutesNotCorrupted", func(t *testing.T) {
		// Test that wt install creates issues.jsonl before routes.jsonl
		...
		if _, err := os.Stat(issuesPath); os.IsNotExist(err) {
			t.Error("issues.jsonl should exist after wt install")
		}
	...
	t.Run("RigLevelNoRoutesJSONL", func(t *testing.T) {
		// Test that wt rig add does NOT create routes.jsonl in rig beads
