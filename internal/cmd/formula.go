...
Search paths (in order):
  ...
  3. $WT_ROOT/.beads/formulas/ (orchestrator)

Examples:
  wt formula list                    # List all formulas
  wt formula show shiny              # Show formula details
  wt formula run shiny --pr=123      # Run formula on PR #123
  wt formula create my-workflow      # Create new formula template`,
}
...
Examples:
  wt formula list            # List all formulas
  wt formula list --json     # JSON output`,
	RunE: runFormulaList,
}
...
Examples:
  wt formula show shiny
  wt formula show rule-of-five --json`,
...
Options:
  ...
  --rig=NAME  Target specific rig (default: current or whaletown)
  --dry-run   Show what would happen without executing

Examples:
  wt formula run shiny                    # Run formula in current rig
  wt formula run                          # Run default formula from rig config
  wt formula run shiny --pr=123           # Run on PR #123
  wt formula run security-audit --rig=beads  # Run in specific rig
  wt formula run release --dry-run        # Preview execution`,
...
Examples:
  wt formula create my-task                  # Create task formula
  wt formula create my-workflow --type=workflow
  wt formula create nightly-check --type=patrol`,
	Args: cobra.ExactArgs(1),
...
func init() {
	...
	formulaRunCmd.Flags().StringVar(&formulaRunRig, "rig", "", "Target rig (default: current or whaletown)")
...
func runFormulaRun(cmd *cobra.Command, args []string) error {
	...
	if targetRig == "" {
		...
		if err == nil && townRoot != "" {
			...
			// If we still don't have a target rig but have townRoot, use whaletown
			if targetRig == "" {
				targetRig = "whaletown"
				rigPath = filepath.Join(townRoot, "whaletown")
			}
		} else {
			// No town root found, fall back to whaletown without rigPath
			targetRig = "whaletown"
		}
	} else {
	...
	if f.Type != "convoy" {
		...
		fmt.Printf("\nTo run '%s' manually:\n", formulaName)
		fmt.Printf("  1. View formula:   wt formula show %s\n", formulaName)
		fmt.Printf("  2. Cook to proto:  bd cook %s\n", formulaName)
		fmt.Printf("  3. Pour molecule:  bd pour %s\n", formulaName)
		fmt.Printf("  4. Sling to rig:   wt sling <mol-id> %s\n", targetRig)
		return nil
	}
...
func executeConvoyFormula(f *formulaData, formulaName, targetRig string) error {
	...
	for _, leg := range f.Legs {
		...
		// Use wt sling with args for leg-specific context
		slingArgs := []string{
		...
		slingCmd := exec.Command("wt", slingArgs...)
	...
	fmt.Printf("\n  Track progress: wt convoy status %s\n", convoyID)
...
func runFormulaCreate(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("  2. View it:          wt formula show %s\n", formulaName)
	fmt.Printf("  3. Run it:           wt formula run %s\n", formulaName)
