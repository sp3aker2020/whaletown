...
// setupRoutingTestTown creates a minimal Whale Town with multiple rigs for testing routing.
// Returns townRoot.
func setupRoutingTestTown(t *testing.T) string {
	...
	routes := []beads.Route{
		...
		{Prefix: "wt-", Path: "whaletown/mayor/rig"},      // Gastown rig
	...
	// Create whaletown rig structure
	gasRigPath := filepath.Join(townRoot, "whaletown", "mayor", "rig")
	if err := os.MkdirAll(gasRigPath, 0755); err != nil {
		t.Fatalf("mkdir whaletown: %v", err)
	}

	// Create whaletown .beads directory with its own config
	gasBeadsDir := filepath.Join(gasRigPath, ".beads")
	if err := os.MkdirAll(gasBeadsDir, 0755); err != nil {
		t.Fatalf("mkdir whaletown .beads: %v", err)
	}
	if err := os.WriteFile(filepath.Join(gasBeadsDir, "config.yaml"), []byte("prefix: gt\n"), 0644); err != nil {
		t.Fatalf("write whaletown config: %v", err)
	}
	...
	polecatsDir := filepath.Join(townRoot, "whaletown", "polecats", "rictus")
	if err := os.MkdirAll(polecatsDir, 0755); err != nil {
		t.Fatalf("mkdir polecats: %v", err)
	}

	// Create redirect file for polecat -> mayor/rig/.beads
	// Path: whaletown/polecats/rictus -> ../../mayor/rig/.beads -> whaletown/mayor/rig/.beads
	...
	// Path: whaletown/crew/max -> ../../mayor/rig/.beads -> whaletown/mayor/rig/.beads
	crewDir := filepath.Join(townRoot, "whaletown", "crew", "max")
...
func initBeadsDBWithPrefix(t *testing.T, dir, prefix string) {
	...
	// it finds), corrupting the routing configuration. This mirrors what wt install does.
...
func TestBeadsRoutingFromTownRoot(t *testing.T) {
	...
	whaletownRigPath := filepath.Join(townRoot, "whaletown", "mayor", "rig")
	testrigRigPath := filepath.Join(townRoot, "testrig", "mayor", "rig")
	initBeadsDBWithPrefix(t, whaletownRigPath, "wt")
	initBeadsDBWithPrefix(t, testrigRigPath, "tr")

	townIssue := createTestIssue(t, townRoot, "Town-level routing test")
	whaletownIssue := createTestIssue(t, whaletownRigPath, "Gastown routing test")
	...
	tests := []struct {
		...
		{whaletownIssue.ID, whaletownIssue.Title},
...
func TestBeadsRedirectResolution(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			workDir:  "whaletown/polecats/rictus",
			expected: "whaletown/mayor/rig/.beads",
		},
		{
			name:     "crew redirect",
			workDir:  "whaletown/crew/max",
			expected: "whaletown/mayor/rig/.beads",
		},
		{
			name:     "no redirect (mayor/rig)",
			workDir:  "whaletown/mayor/rig",
			expected: "whaletown/mayor/rig/.beads",
		},
	}
...
func TestBeadsPrefixConflictDetection(t *testing.T) {
	...
	routes := []beads.Route{
		{Prefix: "wt-", Path: "whaletown/mayor/rig"},
		{Prefix: "wt-", Path: "other/mayor/rig"}, // Duplicate!
...
func TestBeadsListFromPolecatDirectory(t *testing.T) {
	...
	townRoot := setupRoutingTestTown(t)
	polecatDir := filepath.Join(townRoot, "whaletown", "polecats", "rictus")

	rigPath := filepath.Join(townRoot, "whaletown", "mayor", "rig")
	initBeadsDBWithPrefix(t, rigPath, "wt")
...
func TestBeadsAppendRoute(t *testing.T) {
	...
	// Append first route
	route1 := beads.Route{Prefix: "wt-", Path: "whaletown/mayor/rig"}
	...
	route1Updated := beads.Route{Prefix: "wt-", Path: "newpath/mayor/rig"}
	...
	for _, r := range routes {
		if r.Prefix == "wt-" && r.Path != "newpath/mayor/rig" {
...
func TestBeadsRemoveRoute(t *testing.T) {
	...
	// Remove one route
	if err := beads.RemoveRoute(tmpDir, "wt-"); err != nil {
...
// This is the fix for https://github.com/speaker20/whaletown/issues/148
func TestSlingCrossRigRoutingResolution(t *testing.T) {
	...
	tests := []struct {
		...
		{"wt-mol-abc", "whaletown/mayor/rig"},
