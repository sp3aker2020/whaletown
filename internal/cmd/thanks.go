...
import (
	...
	"github.com/speaker20/whaletown/internal/ui"
)
...
// whaletownContributors maps HUMAN contributor names to their commit counts.
// Agent names (whaletown/*, beads/*, lowercase single-word names) are excluded.
// Generated from: git shortlog -sn --all (then filtered for humans only)
var whaletownContributors = map[string]int{
	"Steve Yegge":               2056,
	"Mike Lady":                 19,
...
var thanksCmd = &cobra.Command{
	Use:     "thanks",
	Short:   "Thank the human contributors to Whale Town",
	GroupID: GroupDiag,
	Long: `Display acknowledgments to all the humans who have contributed
to the Whale Town project. This command celebrates the collaborative
...
func getContributorsSorted() []string {
	names := make([]string, 0, len(whaletownContributors))
	for name := range whaletownContributors {
		names = append(names, name)
	}

	slices.SortFunc(names, func(a, b string) int {
		// sort by commit count descending, then by name ascending for ties
		countCmp := cmp.Compare(whaletownContributors[b], whaletownContributors[a])
		if countCmp != 0 {
			return countCmp
		}
...
func printThanksPage() {
	...
	subtitle := thanksSubtitleStyle.Render("To all the humans who contributed to Whale Town")
	headerContent := title + "\n\n" + subtitle
