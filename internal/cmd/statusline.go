...
func runStatusLine(cmd *cobra.Command, args []string) error {
	...
	if statusLineSession != "" {
		...
		rigName, _ = t.GetEnvironment(statusLineSession, "WT_RIG")
		polecat, _ = t.GetEnvironment(statusLineSession, "WT_POLECAT")
		crew, _ = t.GetEnvironment(statusLineSession, "WT_CREW")
		issue, _ = t.GetEnvironment(statusLineSession, "WT_ISSUE")
		role, _ = t.GetEnvironment(statusLineSession, "WT_ROLE")
	} else {
		// Fallback to process environment
		rigName = os.Getenv("WT_RIG")
		polecat = os.Getenv("WT_POLECAT")
		crew = os.Getenv("WT_CREW")
		issue = os.Getenv("WT_ISSUE")
...
func runWorkerStatusLine(t *tmux.Tmux, session, rigName, polecat, crew, issue string) error {
	...
	// Priority 2: Fall back to WT_ISSUE env var or in_progress beads
	currentWork := issue
...
func runWitnessStatusLine(t *tmux.Tmux, rigName string) error {
	if rigName == "" {
		...
		if strings.HasSuffix(statusLineSession, "-witness") && strings.HasPrefix(statusLineSession, "wt-") {
			rigName = strings.TrimPrefix(strings.TrimSuffix(statusLineSession, "-witness"), "wt-")
		}
	}

	// Get town root from witness pane's working directory
	var townRoot string
	sessionName := fmt.Sprintf("wt-%s-witness", rigName)
...
func runRefineryStatusLine(t *tmux.Tmux, rigName string) error {
	if rigName == "" {
		...
		if strings.HasPrefix(statusLineSession, "wt-") && strings.HasSuffix(statusLineSession, "-refinery") {
			rigName = strings.TrimPrefix(statusLineSession, "wt-")
	...
	var townRoot string
	sessionName := fmt.Sprintf("wt-%s-refinery", rigName)
	paneDir, err := t.GetPaneWorkDir(sessionName)
	if err == nil && paneDir != "" {
		townRoot, _ = workspace.Find(paneDir)
	}
...
func getMailPreviewWithRoot(identity string, maxLen int, townRoot string) (int, string) {
	// Use NewMailboxFromAddress to normalize identity (e.g., whaletown/crew/gus -> whaletown/gus)
