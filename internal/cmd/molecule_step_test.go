...
func TestExtractMoleculeIDFromStep(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "complex id",
			stepID:   "wt-some-complex-id.99",
			expected: "wt-some-complex-id",
		},
		{
...
func TestFindNextReadyStep(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			moleculeID: "wt-mol",
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Step 1", "wt-mol", "closed", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step 2", "wt-mol", "closed", []string{"wt-mol.1"}))
			},
			wantComplete: true,
			wantNilStep:  true,
		},
		{
			...
			moleculeID: "wt-mol",
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Step 1", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step 2", "wt-mol", "open", []string{"wt-mol.1"}))
			},
			wantStepID:   "wt-mol.1",
			wantComplete: false,
		},
		...
		},
		{
			...
			moleculeID: "wt-mol",
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Step 1", "wt-mol", "in_progress", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step 2", "wt-mol", "open", []string{"wt-mol.1"}))
				m.addIssue(makeStepIssue("wt-mol.3", "Step 3", "wt-mol", "open", []string{"wt-mol.2"}))
			},
			wantComplete: false,
		...
		},
		{
			...
			setupFunc: func(m *mockBeadsForStep) {
				...
				m.addIssue(makeStepIssue("wt-mol.1", "Step 1", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step 2", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.3", "Synthesis", "wt-mol", "open", []string{"wt-mol.1", "wt-mol.2"}))
			},
			wantComplete: false,
		...
		},
		{
			...
			moleculeID: "wt-mol",
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Step A", "wt-mol", "closed", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step B", "wt-mol", "open", nil)) // still open
				m.addIssue(makeStepIssue("wt-mol.3", "Synthesis", "wt-mol", "open", []string{"wt-mol.1", "wt-mol.2"}))
			},
			wantStepID:   "wt-mol.2", // B is ready (no deps)
			wantComplete: false,
		},
	}
...
func TestStepDoneScenarios(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Step 1", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Step 2", "wt-mol", "in_progress", nil)) // another parallel task
				m.addIssue(makeStepIssue("wt-mol.3", "Synthesis", "wt-mol", "open", []string{"wt-mol.1", "wt-mol.2"}))
			},
		...
		},
		{
			...
			stepID: "wt-mol.1",
			setupFunc: func(m *mockBeadsForStep) {
				m.addIssue(makeStepIssue("wt-mol.1", "Parallel A", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.2", "Parallel B", "wt-mol", "open", nil))
				m.addIssue(makeStepIssue("wt-mol.3", "Synthesis", "wt-mol", "open", []string{"wt-mol.1", "wt-mol.2"}))
			},
			wantAction:   "continue",
			wantNextStep: "wt-mol.2", // B is still ready
		},
	}
