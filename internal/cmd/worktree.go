...
For example, if you're whaletown/crew/joe and run 'wt worktree beads':
- Creates worktree at ~/gt/beads/crew/whaletown-joe/
- The worktree checks out main branch
- Your identity (BD_ACTOR, WT_ROLE) remains whaletown/crew/joe

Use --no-cd to just print the path without printing shell commands.

Examples:
  wt worktree beads         # Create worktree in beads rig
  wt worktree whaletown       # Create worktree in whaletown rig (from another rig)
  wt worktree beads --no-cd # Just print the path`,
...
Example output:
  Cross-rig worktrees for whaletown/crew/joe:

    beads     ~/gt/beads/crew/whaletown-joe/     (clean)
    mayor     ~/gt/mayor/crew/whaletown-joe/     (2 uncommitted)`,
...
This command removes a worktree that was previously created with 'wt worktree <rig>'.
It will refuse to remove a worktree with uncommitted changes unless --force is used.

Examples:
  wt worktree remove beads         # Remove beads worktree
  wt worktree remove beads --force # Force remove even with uncommitted changes`,
	Args: cobra.ExactArgs(1),
...
func runWorktree(cmd *cobra.Command, args []string) error {
	...
	if targetRig == sourceRig {
		return fmt.Errorf("already in rig '%s' - use wt worktree to work in a different rig", targetRig)
	}

	// Verify target rig exists
	_, targetRigInfo, err := getRig(targetRig)
	if err != nil {
		return fmt.Errorf("rig '%s' not found - run 'wt rig list' to see available rigs", targetRig)
	}
	...
	if worktreeNoCD {
		...
		fmt.Printf("  export WT_ROLE=crew\n")
		fmt.Printf("  export WT_RIG=%s\n", sourceRig)
		fmt.Printf("  export WT_CREW=%s\n", crewName)
	}
...
func runWorktreeList(cmd *cobra.Command, args []string) error {
	...
	if !found {
		fmt.Printf("  (none)\n")
		fmt.Printf("\nCreate a worktree with: wt worktree <rig>\n")
	}
