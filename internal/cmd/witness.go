...
The Witness patrols a single rig, watching over its polecats:
  ...
  - Nukes sandboxes when polecats complete via 'wt done'

The Witness does NOT force session cycles or interrupt working polecats.
Polecats manage their own sessions (via wt handoff). The Witness handles
...
Examples:
  wt witness start greenplace
  wt witness start greenplace --agent codex
  wt witness start greenplace --env ANTHROPIC_MODEL=claude-3-haiku
  wt witness start greenplace --foreground`,
...
Examples:
  wt witness attach greenplace
  wt witness attach          # infer rig from cwd`,
...
Examples:
  wt witness restart greenplace
  wt witness restart greenplace --agent codex
  wt witness restart greenplace --env ANTHROPIC_MODEL=claude-3-haiku`,
	Args: cobra.ExactArgs(1),
...
func runWitnessStart(cmd *cobra.Command, args []string) error {
	...
	if err := mgr.Start(witnessForeground, witnessAgentOverride, witnessEnvOverrides); err != nil {
		if err == witness.ErrAlreadyRunning {
			...
			fmt.Printf("  %s\n", style.Dim.Render("Use 'wt witness attach' to connect"))
			return nil
		}
	...
	fmt.Printf("  %s\n", style.Dim.Render("Use 'wt witness attach' to connect"))
	fmt.Printf("  %s\n", style.Dim.Render("Use 'wt witness status' to check progress"))
	return nil
}
...
func witnessSessionName(rigName string) string {
	return fmt.Sprintf("wt-%s-witness", rigName)
}
