...
var accountCmd = &cobra.Command{
	...
	Long: `Manage multiple Claude Code accounts for Whale Town.

This enables switching between accounts (e.g., personal vs work) with
easy account selection per spawn or globally.

Commands:
  wt account list              List registered accounts
  wt account add <handle>      Add a new account
  wt account default <handle>  Set the default account
  wt account status            Show current account info`,
}
...
Examples:
  wt account list           # Text output
  wt account list --json    # JSON output`,
	RunE: runAccountList,
}
...
Examples:
  wt account add work
  wt account add work --email steve@company.com
  wt account add work --email steve@company.com --desc "Work account"`,
...
The default account is used when no --account flag or WT_ACCOUNT env var
is specified during spawn or attach.

Examples:
  wt account default work
  wt account default personal`,
	Args: cobra.ExactArgs(1),
...
func runAccountList(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		fmt.Println("\nTo add an account:")
		fmt.Println("  wt account add <handle>")
		return nil
	}
...
1. WT_ACCOUNT environment variable (highest priority)
2. Default account from config

Examples:
  wt account status           # Show current account
  WT_ACCOUNT=work wt account status  # Show with env override`,
...
Examples:
  wt account switch work       # Switch to work account
  wt account switch personal   # Switch to personal account`,
	Args: cobra.ExactArgs(1),
...
func runAccountStatus(cmd *cobra.Command, args []string) error {
	...
	// Check if WT_ACCOUNT is overriding
	envAccount := os.Getenv("WT_ACCOUNT")
	...
	if envAccount != "" {
		fmt.Printf("\n%s\n", style.Dim.Render("(set via WT_ACCOUNT environment variable)"))
...
func runAccountSwitch(cmd *cobra.Command, args []string) error {
	...
	if err == nil && fileInfo.Mode()&os.ModeSymlink == 0 && fileInfo.IsDir() {
		...
		if currentHandle != "" {
			...
			return fmt.Errorf("~/.claude is a directory but no default account is set. Please set a default account first with 'wt account default <handle>'")
		}
