...
Use 'wt hook' to see what's on your hook (alias for 'wt mol status').

VIEWING YOUR WORK:
  wt hook              Show what's on your hook
  wt mol current       Show what you should be working on
  wt mol progress      Show execution progress

WORKING ON STEPS:
  wt mol step done     Complete current step (auto-continues)

LIFECYCLE:
  wt mol attach        Attach molecule to your hook
  wt mol detach        Detach molecule from your hook
  wt mol burn          Discard attached molecule (no record)
  wt mol squash        Compress to digest (permanent record)

TO DISPATCH WORK (with molecules):
  wt sling mol-xxx target   # Pour formula + sling to agent
  wt formulas               # List available formulas`,
}
...
Examples:
  wt molecule attach gt-abc mol-xyz  # Explicit pinned bead
  wt molecule attach mol-xyz         # Auto-detect from cwd`,
...
Usage: wt mol attach-from-mail <mail-id>
...
Example:
  wt mol attach-from-mail msg-abc123`,
	Args: cobra.ExactArgs(1),
...
Examples:
  wt mol status                       # Show current agent's hook
  wt mol status greenplace/nux        # Show specific polecat's hook
  wt mol status greenplace/witness    # Show witness's hook`,
...
Examples:
  wt molecule current                 # Current agent's work
  wt molecule current greenplace/furiosa
  wt molecule current deacon
  wt mol current greenplace/witness`,
	Args: cobra.MaximumNArgs(1),
...
2. When done: wt mol step done <step-id>
3. System auto-continues to next ready step

IMPORTANT: Always use 'wt mol step done' to complete steps. Do not manually
