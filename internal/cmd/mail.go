...
Examples:
  wt mail send greenplace/Toast -s "Status check" -m "How's that bug fix going?"
  wt mail send mayor/ -s "Work complete" -m "Finished gt-abc"
  wt mail send whaletown/ -s "All hands" -m "Swarm starting" --notify
  wt mail send greenplace/Toast -s "Task" -m "Fix bug" --type task --priority 1
  wt mail send greenplace/Toast -s "Urgent" -m "Help!" --urgent
  wt mail send mayor/ -s "Re: Status" -m "Done" --reply-to msg-abc123
  wt mail send --self -s "Handoff" -m "Context for next session"
  wt mail send greenplace/Toast -s "Update" -m "Progress report" --cc overseer
  wt mail send list:oncall -s "Alert" -m "System down"`,
...
Examples:
  wt mail inbox                       # Current context (auto-detected)
  wt mail inbox mayor/                # Mayor's inbox
  wt mail inbox greenplace/Toast         # Polecat's inbox
  wt mail inbox --identity greenplace/Toast  # Explicit polecat identity`,
	Args: cobra.MaximumNArgs(1),
	RunE: runMailInbox,
}
...
The message ID can be found from 'wt mail inbox'.
Use 'wt mail mark-read' to mark messages as read.`,
	Aliases: []string{"show"},
...
Examples:
  wt mail archive hq-abc123
  wt mail archive hq-abc123 hq-def456 hq-ghi789`,
...
Examples:
  wt mail mark-read hq-abc123
  wt mail mark-read hq-abc123 hq-def456`,
	Args: cobra.MinimumNArgs(1),
...
Examples:
  wt mail mark-unread hq-abc123
  wt mail mark-unread hq-abc123 hq-def456`,
	Args: cobra.MinimumNArgs(1),
...
Examples:
  wt mail check                           # Simple check (auto-detect identity)
  wt mail check --inject                  # For hooks
  wt mail check --identity greenplace/Toast  # Explicit polecat identity`,
	RunE: runMailCheck,
}
...
Examples:
  wt mail reply msg-abc123 -m "Thanks, working on it now"
  wt mail reply msg-abc123 -s "Custom subject" -m "Reply body"`,
	Args: cobra.ExactArgs(1),
	RunE: runMailReply,
}
...
Pattern examples: "*" (anyone), "whaletown/polecats/*" (specific rig crew).

Examples:
  wt mail claim work-requests   # Claim from specific queue
  wt mail claim                 # Claim from any eligible queue`,
	Args: cobra.MaximumNArgs(1),
	RunE: runMailClaim,
}
...
Examples:
  wt mail release hq-abc123    # Release a claimed message`,
	Args: cobra.ExactArgs(1),
	RunE: runMailRelease,
}
...
SYNTAX:
  wt mail clear              # Clear your own inbox
  wt mail clear <target>     # Clear another agent's inbox
...
Examples:
  wt mail clear                      # Clear your inbox
  wt mail clear whaletown/polecats/joe # Clear joe's inbox
  wt mail clear mayor/               # Clear mayor's inbox`,
	Args: cobra.MaximumNArgs(1),
	RunE: runMailClear,
}
...
Examples:
  wt mail search "urgent"                    # Find messages with "urgent"
  wt mail search "status.*check" --subject   # Regex in subjects only
  wt mail search "error" --from witness      # From witness, containing "error"
  wt mail search "handoff" --archive         # Include archived messages
  wt mail search "" --from mayor/            # All messages from mayor`,
	Args: cobra.ExactArgs(1),
	RunE: runMailSearch,
}
...
SYNTAX:
  wt mail announces              # List all announce channels
  wt mail announces <channel>    # Read messages from a channel
...
Examples:
  wt mail announces              # List all channels
  wt mail announces alerts       # Read messages from 'alerts' channel
  wt mail announces --json       # List channels as JSON`,
	Args: cobra.MaximumNArgs(1),
