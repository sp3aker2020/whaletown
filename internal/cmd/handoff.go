...
import (
	...
	"github.com/speaker20/whaletown/internal/events"
...
This is the canonical way to end any agent session. It handles all roles:
  ...
  - Polecats: Calls 'wt done --status DEFERRED' (Witness handles lifecycle)
...
Examples:
  wt handoff                          # Hand off current session
  wt handoff gt-abc                   # Hook bead, then restart
  wt handoff gt-abc -s "Fix it"       # Hook with context, then restart
  wt handoff -s "Context" -m "Notes"  # Hand off with custom message
  wt handoff -c                       # Collect state into handoff message
  wt handoff crew                     # Hand off crew session
  wt handoff mayor                    # Hand off mayor session
...
The SessionStart hook runs 'wt prime' to restore context.`,
	RunE: runHandoff,
}
...
func runHandoff(cmd *cobra.Command, args []string) error {
	// Check if we're a polecat - polecats use wt done instead
	// WT_POLECAT is set by the session manager when starting polecat sessions
	if polecatName := os.Getenv("WT_POLECAT"); polecatName != "" {
		fmt.Printf("%s Polecat detected (%s) - using wt done for handoff\n",
			style.Bold.Render("🐾"), polecatName)
		// Polecats don't respawn themselves - Witness handles lifecycle
		// Call wt done with DEFERRED exit type to preserve work state
		doneCmd := exec.Command("wt", "done", "--exit", "DEFERRED")
	...
	// The marker is cleared by wt prime after it outputs the warning.
...
func resolveRoleToSession(role string) (string, error) {
	...
	case "crew":
		...
		if rig == "" || crewName == "" {
			return "", fmt.Errorf("cannot determine crew identity - run from crew directory or specify WT_RIG/WT_CREW")
		}
		return fmt.Sprintf("wt-%s-crew-%s", rig, crewName), nil

	case "witness", "wit":
		rig := os.Getenv("WT_RIG")
		if rig == "" {
			return "", fmt.Errorf("cannot determine rig - set WT_RIG or run from rig context")
		}
		return fmt.Sprintf("wt-%s-witness", rig), nil
	...
	case "refinery", "ref":
		...
		return fmt.Sprintf("wt-%s-refinery", rig), nil

	default:
		// Assume it's a direct session name (e.g., gt-whaletown-crew-max)
		return role, nil
	}
}
...
func resolvePathToSession(path string) (string, error) {
	...
	if len(parts) == 3 && parts[1] == "crew" {
		rig := parts[0]
		name := parts[2]
		return fmt.Sprintf("wt-%s-crew-%s", rig, name), nil
	}
	...
	if len(parts) == 3 && parts[1] == "polecats" {
		...
		return fmt.Sprintf("wt-%s-%s", rig, name), nil
	}
	...
	if len(parts) == 2 {
		...
		default:
			...
			// This fixes: wt sling gt-375 whaletown/max failing because max is crew, not polecat.
			townRoot := detectTownRootFromCwd()
			if townRoot != "" {
				crewPath := filepath.Join(townRoot, rig, "crew", second)
				if info, err := os.Stat(crewPath); err == nil && info.IsDir() {
					return fmt.Sprintf("wt-%s-crew-%s", rig, second), nil
				}
			}
			// Not a crew member - treat as polecat name (e.g., whaletown/nux)
			return fmt.Sprintf("wt-%s-%s", rig, secondLower), nil
		}
	}
...
func buildRestartCommand(sessionName string) (string, error) {
	...
	townRoot := detectTownRootFromCwd()
	if townRoot == "" {
		return "", fmt.Errorf("cannot detect town root - run from within a Whale Town workspace")
	}
	...
	// Parse the session name to get the identity (used for WT_ROLE and beacon)
	...
	// 2. export WT_ROLE and BD_ACTOR so role detection works correctly
	...
	if gtRole != "" {
		...
		exports = append(exports, "WT_ROLE="+gtRole)
		...
		if runtimeConfig.Session != nil && runtimeConfig.Session.SessionIDEnv != "" {
			exports = append(exports, "WT_SESSION_ID_ENV="+runtimeConfig.Session.SessionIDEnv)
		}
	}
...
func sessionWorkDir(sessionName, townRoot string) (string, error) {
	...
	case strings.HasSuffix(sessionName, "-witness"):
		...
		rig := strings.TrimPrefix(sessionName, "wt-")
...
// sessionToGTRole converts a session name to a WT_ROLE value.
...
// Examples: "wt-abc123", "bd-ka761", "hq-cv-abc", "beads-xyz", "ap-qtsup.16"
...
func collectHandoffState() string {
	...
	// Get hooked work
	hookOutput, err := exec.Command("wt", "hook").Output()
	if err == nil {
	...
	inboxOutput, err := exec.Command("wt", "mail", "inbox").Output()
	if err == nil {
