...
// Priority:
//  1. WT_ROLE env var → use the role-based identity (agent session)
//  2. No WT_ROLE → try cwd-based detection (witness/refinery/polecat/crew directories)
//  3. No match → return "overseer" (human at terminal)
//
// All Whale Town agents run in tmux sessions with WT_ROLE set at spawn.
// However, cwd-based detection is also tried to support running commands
// from agent directories without WT_ROLE set (e.g., debugging sessions).
func detectSender() string {
	// Check WT_ROLE first (authoritative for agent sessions)
	...
	// No WT_ROLE - try cwd-based detection, defaults to overseer if not in agent directory
	return detectSenderFromCwd()
}

// detectSenderFromRole builds an address from the WT_ROLE and related env vars.
// WT_ROLE can be either a simple role name ("crew", "polecat") or a full address
// ("greenplace/crew/joe") depending on how the session was started.
//
// If WT_ROLE is a simple name but required env vars (WT_RIG, WT_POLECAT, etc.)
...
func detectSenderFromRole(role string) string {
	...
	if strings.Contains(role, "/") {
		// WT_ROLE is already a full address, use it directly
		return role
	}

	// WT_ROLE is a simple role name, build the full address
	switch role {
	case "mayor":
		return "mayor/"
	case "deacon":
