...
var logCmd = &cobra.Command{
	...
	Long: `View the centralized log of Whale Town agent lifecycle events.
...
Examples:
  wt log                     # Show last 20 events
  wt log -n 50               # Show last 50 events
  wt log --type spawn        # Show only spawn events
  wt log --agent greenplace/    # Show events for whaletown rig
  wt log --since 1h          # Show events from last hour
  wt log -f                  # Follow log (like tail -f)`,
	RunE: runLog,
}
...
Examples:
  wt log crash --agent greenplace/Toast --session gt-greenplace-Toast --exit-code 1`,
	RunE: runLogCrash,
}
...
func init() {
	...
	logCmd.Flags().StringVarP(&logAgent, "agent", "a", "", "Filter by agent prefix (e.g., whaletown/, greenplace/crew/max)")
